...
!-- been changed).
    CHARACTER (LEN=12) ::  user_interface_current_revision = '2.0'
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_parin
    ...
    LOGICAL ::  switch_off_module = .FALSE.  !< local namelist parameter to switch off the module
                                             ...
                                             !< the namelist file
    REAL :: lidar_position_x = 0.0 ! PG: lidar parameters
    REAL :: lidar_position_y = 0.0 ! PG: lidar parameters
    REAL :: lidar_position_z = 0.0 ! PG: lidar parameters

    NAMELIST /user_parameters/  lidar_position_x, lidar_position_y, lidar_position_z,              &
...
!-- read (find) a namelist named "user_parameters".
    ...
    WRITE( message_string, * ) 'PG: Lidar position x,y,z = ', lidar_position_x, ',', lidar_position_y, ',', lidar_position_z  ! PG: added an informative message
    CALL message( 'user_parin', 'USR0004', 0, 0, 0, 6, 0 )   ! PG: added an informative message
    WRITE( message_string, * ) 'PG: IO status is ', io_status
    CALL message( 'user_parin', 'USR0004', 0, 0, 0, 6, 0 )
!
...
!
!-- Format-descriptors.
100 FORMAT (//' *** user-interface is activated'/                                                  &
            1X,78('#') // ' User-defined variables and actions:' /                                 &
            ' -----------------------------------'//)
...
!> (i,j,k).
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
