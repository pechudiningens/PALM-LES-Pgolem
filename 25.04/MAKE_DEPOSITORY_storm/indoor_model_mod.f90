...
!-- Define data structure for buidlings.
    TYPE build
       ...
       REAL(wp) ::  air_change_high       !< [1/h] air changes per hour
       REAL(wp) ::  air_change_low        !< [1/h] air changes per hour
    ...
    CHARACTER(LEN=23) ::  high_vent_end_industry_building      = '2019-06-21 22:00:00 +00'  !< end of respective high ventilation
    CHARACTER(LEN=23) ::  high_vent_end_office_building        = '2019-06-21 18:00:00 +00'  !< end of respective high ventilation
    CHARACTER(LEN=23) ::  high_vent_end_residential_building   = '2019-06-21 18:00:00 +00'  !< end of respective high ventilation
    CHARACTER(LEN=23) ::  high_vent_start_industry_building    = '2019-06-21 06:00:00 +00'  !< start of respective high ventilation
    CHARACTER(LEN=23) ::  high_vent_start_office_building      = '2019-06-21 08:00:00 +00'  !< start of respective high ventilation
    CHARACTER(LEN=23) ::  high_vent_start_residential_building = '2019-06-21 08:00:00 +00'  !< start of respective high ventilation
    ...
    INTEGER(iwp) ::  warm_season_start = northward_equinox !< start of warm season (in days of year)
    INTEGER(iwp) ::  warm_season_end = southward_equinox   !< end of warm season (in days of year)
...
!-- Declare all global variables within the module
    ...
    REAL(wp) ::  sod_high_vent_ind_end               !< end time of high ventillation load for industry buildings (in UTC)
    REAL(wp) ::  sod_high_vent_off_end               !< end time of high ventillation load for office buildings (in UTC)
    REAL(wp) ::  sod_high_vent_res_end               !< end time of high ventillation load for residential buildings (in UTC)
    REAL(wp) ::  sod_high_vent_ind_start             !< start time of high ventillation load for industry buildings (in UTC)
    REAL(wp) ::  sod_high_vent_off_start             !< start time of high ventillation load for office buildings (in UTC)
    REAL(wp) ::  sod_high_vent_res_start             !< start time of high ventillation load for residential buildings (in UTC)
    ...
    REAL(wp) ::  time_offset_ind                     !< time offset in case of day change between high ventillation for industry buildings
    REAL(wp) ::  time_offset_off                     !< time offset in case of day change between high ventillation for office buildings
    REAL(wp) ::  time_offset_res                     !< time offset in case of day change between high ventillation for residential buildings
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE im_init
    ...
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  build_ids_unique    !< building IDs on entire model domain,
    ...
    LOGICAL ::  is_summer  !< flag indicating summer season
...
!-- Determine array of all building IDs within the domain where each ID occurs only once.
    ...
    num_build = 0
    IF ( ALLOCATED( build_ids_unique ) )  num_build = SIZE( build_ids_unique )
!
...
!-- allocated on PEs where  the respective building is present (in order to reduce memory demands).
    IF ( num_build > 0 )  ALLOCATE( buildings(1:num_build) )

!
!-- Store building IDs and check if building with certain ID is present on subdomain.
    DO  nb = 1, num_build
       buildings(nb)%id = build_ids_unique(nb)
...
!-- Determine the maximum vertical dimension occupied by each building.
    IF ( num_build > 0 )  ALLOCATE( k_min_l(1:num_build) )
    IF ( num_build > 0 )  ALLOCATE( k_max_l(1:num_build) )
    k_min_l = nzt + 1
    k_max_l = 0
...
!-- Allocate array for counting.
    IF ( num_build > 0 )  ALLOCATE( n_fa(1:num_build) )
    n_fa = 1
...
!-- be individual for each building.
    buildings(:)%theta_int_c_set     = building_indoor(ind_theta_int_c_set,building_type)
    buildings(:)%theta_int_h_set     = building_indoor(ind_theta_int_h_set,building_type)
    buildings(:)%f_c_win             = building_indoor(ind_f_c_win,building_type)
    buildings(:)%g_value_win         = building_indoor(ind_g_value_win,building_type)
    buildings(:)%u_value_win         = building_indoor(ind_u_value_win,building_type)
    buildings(:)%eta_ve              = building_indoor(ind_eta_ve,building_type)
    buildings(:)%factor_a            = building_indoor(ind_factor_a,building_type)
    buildings(:)%factor_c            = building_indoor(ind_factor_c,building_type)
    buildings(:)%lambda_at           = building_indoor(ind_lambda_at,building_type)
    buildings(:)%q_h_max             = building_indoor(ind_q_h_max,building_type)
    buildings(:)%q_c_max             = building_indoor(ind_q_c_max,building_type)
    buildings(:)%qint_high           = building_indoor(ind_qint_high,building_type)
    buildings(:)%qint_low            = building_indoor(ind_qint_low,building_type)
    buildings(:)%height_storey       = building_indoor(ind_height_storey,building_type)
    buildings(:)%height_cei_con      = building_indoor(ind_height_cei_con,building_type)
    buildings(:)%params_waste_heat_h = building_indoor(ind_params_waste_heat_h,building_type)
    buildings(:)%params_waste_heat_c = building_indoor(ind_params_waste_heat_c,building_type)
    buildings(:)%lambda_layer3       = building_hcond(3,ind_wall_gfl,building_type)
    buildings(:)%s_layer3            = building_depth(3,ind_wall_gfl,building_type)
!
...
!
!-- Summer is defined in between warm_season_start and warm_season_end. For the southern
!-- hemisphere it is assumed that the turn of the year lies within the warm season, i.e.
!-- warm_season_start > warm_season_end.
    is_summer = ( latitude >= 0.0_wp  .AND.  day_of_year >= warm_season_start  .AND.               &
                                             day_of_year <= warm_season_end )  .OR.                &
                ( latitude < 0.0_wp   .AND.  day_of_year >= warm_season_start  .OR.                &
                                             day_of_year <= warm_season_end )
       ...
       DO  i = nxl, nxr
          DO  j = nys, nyn
              IF ( building_id_f%var(j,i) /= building_id_f%fill )  THEN
                 ...
                 buildings(nb)%theta_int_c_set     = building_indoor(ind_theta_int_c_set,bt)
                 buildings(nb)%theta_int_h_set     = building_indoor(ind_theta_int_h_set,bt)
                 buildings(nb)%f_c_win             = building_indoor(ind_f_c_win,bt)
                 buildings(nb)%g_value_win         = building_indoor(ind_g_value_win,bt)
                 buildings(nb)%u_value_win         = building_indoor(ind_u_value_win,bt)
                 buildings(nb)%eta_ve              = building_indoor(ind_eta_ve,bt)
                 buildings(nb)%factor_a            = building_indoor(ind_factor_a,bt)
                 buildings(nb)%factor_c            = building_indoor(ind_factor_c,bt)
                 buildings(nb)%lambda_at           = building_indoor(ind_lambda_at,bt)
                 buildings(nb)%q_h_max             = building_indoor(ind_q_h_max,bt)
                 buildings(nb)%q_c_max             = building_indoor(ind_q_c_max,bt)
                 buildings(nb)%qint_high           = building_indoor(ind_qint_high,bt)
                 buildings(nb)%qint_low            = building_indoor(ind_qint_low,bt)
                 buildings(nb)%height_storey       = building_indoor(ind_height_storey,bt)
                 buildings(nb)%height_cei_con      = building_indoor(ind_height_cei_con,bt)
                 buildings(nb)%params_waste_heat_h = building_indoor(ind_params_waste_heat_h,bt)
                 buildings(nb)%params_waste_heat_c = building_indoor(ind_params_waste_heat_c,bt)
                 buildings(nb)%lambda_layer3       = building_hcond(3,ind_wall_gfl,bt)
                 buildings(nb)%s_layer3            = building_depth(3,ind_wall_gfl,bt)

                 IF ( day_of_year >= northward_equinox  .AND.  day_of_year <= southward_equinox )  &
                 THEN
...
!-- Otherwise amount between air and surface taken account twice.
    DO nb = 1, num_build
       IF ( buildings(nb)%on_pe ) THEN
          ...
          u_tmp = buildings(nb)%u_value_win * ( du_win_tmp / ( du_win_tmp - 0.125_wp              &
                                                             + ( 1.0_wp / h_is ) ) )
...
!-- Initially, compute 10-cm temperature.
    CALL calc_pt_10cm
!
!-- Determine start- and end-time of high ventillation loads in UTC.
    CALL get_date_time( 0.0_wp,                                                                    &
                        reference_date_time_str = high_vent_start_industry_building,               &
                        second_of_day = sod_high_vent_ind_start )
    CALL get_date_time( 0.0_wp,                                                                    &
                        reference_date_time_str = high_vent_start_office_building,                 &
                        second_of_day = sod_high_vent_off_start )
    CALL get_date_time( 0.0_wp,                                                                    &
                        reference_date_time_str = high_vent_start_residential_building,            &
                        second_of_day = sod_high_vent_res_start )

    CALL get_date_time( 0.0_wp,                                                                    &
                        reference_date_time_str = high_vent_end_industry_building,                 &
                        second_of_day = sod_high_vent_ind_end )
    CALL get_date_time( 0.0_wp,                                                                    &
                        reference_date_time_str = high_vent_end_office_building,                   &
                        second_of_day = sod_high_vent_off_end )
    CALL get_date_time( 0.0_wp,                                                                    &
                        reference_date_time_str = high_vent_end_residential_building,              &
                        second_of_day = sod_high_vent_res_end )
!
!-- Consider offset times in case of day change between times.
    time_offset_ind = 0.0_wp
    IF ( sod_high_vent_ind_start >= sod_high_vent_ind_end )  THEN
       time_offset_ind = sod_high_vent_ind_start
       sod_high_vent_ind_end = sod_high_vent_ind_end + sod_high_vent_ind_start
    ENDIF

    time_offset_off = 0.0_wp
    IF ( sod_high_vent_off_start >= sod_high_vent_off_end )  THEN
       time_offset_off = sod_high_vent_off_start
       sod_high_vent_off_end = sod_high_vent_off_end + sod_high_vent_off_start
    ENDIF

    time_offset_res = 0.0_wp
    IF ( sod_high_vent_res_start >= sod_high_vent_res_end )  THEN
       time_offset_res = sod_high_vent_res_start
       sod_high_vent_res_end = sod_high_vent_res_end + sod_high_vent_res_start
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE im_main_heatcool
    ...
    LOGICAL  ::  industry_high_vent     !< flag indicating time of high ventillation load for industry buildings
    LOGICAL  ::  office_high_vent       !< flag indicating time of high ventillation load for industry buildings
    LOGICAL  ::  residential_high_vent  !< flag indicating time of high ventillation load for industry buildings
...
!
!-- Determine time of day in seconds.
    CALL get_date_time( time_since_reference_point, second_of_day = second_of_day )

    industry_high_vent = ( sod_high_vent_ind_start <= second_of_day + time_offset_ind  .AND.       &
                           second_of_day + time_offset_ind <= sod_high_vent_ind_end )

    office_high_vent = ( sod_high_vent_off_start <= second_of_day + time_offset_off  .AND.         &
                         second_of_day + time_offset_off <= sod_high_vent_off_end )

    residential_high_vent = ( sod_high_vent_res_start <= second_of_day + time_offset_res  .AND.    &
                              second_of_day + time_offset_res <= sod_high_vent_res_end )
!
...
!--          During spinup set window fraction to zero and add these to wall fraction.
             frac_win   = MERGE( surf_usm%frac(m,ind_wat_win), 0.0_wp, .NOT. spinup_phase )
...
!--          Building geometries --> not time-dependent
             IF ( surf_usm%upward(m)  .OR.  surf_usm%downward(m) )  THEN
!
!--             Surface area per facade element in [m2].
                facade_element_area = dx * dy
             ELSEIF( surf_usm%northward(m)  .OR.  surf_usm%southward(m) )  THEN
!
!--             Surface area per facade element in  [m2].
                facade_element_area = dx * dzw(kk+1)
             ELSEIF( surf_usm%eastward(m)  .OR.  surf_usm%westward(m) )  THEN
!
!--             Surface area per facade element in [m2].
                facade_element_area = dy * dzw(kk+1)
             ENDIF
!
!--          Floor area per facade area in [m2/m2].
             floor_area_per_facade        = buildings(nb)%fapf
!
!--          Indoor air volume per facade area in [m3/m2].
             indoor_volume_per_facade     = buildings(nb)%vpf(kk)
!
!--          Area of total facade in [m2].
             buildings(nb)%area_facade    = facade_element_area *                                  &
                                            ( buildings(nb)%num_facades_per_building_h +           &
                                              buildings(nb)%num_facades_per_building_v )
!
!--          Window area per facade element in [m2].
             window_area_per_facade       = frac_win  * facade_element_area

             buildings(nb)%net_floor_area = buildings(nb)%vol_tot / buildings(nb)%height_storey
!
!--          Area of all surfaces in [m2], pointing to zone  Eq. (9) according to section 7.2.2.2
             total_area                   = buildings(nb)%net_floor_area
!
!--          Standard values in [m2], according to Table 12 section 12.3.1.2 (calculate over
!--          Eq. (65) according to section 12.3.1.2).
             a_m                          = buildings(nb)%factor_a * total_area *                  &
                                            ( facade_element_area / buildings(nb)%area_facade ) *  &
                                            buildings(nb)%lambda_at
!
!--          Standard values in [J/K], according to table 12 section 12.3.1.2 (calculate over
!--          Eq. (66) according to section 12.3.1.2).
             c_m                          = buildings(nb)%factor_c * total_area *                  &
                                            ( facade_element_area / buildings(nb)%area_facade )
!
!--          Calculation of heat transfer coefficient for transmission --> not time-dependent
!--          In [W/K], only for windows.
             h_t_es   = window_area_per_facade * buildings(nb)%h_es
!
!--          In [W/K], with h_is = 3.45 W / (m2 K) between surface and air, Eq. (9).
             h_t_is  = buildings(nb)%area_facade * h_is
!
!--          In[W/K], with h_ms = 9.10 W / (m2 K) between component and surface, Eq. (64).
             h_t_ms  = a_m * h_ms
!
!--          In [W/K], [W/K] for opaque components.
             h_t_wall  = 1.0_wp / ( 1.0_wp / ( ( facade_element_area - window_area_per_facade )    &
                                    * buildings(nb)%lambda_layer3 / buildings(nb)%s_layer3 * 0.5_wp&
                                             ) + 1.0_wp / h_t_ms )
!
!--          In [W/K], emmision Eq. (63), Section 12.2.2.
             h_t_wm  = 1.0_wp / ( 1.0_wp / h_t_wall - 1.0_wp / h_t_ms )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE im_data_output_3d( av, variable, found, local_pf, resorted, mask_topography, nzb_do,   &
                               nzt_do )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE im_parin
    ...
    NAMELIST /indoor_parameters/  high_vent_end_industry_building,                                 &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE im_wrd_local
    ...
    IF ( TRIM( restart_data_format_output ) == 'fortran_binary' )  THEN
       ...
       CALL wrd_write_string( 'im_start_index' )
       WRITE ( 14 )  surf_usm%start_index

       CALL wrd_write_string( 'im_end_index' )
