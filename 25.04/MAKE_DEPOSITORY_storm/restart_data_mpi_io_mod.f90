...
!
!-- File position in bytes, from which on 2d/3d-arrays are written.
!-- Given size corresponds to 128 kByte and should already prepare for the case that
!-- int_names and real_names are extended from 32 to 64 characters.
    array_position  = 131072
...
!--    INTEGER, DIMENSION(nys:nyn,nxl:nxr) ::  data
       ...
       CALL write_on_first_core_along_x
...
!--    INT. Maybe a compiler problem.
       array_position = array_position + INT( (ny+1), KIND=rd_offset_kind ) *                      &
                                         INT( (nx+1), KIND=rd_offset_kind ) * 8
 ...
 CONTAINS
    ...
    SUBROUTINE write_on_first_core_along_x
       ...
       INTEGER(idp), DIMENSION(:,:), ALLOCATABLE ::  buf1  !<
       INTEGER(idp), DIMENSION(:,:), ALLOCATABLE ::  buf3  !<
       INTEGER(idp), DIMENSION(:,:), ALLOCATABLE ::  buf5  !<

       INTEGER(idp), DIMENSION(:,:,:), ALLOCATABLE ::  buf2  !<


       nnx_max = MAXVAL( nnx_pe )
       nny_max = MAXVAL( nny_pe )
!
!--    nny has the same value on all cores along x.
       ALLOCATE( buf1(nxl:nxl+nnx_max-1,nys:nys+nny-1) )
       ALLOCATE( buf2(nxl:nxl+nnx_max-1,nys:nys+nny-1,0:npex-1) )
       ...
       DO  j = nys, nyn
          DO  i = nxl, nxr
             buf1(i,j) = data(j-nys+1,i-nxl+1)
          ENDDO
       ENDDO

!
!--    Gather data of PEs along x in buf2.
       nr = SIZE( buf1 )
       CALL MPI_GATHER( buf1, nr, MPI_INTEGER8, buf2, nr, MPI_INTEGER8, 0, comm1dx, ierr)
       ...
       IF ( myidx == 0 )  THEN
          ...
          IF( sm_io%is_sm_active() )  THEN
             ALLOCATE( buf3(0:nx,nys:nys+nny_max-1) )
          ELSE
             ALLOCATE( buf3(0:nx,nys:nyn) )
          ENDIF
!
!--       Store data in buffer that is global along x.
          DO  ip = 0, npex-1
             DO  j = nys,nyn
                DO  i = nxl_pe(ip), nxr_pe(ip)
                   buf3(i,j) = buf2(i-nxl_pe(ip)+nxl,j,ip)
                ENDDO
             ENDDO
          ENDDO
!
!--       In the IF part only PE0 writes the global 2d array (restart_data_format =
!--       'mpi_shared_memory'), in the ELSE part all PEs along y (with myidx = 0) are writing
!--       (restart_data_format = 'mpi').
          IF ( sm_io%is_sm_active() )  THEN
             ...
             tag = 313
!
!--          Write only on PE0. PE0 collects data from the other PEs along y, which are send
!--          in the ELSE branch.
             IF ( myid == 0 )   THEN

                ALLOCATE( buf5(0:nx,0:ny) )
                DO  jp = 0, npey-1

                   IF ( jp >= 1 )  THEN
!
!--                   Receive Data from all y-PEs for myidx == 0. Re-use buf3 for all jp loop
!--                   iterations
!--                   jp == 0 must not be treated because data is already available in local buf3.
                      CALL MPI_RECV( buf3, nr, MPI_INTEGER8, jp, tag, comm1dy, MPI_STATUS_IGNORE,  &
                                     ierr)
                   ENDIF
!
!--                Store received data in buf5.
                   DO  j = nys_pe(jp), nyn_pe(jp)
                      DO  i = 0, nx
                         buf5(i,j) = buf3(i,j-nys_pe(jp)+nys)
                      ENDDO
                   ENDDO

                ENDDO

                offset = array_position
                nr = ( nx+1 ) * ( ny+1 )
!
!--             Write the global 2d array.
                CALL MPI_FILE_SEEK( fh, offset, MPI_SEEK_SET, ierr )
                CALL MPI_FILE_WRITE( fh, buf5, nr, MPI_INTEGER8, status, ierr )
                DEALLOCATE( buf5 )

             ELSE

                CALL MPI_SEND( buf3, nr, MPI_INTEGER8, 0, tag, comm1dy, ierr )

             ENDIF

          ELSE
!
!--          All PEs along y are writing the data that has been gathered along x.
             offset = array_position
             DO  ip = 1, myidy
                offset = offset + ( nny_pe(ip-1) * (nx+1) ) * 8
             ENDDO
             nr = ( nx+1 ) * nny
             CALL MPI_FILE_SEEK( fh, offset, MPI_SEEK_SET, ierr )
             CALL MPI_FILE_WRITE( fh, buf3, nr, MPI_INTEGER8, status, ierr )
    ...
    END SUBROUTINE write_on_first_core_along_x
#endif
