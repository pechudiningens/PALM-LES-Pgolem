...
!--    (particle release is only allow in the root domain).
       ...
       CALL MPI_BCAST( pts_increment, 1, MPI_INTEGER8, 0, MPI_COMM_WORLD, ierr )
...
!--    Particle attributes.
       ...
       IF ( particle_coupling  .AND.  .NOT. root_model )  THEN
!
!--       No output of features for childs if particle coupling is switched on. All features
!--       are output in the root model.
          WRITE ( io, 484 )
       ...
       ELSE
          ...
          DO  i = 1, number_of_particle_groups
             ...
             ELSE
                WRITE ( io, 492 )
             ENDIF
             WRITE ( io, 493 )  psl(i), psr(i), pss(i), psn(i), psb(i), pst(i), pdx(i), pdy(i),    &
                                pdz(i)
...
484 FORMAT ('       For all further particle features please see _rc file of the root model.')
...
!
!-- Trilinear interpolation does not work for vertically shifted child domains.
    IF ( particle_coupling  .AND.  child_domain  .AND.  nest_shift_z /= 0.0_wp  .AND.              &
         particle_advection_interpolation == 'trilinear' )  THEN
       message_string = 'particle_advection_interpolation = "trilinear" not allowed in case of' // &
                        ' vertically shifted child domains'
       CALL message( 'lpm_check_parameters', 'LPM0034', 1, 2, 0, 6, 0 )
    ENDIF

    IF ( particle_advection_start >= end_time )  THEN
       message_string = 'particle_advection_start >= end_time not allowed'
       CALL message( 'lpm_check_parameters', 'LPM0035', 1, 2, 0, 6, 0 )
    ENDIF
...
!--    Also note that for elevated child domains, trilinear interpolation (which uses the
!--    logarithmic interpolation) is not allowed, and the roughness length calculated below is not
!--    used.
       z0_av_local = 0.0_wp
       DO  m = 1, surf_def%ns
       ...
       IF ( ( surf_def%ns_tot_up + surf_lsm%ns_tot_up + surf_usm%ns_tot_up ) == 0 )  THEN
!
!--       An elevated child may have no bottom surface. Set a dummy value for roughness. This
!--       will never come into effect, because no logarithmic interpolation will be done lateron.
          z0_av_global = 0.001 * z_p
       ELSE
...
#if defined( __parallel )
!
!-- Initialize settings for nested runs, e.g. nested boundary conditions.
...
!--                               In case of stretching the actual k index is found iteratively
                                  ...
                                  ELSE
                                     kp = INT( (tmp_particle%z - nest_shift_z) / dz(1) + 1 + offset_ocean_nzt )
                                  ENDIF
!
...
!--             The MOD function allows for changes in the output interval with restart runs.
             ...
             ENDIF
!
!--          At the beginning of the particle transport, output the particle data to set the initial
!--          values of the time series.
             IF ( first_call_lpm )  THEN
...
!--             Horizontal boundary conditions including exchange between subdmains
                CALL lpm_exchange_horiz
!
!--             IF .FALSE., lpm_sort_and_delete is done inside pmcp.
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lpm_write_exchange_statistics
    ...
    INTEGER(iwp) ::  child_number_of_particles  !< number of particles of all childs attached to this (parent) model
    INTEGER(iwp) ::  ip                         !< loop index
    INTEGER(iwp) ::  jp                         !< loop index
    INTEGER(iwp) ::  kp                         !< loop index
    INTEGER(iwp) ::  tot_number_of_particles    !< total number of particles of this (parent) model
#if defined( __parallel )
    INTEGER(iwp) ::  child_id                   !< id of child
    INTEGER(iwp) ::  m                          !< loop index

    INTEGER(iwp), DIMENSION(2) ::  nr_recv  !< number of particles (received value)
    INTEGER(iwp), DIMENSION(2) ::  nr_send  !< number of particles (sent value)
#endif
    LOGICAL, SAVE ::  first_call = .TRUE.  !< switch for header output
...
!
!-- Calculate total number of particles of this (parent) model and number of particles that
!-- reside on the childs attached to this model.
#if defined( __parallel )
...
#endif
    child_number_of_particles = 0
#if defined( __parallel )
    IF ( myid == 0 )  THEN

       IF ( .NOT. root_model )  THEN
          nr_send(1) = tot_number_of_particles
          CALL pmc_send_to_parent( nr_send, 1, 0, 400, ierr )
       ENDIF

       DO  m = 1, SIZE( pmc_parent_for_child ) - 1
          child_id = pmc_parent_for_child(m)
          CALL pmc_recv_from_child( child_id, nr_recv, 1, 0, 400, ierr )
          child_number_of_particles = child_number_of_particles + nr_recv(1)
       ENDDO

    ENDIF
#endif
!
!-- Output data. Only id 0 prints the total numbers of particles in case it is a parent model.
#if defined( __parallel )
    IF ( myid == 0  .AND.  SIZE( pmc_parent_for_child ) > 1 )  THEN
       WRITE ( 80, 8001 )  current_timestep_number+1, simulated_time+dt_3d, number_of_particles,   &
                           pleft, trlp_count_sum, trlp_count_recv_sum, pright, trrp_count_sum,     &
                           trrp_count_recv_sum, psouth, trsp_count_sum, trsp_count_recv_sum,       &
                           pnorth, trnp_count_sum, trnp_count_recv_sum,                            &
                           tot_number_of_particles + child_number_of_particles,                    &
                           tot_number_of_particles, child_number_of_particles
    ENDIF
    WRITE ( 80, 8001 )  current_timestep_number+1, simulated_time+dt_3d, number_of_particles,      &
                        pleft, trlp_count_sum, trlp_count_recv_sum, pright, trrp_count_sum,        &
...
8000 FORMAT (A/                                                                                    &
             ...
             'sPE sent/recv  nPE sent/recv    tot # of parts   parent # parts  child # parts '/    &
             141('-'))
8001 FORMAT (I6,1X,F7.2,4X,I10,5X,4(I3,1X,I4,'/',I4,2X),6X,I10,7X,I10,5X,I10)
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lpm_advec( ip, jp, kp )
    ...
    INTEGER(iwp) ::  first_subbox                !< start index for loop over subboxes in particle advection
    INTEGER(iwp) ::  i                           !< index variable along x
    ...
    INTEGER(iwp) ::  last_subbox                 !< end index for loop over subboxes in particle advection
    ...
    INTEGER(iwp), DIMENSION(0:7) ::  prt_end_index   !< start particle index for current sub-box
    INTEGER(iwp), DIMENSION(0:7) ::  prt_start_index !< start particle index for current sub-box
...
!
!-- Set index range of particles. In case of no SGS-velocities, all particles are treated, otherwise
!-- only those that didn't reach the 3d-model timestep so far. In case of the simple interpolation
!-- methods, no sub-box partitioning is used, and the respective indices are stored in
!-- prt_start_index(0) and prt_end_index(0).
    first_subbox = 0
    last_subbox  = 0
    IF ( interpolation_trilinear )  last_subbox  = 7

    prt_start_index(:) = grid_particles(kp,jp,ip)%start_index(:)
    prt_end_index(:)   = grid_particles(kp,jp,ip)%end_index(:)
!
...
!--       Interpolate u velocity-component
          ...
          DO  n = prt_start_index(nb), prt_end_index(nb)
!
...
!--          Determine vertical index of topography top
             ...
             IF ( constant_flux_layer  .AND.  zv(n) - zw(k_wall) < z_p .AND. nest_shift_z == 0.0)  THEN
!
...
!--          In case of topography check if subbox is adjacent to a wall
             ...
             IF ( subbox_at_wall )  THEN
                e_int(prt_start_index(nb):prt_end_index(nb))     = e(kp,jp,ip)
                diss_int(prt_start_index(nb):prt_end_index(nb))  = diss(kp,jp,ip)
                de_dx_int(prt_start_index(nb):prt_end_index(nb)) = de_dx(kp,jp,ip)
                de_dy_int(prt_start_index(nb):prt_end_index(nb)) = de_dy(kp,jp,ip)
                de_dz_int(prt_start_index(nb):prt_end_index(nb)) = de_dz(kp,jp,ip)
!
!--             Set flag for stochastic equation.
                term_1_2(prt_start_index(nb):prt_end_index(nb)) = 0.0_wp
             ELSE
                i = ip + block_offset(nb)%i_off
                j = jp + block_offset(nb)%j_off
                k = kp + block_offset(nb)%k_off
...
!--             SGS particle velocities at the next timestep.
       ...
       ELSEIF ( interpolation_simple_corrector  .OR.  interpolation_simple_predictor )  THEN
          ...
          e_int(prt_start_index(0):prt_end_index(0))     = e(kp,jp,ip)
          diss_int(prt_start_index(0):prt_end_index(0))  = diss(kp,jp,ip)
          de_dx_int(prt_start_index(0):prt_end_index(0)) = de_dx(kp,jp,ip)
          de_dy_int(prt_start_index(0):prt_end_index(0)) = de_dy(kp,jp,ip)
          de_dz_int(prt_start_index(0):prt_end_index(0)) = de_dz(kp,jp,ip)
!
!--       Set flag for stochastic equation.
          term_1_2(1:number_of_particles) = 0.0_wp
!
!--       k = kp - 1
          DO  n = prt_start_index(0), prt_end_index(0)
             IF ( zu(kp) > zv(n) ) THEN
                k = kp -1
             ELSE
                k = kp
...
!--          current LES timestep.
             dt_gap(n) = dt_3d - particles(n)%dt_sum
             dt_particle(n) = MIN( dt_3d, 0.025_wp * lagr_timescale(n), dt_gap(n) )
             IF ( dt_particle(n) == 0.0_wp )  THEN
                print*, '### n = ', n, ' dt_3d = ', dt_3d, ' lagr_timescale = ', lagr_timescale(n), ' dt_gap = ', dt_gap(n)
             ENDIF
             particles(n)%aux1 = lagr_timescale(n)
!
!--          The particle timestep should not be too small in order to prevent the number of
...
!
!--    Loop over subboxes, or over the whole gridbox (in case of simple interpolation scheme).
       DO  nb = first_subbox, last_subbox
!
...
!-- Loop over subboxes.
    DO  nb = first_subbox, last_subbox
!
!--    Increment the particle age and the total time that the particle has advanced within the
!--    particle timestep procedure.
       ...
       ENDDO
!
!--    Particles that leave the child domain during the SGS-timestep loop must not continue
!--    timestepping until they are transferred to the parent. Hence, set their dt_sum to dt.
       IF ( child_domain  .AND.  use_sgs_for_particles )  THEN
          DO  n = prt_start_index(nb), prt_end_index(nb)
             IF ( particles(n)%x < 0.0_wp  .OR.  particles(n)%x > ( nx+1 ) * dx  .OR.              &
                  particles(n)%y < 0.0_wp  .OR.  particles(n)%y < ( ny+1 ) * dy )                  &
             THEN
...
!--    delete those particles, which are older than allowed. In case of nested boundary conditions
!--    (ibc_par_.. = 3) particles are not treated here.
       DO  n = 1, number_of_particles
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE turbsd
    ...
    eta       = SQRT( SQRT( molecular_viscosity**3 / epsilon_collision ) )
    vk        = eta / tauk
...
!--    In case of stretching the actual k index must be found
       ...
       ELSE
          kp = INT( (particle_array(n)%z - nest_shift_z) / dz(1) + 1 + offset_ocean_nzt )
       ENDIF

!
!--    Particles beyond bottom and top boundary will be treated later in nest exchange. They will
!--    be just in the vertical box they stayed before.
!--    The same holds for particles that left the total domain of a child along x. They have not
!--    been treated in the x boundary exchange, and so will not be treated here.
       kp = MAX( kp, nzb+1 )
       kp = MIN( kp, nzt   )
       IF ( nxl ==  0 )  ip = MAX( ip,  0 )
       IF ( nxr == nx )  ip = MIN( ip, nx )
       IF ( jp == -1    .AND.  ibc_par_ns == 3 )  jp = 0
       IF ( jp == ny+1  .AND.  ibc_par_ns == 3 )  jp = ny

       IF ( ( ip >= nxl    .AND.  ip <= nxr  .AND.  jp >= nys  .AND.  jp <= nyn  .AND.             &
            kp >= nzb+1  .AND.  kp <= nzt ) )  THEN
!
!--       This particle stays on the PE and will be sorted into the respective grid box.
!--       Note, that its y-coordinate (or z-coordinate) may be outside the subdomain. It will be
!--       send to the respective PE (or to the parent domain) in a later step.
          number_of_particles = prt_count(kp,jp,ip)
          particles => grid_particles(kp,jp,ip)%particles(1:number_of_particles)
...
!--             At first, allocate further temporary array to swap particle information.
                ...
                temp_ns(1:nr_move_north-1) = move_also_north(1:nr_move_north-1)
                DEALLOCATE( move_also_north )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE realloc_particles_array ( i, j, k, size_in )
    ...
    TYPE(particle_type), DIMENSION(:), ALLOCATABLE ::  tmp_particles_d !< temporary particle array for sorting larger number of particles
!
!-- SAVE attribute to avoid gfortran compiler warning about moving array from stack to static storage.
    TYPE(particle_type), DIMENSION(500), SAVE      ::  tmp_particles_s !< temporary particle array with fixed size for sorting smaller number of particles
...
!
!--    Fixed size array is used to avoid possible memory defragmentation.
...
!--             Repack particles array, i.e. delete particles and recalculate number of particles
                ...
                prt_count(kp,jp,ip) = number_of_particles
!
!--             Although no sub-boxes are used, start and end indices must be set, which are
!--             required to decide which particles need to be treated for sub-timesteps in case
!--             that SGS-vwelocities are used.
                grid_particles(kp,jp,ip)%start_index(0) = 1
                grid_particles(kp,jp,ip)%end_index(0) = number_of_particles
             ENDDO
          ENDDO
       ENDDO
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lpm_sort_timeloop_done
    ...
    INTEGER(iwp) ::  first_subbox  !< first sub-box to be treated
    ...
    INTEGER(iwp) ::  last_subbox    !< last sub-box to be treated
    ...
    INTEGER(iwp) ::  prt_end_index   !< start particle index for current sub-box
    INTEGER(iwp) ::  prt_start_index !< start particle index for current sub-box

    TYPE(particle_type), DIMENSION(:), ALLOCATABLE ::  sort_particles  !< temporary particle array


!
!-- Decide whether the particle loop has to run over the 8 subboxes or over the whole grid box.
    first_subbox = 0
    last_subbox  = 0
    IF ( interpolation_trilinear )  last_subbox  = 7
       ...
       DO  j = nys, nyn
          DO  k = nzb+1, nzt
             ...
             DO  nb = first_subbox, last_subbox
                ...
                prt_start_index = grid_particles(k,j,i)%start_index(nb)
                prt_end_index   = grid_particles(k,j,i)%end_index(nb)
!
!--             Allocate temporary array used for sorting.
                ALLOCATE( sort_particles(prt_start_index:prt_end_index) )
!
!--             Determine number of particles already completed the LES timestep, and write them
!--             into a temporary array.
                nf = prt_start_index
                num_finalized = 0
                DO  n = prt_start_index, prt_end_index
...
!--             Write back sorted particles
                particles(prt_start_index:prt_end_index) =                                         &
                                                       sort_particles(prt_start_index:prt_end_index)
!
