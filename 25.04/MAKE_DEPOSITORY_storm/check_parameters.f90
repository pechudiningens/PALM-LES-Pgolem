...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE check_parameters
    ...
    REAL(wp)    ::  dum                              !< dummy value
    REAL(wp)    ::  section_position                 !< relative position of cross sections in m
    REAL(wp)    ::  time_interval                    !< length of output time interval
...
!
!--    Compute initial ug/vg-profile using the given velocity gradients.
       CALL init_vertical_profiles( ug_vertical_gradient_level_ind, ug_vertical_gradient_level,    &
                                    ug_vertical_gradient, ug, ug_surface, dum, 'ug' )
       CALL init_vertical_profiles( vg_vertical_gradient_level_ind, vg_vertical_gradient_level,    &
                                    vg_vertical_gradient, vg, vg_surface, dum, 'vg' )
...
!--    Compute initial temperature profile using the given temperature gradients
       CALL init_vertical_profiles( pt_vertical_gradient_level_ind, pt_vertical_gradient_level,    &
                                    pt_vertical_gradient, pt_init, pt_surface, bc_pt_t_val, 'pt' )
!
!--    Compute initial humidity profile using the given humidity gradients
       IF ( humidity )  THEN
          CALL init_vertical_profiles( q_vertical_gradient_level_ind, q_vertical_gradient_level,   &
                                       q_vertical_gradient, q_init, q_surface, bc_q_t_val, 'q' )
       ENDIF
!
!--    Compute initial scalar profile using the given scalar gradients
       IF ( passive_scalar )  THEN
          CALL init_vertical_profiles( s_vertical_gradient_level_ind, s_vertical_gradient_level,   &
                                       s_vertical_gradient, s_init, s_surface, bc_s_t_val, 's' )
       ENDIF
...
!-- Initialize large scale subsidence if required
    If ( large_scale_subsidence )  THEN
       IF ( subs_vertical_gradient_level(1) /= HUGE( 1.0_wp )  .AND. .NOT. large_scale_forcing )   &
       THEN
...
!--    LSF_DATA.
       ...
       ENDIF
    ELSE
        IF ( subs_vertical_gradient_level(1) /= HUGE( 1.0_wp ) )  THEN
...
!-- Set wind speed in the Galilei-transformed system
    IF ( galilei_transformation )  THEN
       IF ( use_ug_for_galilei_tr  .AND.  .NOT. ( ANY( ug_vertical_gradient /= 0.0_wp )  .OR.      &
                                                  ANY( vg_vertical_gradient /= 0.0_wp ) ) )        &
       THEN
          u_gtrans = ug_surface * 0.6_wp
          v_gtrans = vg_surface * 0.6_wp
       ELSEIF ( use_ug_for_galilei_tr  .AND.  ANY( ug_vertical_gradient /= 0.0_wp ) )  THEN
          message_string = 'galilei transformation does not allow height depending ug'
          CALL message( 'check_parameters', 'PAC0069', 1, 2, 0, 6, 0 )
       ELSEIF ( use_ug_for_galilei_tr  .AND.  ANY( vg_vertical_gradient /= 0.0_wp ) )  THEN
...
!-- This IF clause needs revision, got too complex!!
    ...
    IF ( top_heatflux == 9999999.9_wp )  THEN
       IF ( .NOT. ocean_mode  .AND.  use_top_fluxes )  THEN
          message_string = 'top_heatflux is not set for use_top_fluxes = .T.'
          CALL message( 'check_parameters', 'PAC0356', 1, 2, 0, 6, 0 )
       ENDIF
...
!-- ATTENTION: the additional condition ibc_pt_b == 3 (nested at bottom boundary) may be removed
!--            lateron, if it turns out that surface layer fluxes are not calculated for bottom
!--            boundary grid points in such a case (because they are actually located in the
!--            atmosphere).
...
!
!-- Determine the total number of module specific profiles and sizes required by flow statistics.
    pr_total_modules = max_pr_cs + max_pr_det + max_pr_salsa + max_pr_user
    pr_total         = pr_palm + pr_total_modules
    ngp_sums         = ( nz + 2 ) * pr_total
...
!
!-- Averaged 2d, 3d, or masked output requires that an averaging interval has been set.
...
!--    output.
       DO  i = 1, masks
          WRITE( message_string, '(A,I3.3,A)' )  'dt_domask(',i,')'
          CALL check_dt_do( dt_domask(i), message_string )
       ENDDO
...
!--    Note that end_time already includes spinup_time at this point, but time_restart not.
!--    The total output time interval for this run as calculated below can be negative, if
!--    the skip time is set larger than the end time.
       DO  i = 1, masks

          time_interval = MIN( end_time, time_restart + spinup_time ) -                            &
                          MAX( MERGE( skip_time_domask(i), skip_time_domask(i) + spinup_time,      &
                                      data_output_during_spinup ),                                 &
                               simulated_time_at_begin )
          IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
          ntdim_masks(i,0) = CEILING( time_interval / dt_domask(i) )

          time_interval = MIN( end_time, time_restart + spinup_time  ) -                           &
                          MAX( MERGE( skip_time_data_output_av, skip_time_data_output_av +         &
                                      spinup_time, data_output_during_spinup ),                    &
                               simulated_time_at_begin )
          IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
          ntdim_masks(i,1) = CEILING( time_interval / dt_data_output_av )

       ENDDO

       time_interval = MIN( end_time, time_restart + spinup_time ) -                               &
                       MAX( MERGE( skip_time_do3d, skip_time_do3d + spinup_time,                   &
                                   data_output_during_spinup ),                                    &
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_3d(0) = CEILING( time_interval / dt_do3d )
       ...
       time_interval = MIN( end_time, time_restart + spinup_time  ) -                              &
                       MAX( MERGE( skip_time_data_output_av, skip_time_data_output_av +            &
                            ...
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_3d(1) = CEILING( time_interval / dt_data_output_av )

       time_interval = MIN( end_time, time_restart + spinup_time  ) -                              &
                       MAX( MERGE( skip_time_do2d_xy, skip_time_do2d_xy + spinup_time,             &
                                   data_output_during_spinup ),                                    &
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_2d_xy(0) = CEILING( time_interval / dt_do2d_xy )

       time_interval = MIN( end_time, time_restart + spinup_time  ) -                              &
                       MAX( MERGE( skip_time_do2d_xz, skip_time_do2d_xz + spinup_time,             &
                                   data_output_during_spinup ),                                    &
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_2d_xz(0) = CEILING( time_interval / dt_do2d_xz )

       time_interval = MIN( end_time, time_restart + spinup_time  ) -                              &
                       MAX( MERGE( skip_time_do2d_yz, skip_time_do2d_yz + spinup_time,             &
                                   data_output_during_spinup ),                                    &
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_2d_yz(0) = CEILING( time_interval / dt_do2d_yz )
...
!--    parameter.
       time_interval = MIN( end_time, time_restart + spinup_time  ) -                              &
                       MAX( MERGE( skip_time_data_output_av, skip_time_data_output_av +            &
                            ...
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_2d_xy(1) = CEILING( time_interval / dt_data_output_av )
       ...
       time_interval = MIN( end_time, time_restart + spinup_time  ) -                              &
                       MAX( MERGE( skip_time_data_output_av, skip_time_data_output_av +            &
                            ...
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_2d_xz(1) = CEILING( time_interval / dt_data_output_av )
       ...
       time_interval = MIN( end_time, time_restart + spinup_time  ) -                              &
                       MAX( MERGE( skip_time_data_output_av, skip_time_data_output_av +            &
                            ...
                            simulated_time_at_begin )
       IF ( time_interval < 0.0_wp )  time_interval = 0.0_wp
       ntdim_2d_yz(1) = CEILING( time_interval / dt_data_output_av )
