...
!--    which are needed for the following read instructions.
       ...
       READ ( 13 )  max_pr_user    ! This value is checked against the number of
                                   ...
                                   ! in routine parin (it has to match)
...
!--    Now read all control parameters:
       ...
       DO WHILE ( restart_string(1:length) /= 'binary_version_local' )
          ...
          SELECT CASE ( restart_string(1:length) )
             ...
             CASE ( 'cut_cell_topography' )
                READ ( 13 )  cut_cell_topography
             CASE ( 'cycle_mg' )
                READ ( 13 )  cycle_mg
             CASE ( 'damp_level_1d' )
             ...
             CASE ( 'nesting_coupling_time' )
                READ ( 13 )  nesting_coupling_time
             CASE ( 'neutral' )
                READ ( 13 )  neutral
             CASE ( 'ngsrb' )
                READ ( 13 )  ngsrb
...
!--    There is no module procedure for CHARACTER arrays
       ...
       CALL rrd_mpi_io( 'nesting_coupling_time', nesting_coupling_time )
...
!
!-- Finally check, that a mainrun domain is not smaller than the prerun domain. The cyclic-fill
!-- algorithm can not handle such a case. The problem is related to the surface data.
!-- Anyhow, using a mainrun domain smaller than the prerun domain does not really makes sense.
    IF ( nx < nx_on_file )  THEN
       WRITE( message_string, * ) 'number of grid points in main run along x is smaller &than ',   &
                                  'in the prerun:',                                                &
                                  '&nx on file    = "', nx_on_file, '"',                           &
                                  '&nx from run   = "', nx, '"'
       CALL message( 'rrd_read_parts_of_global', 'PAC0354', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( ny < ny_on_file )  THEN
       WRITE( message_string, * ) 'number of grid points in main run along y is smaller &than ',   &
                                  'in the prerun:',                                                &
                                  '&ny on file    = "', ny_on_file, '"',                           &
                                  '&ny from run   = "', ny, '"'
       CALL message( 'rrd_read_parts_of_global', 'PAC0355', 1, 2, 0, 6, 0 )
    ENDIF
