...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE diffusion_w
       ...
       !$ACC PRESENT(tend) &
       !$ACC PRESENT(surf_w, surf_w%westward, surf_w%eastward, surf_w%northward, surf_w%southward) &
       !$ACC PRESENT(surf_w%start_index, surf_w%end_index, surf_w%k, surf_w%wsus_wsvs) &
...
!--             Cut-cell branch.
                ELSE
                   surf_s = surf_w%start_index(j,i)
                   surf_e = surf_w%end_index(j,i)
                   !$ACC LOOP PRIVATE(k, m)
                   DO  m = surf_s, surf_e
                      k         = surf_w%k(m)

                      flux_r(k) = MERGE( surf_w%wsus_wsvs(m), flux_r(k), surf_w%westward(m)  )
                      flux_l(k) = MERGE( surf_w%wsus_wsvs(m), flux_l(k), surf_w%eastward(m)  )
                      flux_n(k) = MERGE( surf_w%wsus_wsvs(m), flux_n(k), surf_w%southward(m) )
                      flux_s(k) = MERGE( surf_w%wsus_wsvs(m), flux_s(k), surf_w%northward(m) )
                   ENDDO
                ENDIF
             ENDIF
!
