...
!$  INTEGER(iwp) ::  omp_get_thread_num  !<
    ...
    REAL(wp) ::  rh_tmp         !< local value of relative humidity
                ...
                DO  m = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                   IF ( surf_lsm%upward(m)  .OR.  surf_lsm%downward(m) )  THEN
                      ...
                      IF ( humidity )  THEN
                         sums_l(k,48,tn) = sums_l(k,48,tn) +                                       &
                                           waterflux_output_conversion(k) *                        &
                                           surf_lsm%qsws_agg(m) * rmask(j,i,sr)  ! w"q" (w"qv")
                         sums_l(k,45,tn) = sums_l(k,45,tn) +  (                                    &
                                           ( 1.0_wp + 0.61_wp * q(k,j,i) ) *                       &
...
!
!--                Also consider default surfaces for RRTMG output (radiation-only option).
                   DO  m = surf_def%start_index(j,i), surf_def%end_index(j,i)
                      IF ( surf_def%upward(m) )  THEN
                   ...
                   DO  m = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                      IF ( surf_lsm%upward(m) )  THEN
                         sums_l(nzb,108,tn) = sums_l(nzb,108,tn)  +                                &
                                              surf_lsm%rrtm_aldif(m,0) * rmask(j,i,sr)
                         sums_l(nzb,109,tn) = sums_l(nzb,109,tn)  +                                &
                                              surf_lsm%rrtm_aldir(m,0) * rmask(j,i,sr)
                         sums_l(nzb,110,tn) = sums_l(nzb,110,tn)  +                                &
                                              surf_lsm%rrtm_asdif(m,0) * rmask(j,i,sr)
                         sums_l(nzb,111,tn) = sums_l(nzb,111,tn)  +                                &
                                              surf_lsm%rrtm_asdir(m,0) * rmask(j,i,sr)
                      ENDIF
                   ENDDO
                   DO  m = surf_usm%start_index(j,i), surf_usm%end_index(j,i)
                      IF ( surf_usm%upward(m) )  THEN
                         sums_l(nzb,108,tn) = sums_l(nzb,108,tn)  +                                &
                                              surf_usm%rrtm_aldif(m,0) * rmask(j,i,sr)
                         sums_l(nzb,109,tn) = sums_l(nzb,109,tn)  +                                &
                                              surf_usm%rrtm_aldir(m,0) * rmask(j,i,sr)
                         sums_l(nzb,110,tn) = sums_l(nzb,110,tn)  +                                &
                                              surf_usm%rrtm_asdif(m,0) * rmask(j,i,sr)
                         sums_l(nzb,111,tn) = sums_l(nzb,111,tn)  +                                &
                                              surf_usm%rrtm_asdir(m,0) * rmask(j,i,sr)
                      ENDIF
                   ENDDO
                ...
                IF ( ocean_mode )  THEN
                   sums_l(nzt,65,tn)   = sums_l(nzt,65,tn) +                                       &
                                         surf_top%sasws(m) * scalarflux_output_conversion(nzt)     &
                                                           * rmask(j,i,sr)  ! w"sa"
                   sums_l(nzt+1,65,tn) = sums_l(nzt,65,tn)  ! to avoid kinks in plots
                ENDIF
                IF ( humidity )  THEN
...
!--                      fluxes must be in kinematic units here.
                   ...
                   rh_tmp = q(k,j,i) / q_s * 100.0_wp
                   sums_l(k,130,tn) = sums_l(k,130,tn) + rh_tmp * rmask(j,i,sr) * flag
                ENDIF
!
!--             Passive scalar flux
          ...
          IF ( radiation_scheme == 'rrtmg' )  THEN
             DO  i = nxl, nxr
                DO  j =  nys, nyn
                   DO  k = nzb+1, nzt+1
                      flag = MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(k,j,i), 0 ) )
                      sums_l(k,104,tn) = sums_l(k,104,tn) + rad_lw_cs_hr(k,j,i) * rmask(j,i,sr) *  &
                                                            flag
                      sums_l(k,106,tn) = sums_l(k,106,tn) + rad_sw_cs_hr(k,j,i) * rmask(j,i,sr) *  &
                                                            flag
                   ENDDO
                ENDDO
             ENDDO
...
!--    Summation of thread sums
       IF ( threads_per_task > 1 )  THEN
          DO  i = 1, threads_per_task-1
             ...
             sums_l(:,45:pr_total,0) = sums_l(:,45:pr_total,0) + sums_l(:,45:pr_total,i)
          ENDDO
       ENDIF
...
!
!--    Average profiles of all other modules.
       IF ( pr_total_modules > 0 )  THEN
          DO  k = nzb, nzt+1
             sums(k,pr_palm+1:pr_total) = sums(k,pr_palm+1:pr_total) / ngp_2dh_s_inner(k,sr)
          ENDDO
       ENDIF
...
!--    Compute deduced averages (e.g. total heat flux)
       ...
       hom(:,1,126,sr) = sums(:,126)   ! ng
       hom(:,1,127,sr) = sums(:,127)   ! qg
       ...
       hom(:,1,pr_palm,sr) = sums(:,pr_palm)  ! u*, w'u', w'v', t* (in last profile)

       IF ( pr_total_modules > 0 )  THEN  ! quantities from all other modules
          hom(:,1,pr_palm+1:pr_total, sr) = sums(:,pr_palm+1:pr_total)
       ENDIF
...
!--    The horizontal average at surface level index + 1 is input for the average temperature.
       IF ( hom(k_surface_level,1,18,sr) > 1.0E-8_wp  .AND.  z_i(1) /= 0.0_wp )  THEN
          hom(nzb+8,1,pr_palm,sr) = ( g / hom(k_surface_level+1,1,4,sr) *                          &
                                      ( hom(k_surface_level,1,18,sr) /                             &
                                      ( heatflux_output_conversion(k_surface_level) *              &
                                      rho_air(k_surface_level) )                                   &
                                      ) * ABS( z_i(1) - mean_surface_level_height(sr) )            &
                                    )**0.333333333_wp
       ELSE
