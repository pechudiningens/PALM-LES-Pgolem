...
!>  TODO: It should be enough to determine size and rank from comm_palm. comm_palm has already
!>        been set to MPI_COMM_WORLD in pmci_init in case of no nesting/coupling. Also, it should
!>        not be required to set comm2d here. It is only used once in parin, unless it is finally
!>        determined in init_pegrid,a nd could be replaced by comm_palm there.
...
!-- preliminary communicator. myid_char will be set again in routine init_pegrid, based on the
...
!-- preliminary communicator. If the order is the same (and it should be, otherwise restart files
...
!-- errors.
    DO i = 0, local_num_procs-1
       ...
       IF ( i == local_id )  THEN
          ...
          num_acc_devices = acc_get_num_devices( device_type )
          my_device = MOD( local_id, num_acc_devices )
...
!--    Child initialization must be done first, if the model is both child and parent.
       IF ( TRIM( initializing_actions )  /= 'read_restart_data' )  CALL pmci_child_initialize
!
!--    Send initial condition data from parent to children.
!--    Parents immediately return if initializing_actions = 'read_restart_data', but with the
!--    important exception in case that the corresponding child has to be activated/initialized
!--    in this run!
...
!-- Integration of the non-atmospheric equations (land surface model, urban surface model)
    IF ( spinup )  THEN
       CALL cpu_log( log_point(41), 'surface model spinup', 'start' )
       CALL time_integration_spinup
       CALL cpu_log( log_point(41), 'surface model spinup', 'stop' )
!
