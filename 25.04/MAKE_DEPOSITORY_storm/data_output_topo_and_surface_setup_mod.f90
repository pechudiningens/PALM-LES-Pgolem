...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE do_topo_and_surface_actions( location )
       ...
       REAL(sp), DIMENSION(:,:,:), POINTER             ::  output_values_3d_real32_pointer  !< pointer to output array
       REAL(sp), DIMENSION(:,:,:), ALLOCATABLE, TARGET ::  output_values_3d_real32_target   !< output array
...
!--                top index of uppermost terrain grid point.
                   DO  i = nxl, nxr
                      DO  j = nys, nyn
                         ...
                         ENDDO
!
!--                      Set lowest grid point in output. Do this only if building top
!--                      index is greater zero. Zero values can appear if buildings are defined
!--                      but are not resolved by the numeric grid.
                         IF ( k_rel_max_buildings > 0 )  THEN
...
!--          Output of water types.
             ...
             ENDIF
!
!--          Output of leaf area density and basal area density.
             IF ( plant_canopy )  THEN
                variable_name = 'lad'
                ALLOCATE( output_values_3d_real32_target(nzb:nzb+pch_index,nys:nyn,nxl:nxr) )
                output_values_3d_real32_target = lad_s(nzb:nzb+pch_index,nys:nyn,nxl:nxr)
                output_values_3d_real32_pointer => output_values_3d_real32_target
                return_value = dom_write_var( nc_filename, variable_name,                          &
                                              values_real32_3d = output_values_3d_real32_pointer,  &
                                              bounds_start = (/nxl, nys, nzb/),                    &
                                              bounds_end   = (/nxr, nyn, nzb+pch_index/) )
 
                variable_name = 'bad'
                output_values_3d_real32_target = bad_s(nzb:nzb+pch_index,nys:nyn,nxl:nxr)
                output_values_3d_real32_pointer => output_values_3d_real32_target
                return_value = dom_write_var( nc_filename, variable_name,                          &
                                              values_real32_3d = output_values_3d_real32_pointer,  &
                                              bounds_start = (/nxl, nys, nzb/),                    &
                                              bounds_end   = (/nxr, nyn, nzb+pch_index/) )
                DEALLOCATE( output_values_3d_real32_target )
             ENDIF
...
!--    z-coordinate for buildings only
       ...
       return_value = dom_def_att( nc_filename, variable_name = 'zb', attribute_name = 'units',    &
                                   value = 'm' )
!
!--    z-coordinate for resolved vegetation only.
       return_value = dom_def_dim( nc_filename, dimension_name = 'zpc', output_type = 'real32',    &
                                   bounds = (/0, pch_index/),                                      &
                                   values_realwp = zu(nzb:nzb+pch_index) )
       return_value = dom_def_att( nc_filename, variable_name = 'zpc',                             &
                                   attribute_name = 'long_name',                                   &
                                   value = 'z-coordinate for plant canopy' )
       return_value = dom_def_att( nc_filename, variable_name = 'zpc', attribute_name = 'units',   &
                                   value = 'm' )   
!
...
!--    Define output for water type.
       ...
       ENDIF
!
!--    Define output for leaf area density and basal area density.
       IF ( plant_canopy )  THEN
         variable_name = 'lad'
         return_value = dom_def_var( nc_filename, variable_name = variable_name,                   &
                                     dimension_names = (/ 'x  ', 'y  ', 'zpc' /),                  &
                                     output_type = 'real32' )
         return_value = dom_def_att( nc_filename, variable_name = variable_name,                   &
                                     attribute_name = char_fill,                                   &
                                     value = REAL( -9999.0, KIND = sp ) )
         return_value = dom_def_att( nc_filename, variable_name = variable_name,                   &
                                     attribute_name = 'long_name',                                 &
                                     value = 'leaf area density' )
         return_value = dom_def_att( nc_filename, variable_name = variable_name,                   &
                                     attribute_name = 'units',                                     &
                                     value = 'm2/m3' )
         variable_name = 'bad'
