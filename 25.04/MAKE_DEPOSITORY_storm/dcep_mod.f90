...
!-- Calculate total emitted radiation.
    ...
    DO  j = nys, nyn
       DO  i = nxl, nxr
          ...
          IF ( fr_urb(j,i) > eps_urb )  THEN
             t_grad_urb(j,i) = SQRT( SQRT( ( rlup(j,i) - ( 1.0_wp - emiss_urb(j,i) ) * rl(j,i) ) / &
                                           sigma_sb/emiss_urb(j,i) )                               &
                                   )
          ELSE
...
!--    Read the fraction of urban parts in a grid element (fr_urb).
       IF ( check_existence( var_names, 'FR_URBAN' ) )  THEN
          CALL get_variable( id_dcep, 'FR_URBAN', fr_urb, nxl, nxr, nys, nyn, nbgp=0 )
       ELSE
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE dcep_data_output_3d( av, variable, found, local_pf, resorted, mask_topography, nzb_do, &
                                 nzt_do )
