...
!--------------------------------------------------------------------------------------------------!
 MODULE netcdf_interface
    ...
    INTEGER ::  netcdf_compression_level = HUGE( 1 )  !< netCDF compression, default: no compression
    INTEGER ::  netcdf_compression_nsd = HUGE( 1 )    !< netCDF number of significant digits in case of compression
...
!
!-- SLUrb
    INTEGER(iwp), DIMENSION(0:1) ::  id_dim_nroad_3d  !<
    INTEGER(iwp), DIMENSION(0:1) ::  id_dim_nroof_3d  !<
    INTEGER(iwp), DIMENSION(0:1) ::  id_dim_nwall_3d  !<
    INTEGER(iwp), DIMENSION(0:1) ::  id_dim_nwin_3d   !<
    INTEGER(iwp), DIMENSION(0:1) ::  id_var_nroad_3d  !<
    INTEGER(iwp), DIMENSION(0:1) ::  id_var_nroof_3d  !<
    INTEGER(iwp), DIMENSION(0:1) ::  id_var_nwall_3d  !<
    INTEGER(iwp), DIMENSION(0:1) ::  id_var_nwin_3d   !<
...
!-- Masked output
    ...
    LOGICAL ::  netcdf_compression = .FALSE.        !< indicates if compression is switched on
    LOGICAL ::  netcdf_compression_lossy = .FALSE.  !< indicates if lossy compression is switched on
    ...
    PUBLIC  dofl_label_x, dofl_label_y, dofl_label_z, dofl_label, dofl_time_count, dofl_unit,      &
            ...
            id_var_time_agt, id_var_time_fl, id_var_time_mask, id_var_time_pr,                     &
            ...
            nc_stat, netcdf_compression_level, netcdf_compression_nsd,                             &
            netcdf_data_format, netcdf_data_format_string,                                         &
            netcdf_precision, output_for_t0, heatflux_output_unit, waterflux_output_unit,          &
            momentumflux_output_unit

    SAVE

    INTERFACE netcdf_check_parameters
       MODULE PROCEDURE netcdf_check_parameters
    END INTERFACE netcdf_check_parameters
    ...
    PUBLIC netcdf_check_parameters,                                                                &
           netcdf_create_att,                                                                      &
           netcdf_create_dim,                                                                      &
           netcdf_create_file,                                                                     &
...
!
!--       Define time coordinate for masked data.
...
!--       significantly.
          ...
          ELSE
             CALL netcdf_create_dim( id_set_mask(mid,av), 'time', ntdim_masks(mid,av),             &
                                     id_dim_time_mask(mid,av), 467 )
          ENDIF
...
!--                Check for DCEP quantities
                   ...
                   ENDIF
!
!--                Check for DET quantities
                   IF ( .NOT. found  .AND.  det_enabled )  THEN
                      CALL det_define_netcdf_grid( trimvar, found, grid_x, grid_y, grid_z )
                   ENDIF
!
!--                Check for diagnostic output quantities
                   IF ( .NOT. found )                                                              &
                   ...
                   IF ( .NOT. found  .AND.  slurb )  THEN
                      CALL slurb_define_netcdf_grid( trimvar, found, grid_x, grid_y, grid_z )
                   ENDIF
...
!--          Define the grid
             CALL netcdf_create_var( id_set_mask(mid,av),                                          &
                                     ...
                                     domask(mid,av,i), 494, 495, 496,                              &
                                     set_fillvalue = .TRUE., no_prefill = .TRUE.,                  &
                                     collective = .TRUE., compression = netcdf_compression,        &
                                     lossy_compression = netcdf_compression_lossy )
...
!--       Set general no fill, otherwise the performance drops significantly for parallel output.
          nc_stat = NF90_SET_FILL( id_set_mask(mid,av), NF90_NOFILL, oldmode )
          CALL netcdf_handle_error( 'netcdf_define_header', 497 )
...
!--          Define vertical coordinate grid zwall_3d
          ...
          IF ( slurb )  THEN
!
!--          Define vertical coordinate grid nroof_3d
             CALL netcdf_create_dim( id_set_3d(av), 'nroof_3d', n_layers_roofs,                    &
                                     id_dim_nroof_3d(av), 700 )
             CALL netcdf_create_var( id_set_3d(av), (/ id_dim_nroof_3d(av) /), 'nroof_3d',         &
                                     NF90_INT, id_var_nroof_3d(av), '-', '', 711, 701, 00)
             CALL netcdf_create_att( id_set_3d(av), id_var_nroof_3d(av), 'axis', 'Z', 000 )
!
!--          Define vertical coordinate grid nwall_3d
             CALL netcdf_create_dim( id_set_3d(av), 'nwall_3d', n_layers_walls,                    &
                                     id_dim_nwall_3d(av), 700 )
             CALL netcdf_create_var( id_set_3d(av), (/ id_dim_nwall_3d(av) /), 'nwall_3d',         &
                                     NF90_INT, id_var_nwall_3d(av), '-', '', 712, 712, 00 )
             CALL netcdf_create_att( id_set_3d(av), id_var_nwall_3d(av), 'axis', 'Z', 000 )
!
!--          Define vertical coordinate grid nwall_3d
             CALL netcdf_create_dim( id_set_3d(av), 'nwin_3d', n_layers_windows,                   &
                                     id_dim_nwin_3d(av), 700 )
             CALL netcdf_create_var( id_set_3d(av), (/ id_dim_nwin_3d(av) /), 'nwin_3d',           &
                                     NF90_INT, id_var_nwin_3d(av), '-', '', 713, 712, 00 )
             CALL netcdf_create_att( id_set_3d(av), id_var_nwin_3d(av), 'axis', 'Z', 000 )
!
!--          Define vertical coordinate grid nroad_3d
             CALL netcdf_create_dim( id_set_3d(av), 'nroad_3d', n_layers_roads,                    &
                                     id_dim_nroad_3d(av), 700 )
             CALL netcdf_create_var( id_set_3d(av), (/ id_dim_nroad_3d(av) /), 'nroad_3d',         &
                                     NF90_INT, id_var_nroad_3d(av), '-', '', 714, 712, 00 )
             CALL netcdf_create_att( id_set_3d(av), id_var_nwall_3d(av), 'axis', 'Z', 000 )
...
!--                Check for DET quantities
                   IF ( .NOT. found  .AND.  det_enabled )  THEN
                      CALL det_define_netcdf_grid( do3d(av,i), found, grid_x, grid_y, grid_z )
                   ENDIF
!
!--                Check for diagnostic output quantities
                   IF ( .NOT. found )  THEN
...
!--                Check for SALSA quantities
                   ...
                   ENDIF
!
!--                Check for SLUrb quantities
                   IF ( .NOT. found  .AND.  slurb )  THEN
                      CALL slurb_define_netcdf_grid( do3d(av,i), found, grid_x, grid_y, grid_z )
                   ENDIF
...
!--                Check for user-defined quantities
                   ...
                   ENDIF
!
!--                Check for UV-radiation quantities.
                   IF ( .NOT. found  .AND.  uv_radiation )  THEN
...
!--          Select the respective dimension ids
             ...
             ELSEIF ( grid_z == 'nroad_3d' ) THEN
                id_z = id_dim_nroad_3d(av)
             ELSEIF ( grid_z == 'nroof_3d' ) THEN
                id_z = id_dim_nroof_3d(av)
             ELSEIF ( grid_z == 'nwall_3d' ) THEN
                id_z = id_dim_nwall_3d(av)
             ELSEIF ( grid_z == 'nwin_3d' ) THEN
                id_z = id_dim_nwin_3d(av)
...
!--          Define the grid
             CALL netcdf_create_var( id_set_3d(av),(/ id_x, id_y, id_z, id_dim_time_3d(av) /),     &
                                     ...
                                     TRIM( do3d_unit(av,i) ), do3d(av,i), 79, 80, 357,             &
                                     set_fillvalue = .TRUE., no_prefill = .TRUE.,                  &
                                     collective = .TRUE., compression = netcdf_compression,        &
                                     lossy_compression = netcdf_compression_lossy )
...
!
!--             Write nroof grid
                nc_stat = NF90_PUT_VAR( id_set_3d(av), id_var_nroof_3d(av),                        &
                                        (/ (i-1, i=1,n_layers_roofs) /), start = (/ 1 /),          &
                                        count = (/ n_layers_roofs /) )
                CALL netcdf_handle_error( 'netcdf_define_header', 536 )
!
!--             Write nwall grid
                nc_stat = NF90_PUT_VAR( id_set_3d(av), id_var_nwall_3d(av),                        &
                                        (/ (i-1, i=1,n_layers_walls) /), start = (/ 1 /),          &
                                        count = (/ n_layers_walls /) )
                CALL netcdf_handle_error( 'netcdf_define_header', 537 )
!
!--             Write nroad grid
                nc_stat = NF90_PUT_VAR( id_set_3d(av), id_var_nwin_3d(av),                         &
                                        (/ (i-1, i=1,n_layers_windows) /), start = (/ 1 /),        &
                                        count = (/ n_layers_windows /) )
                CALL netcdf_handle_error( 'netcdf_define_header', 539 )
!
!--             Write nroad grid
                nc_stat = NF90_PUT_VAR( id_set_3d(av), id_var_nroad_3d(av),                        &
                                        (/ (i-1, i=1,n_layers_roads) /), start = (/ 1 /),          &
                                        count = (/ n_layers_roads /) )
                CALL netcdf_handle_error( 'netcdf_define_header', 540 )
...
!--                      Check for DET quantities
                         IF ( .NOT. found  .AND.  det_enabled )  THEN
                            CALL det_define_netcdf_grid( do2d(av,i), found, grid_x, grid_y, grid_z )
                         ENDIF
!
!--                      Check for diagnostic output quantities
                         IF ( .NOT. found )  THEN
...
!--                      Check for SALSA quantities
                         ...
                         ENDIF
!
!--                      Check for SLUrb surface quantities
                         IF ( .NOT. found  .AND.  slurb )  THEN
                            CALL slurb_define_netcdf_grid( do2d(av,i), found, grid_x, grid_y,      &
                                                           grid_z )
                         ENDIF
...
!--                   Check for SLUrb surface quantities
                      IF ( .NOT. found  .AND. slurb )  THEN
                         CALL slurb_define_netcdf_grid( do2d(av,i), found, grid_x, grid_y, grid_z )
                      ENDIF
...
!--             Define the grid
                CALL netcdf_create_var( id_set_xz(av), (/ id_x, id_y, id_z, id_dim_time_xz(av) /), &
                                        ...
                                        TRIM( do2d_unit(av,i) ), do2d(av,i), 159, 160, 355,        &
                                        set_fillvalue = .TRUE., no_prefill = .TRUE.,               &
                                        collective = .TRUE., compression = netcdf_compression,     &
                                        lossy_compression = netcdf_compression_lossy )
...
!--             Define the grid.
                CALL netcdf_create_var( id_set_yz(av),  (/ id_x, id_y, id_z, id_dim_time_yz(av) /),&
                                        ...
                                        TRIM( do2d_unit(av,i) ), do2d(av,i), 198, 199, 356,        &
                                        set_fillvalue = .TRUE., no_prefill = .TRUE.,               &
                                        collective = .TRUE., compression = netcdf_compression,     &
                                        lossy_compression = netcdf_compression_lossy )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE netcdf_create_var( ncid, dim_id, var_name, var_type, var_id, unit_name, long_name,     &
                               error_no1, error_no2, error_no3, set_fillvalue, no_prefill,         &
                               collective, compression, lossy_compression )
    ...
    LOGICAL, OPTIONAL ::  collective         !< enables setting of collective attribute
    LOGICAL, OPTIONAL ::  compression        !< enables setting of compression rate
    LOGICAL, OPTIONAL ::  lossy_compression  !< enables setting of compression rate
    LOGICAL, OPTIONAL ::  no_prefill         !< disables pre-filling with fill values (for performance reasons)
    LOGICAL, OPTIONAL ::  set_fillvalue      !< indicates setting of _FillValue attribute
...
!-- Set no fill for every variable to increase performance.
    IF ( PRESENT( no_prefill ) )  THEN
       IF ( no_prefill  .AND.  netcdf_data_format > 4 )  THEN
          nc_stat = NF90_DEF_VAR_FILL( ncid, var_id, NF90_NOFILL, 0 )
          CALL netcdf_handle_error( 'NF90_DEF_VAR_FILL', 0 )
       ENDIF
    ENDIF

    IF ( PRESENT( collective ) )  THEN
       IF ( collective  .AND.  netcdf_data_format > 4 )  THEN
!
!--       Set collective io operations for parallel io
          nc_stat = NF90_VAR_PAR_ACCESS( ncid, var_id, NF90_COLLECTIVE )
          CALL netcdf_handle_error( 'NF90_VAR_PAR_ACCESS', 0 )
       ENDIF
    ENDIF
#else
!
!-- To avoid compiler warning.
    IF ( PRESENT( no_prefill )  .OR.  PRESENT( collective ) )  CONTINUE
#endif
...
!-- with HDF5 supports compression).
    IF ( PRESENT( compression ) )  THEN
       IF ( compression  .AND.  netcdf_data_format > 2 )  THEN
          IF ( netcdf_compression_method == 'deflate' )  THEN
             nc_stat = NF90_DEF_VAR_DEFLATE( ncid, var_id, 1, 1, netcdf_compression_level )
             CALL netcdf_handle_error( 'NF90_DEF_VAR_DEFLATE', 0 )
          ELSEIF ( netcdf_compression_method == 'zstd' )  THEN
#if defined( __netcdf4_compression )
             nc_stat = NF90_DEF_VAR_ZSTANDARD( ncid, var_id, netcdf_compression_level )
             CALL netcdf_handle_error( 'NF90_DEF_VAR_ZSTANDARD', 0 )
#endif
          ENDIF
       ENDIF
    ENDIF
...
!-- Set lossy compression.
    IF ( PRESENT( lossy_compression ) )  THEN
       IF ( lossy_compression )  THEN
#if defined( __netcdf4_compression )
          nc_stat = NF90_DEF_VAR_QUANTIZE( ncid, var_id, quantize_mode=NF90_QUANTIZE_BITGROOM,     &
                                           nsd=netcdf_compression_nsd )
          CALL netcdf_handle_error( 'NF90_DEF_VAR_QUANTIZE', 0 )
#endif
       ENDIF
    ENDIF
#else
!
!-- To avoid compiler warning.
    IF ( PRESENT( compression )  .OR.  PRESENT( lossy_compression ) )  CONTINUE
#endif

!
!-- Set _FillValue attribute and fill value with respect to the corresponding output precision.
!-- For an unknown reason, from netCDF version 4.9.0 on, the call of NF90_DEF_VAR_FILL above removes
!-- an existing fill value attribute, so it needs to be set here.
    IF ( PRESENT( set_fillvalue ) )  THEN
...
! Description:
! ------------
!> Routine for the checking the netcdf namelist parameters.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE netcdf_check_parameters

    CHARACTER(LEN=10) ::  precision  !< temporary string to hold the netcdf precision
    CHARACTER(LEN=10) ::  var        !< temporary string to hold the output variable

    INTEGER(iwp) ::  i  !< temporary index
    INTEGER(iwp) ::  j  !< temporary index
...
!
!--    Check for allowed compression method and compression range.
       SELECT CASE ( netcdf_compression_method )
          ...
          CASE ( 'deflate' )
             ...
             ELSEIF ( netcdf_compression_level < 0  .OR.  netcdf_compression_level > 9 )  THEN
                WRITE ( message_string, '(A,A,I7,A)' ) 'netcdf_compression_level = ',              &
                                                       netcdf_compression_level, ' out of range'
          ...
          CASE ( 'zstd' )
             ...
             ELSEIF ( netcdf_compression_level < -131072  .OR.  netcdf_compression_level > 22 )    &
             THEN
                WRITE ( message_string, '(A,A,I7,A)' ) 'netcdf_compression_level = ',              &
                                                       netcdf_compression_level, ' out of range'
                CALL message( 'netcdf_define_header', 'NCI0004', 2, 2, 0, 6, 0 )
...
!
!--    Check for allowed range of number of significant digits.
       IF ( netcdf_compression )  THEN
          IF ( netcdf_compression_nsd /= HUGE( 1 ) )  THEN
             ...
             IF ( netcdf_compression_nsd < 1  .OR.  netcdf_compression_nsd > 15 )  THEN
                WRITE ( message_string, '(A,A,I7,A)' ) 'netcdf_compression_nsd = ',                &
                                                       netcdf_compression_nsd, ' out of range'
                CALL message( 'netcdf_define_header', 'NCI0005', 2, 2, 0, 6, 0 )
             ENDIF
             netcdf_compression_lossy = .TRUE.
          ENDIF
       ENDIF

 END SUBROUTINE netcdf_check_parameters
