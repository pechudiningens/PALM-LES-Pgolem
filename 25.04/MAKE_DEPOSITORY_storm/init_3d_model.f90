...
!
!-- Try to read surface pressure from dynamic file.
...
!
!--    If variable is present in the driver, netcdf_data_input_1d has allocated tmp_1d%var1d.
       IF ( ALLOCATED( tmp_1d%var1d ) )  THEN
!
!--       Convert pressure unit to hPa, because in the driver it is given in Pa.
...
!
!--       In case of LOD1, homogeneously initialize 3D arrays with vertical profiles.
          DO  i = nxlg, nxrg
             DO  j = nysg, nyng
                IF( init_3d%lod_u == 1 )  u(:,j,i) = u_init(:)
                IF( init_3d%lod_v == 1 )  v(:,j,i) = v_init(:)
                IF( .NOT. neutral  .AND.  init_3d%lod_pt == 1 )  pt(:,j,i) = pt_init(:)
...
!
!--       Also initialize prognostic levels as these also enter the solution at the second
!--       RK timestep.
          u_p = u; v_p = v; w_p = w
...
!--       Run the 1d-model first.
          CALL location_message( 'running 1d-model', 'start' )
          CALL cpu_log ( log_point_s(104), '1d-model', 'start')
          CALL init_1d_model
          CALL cpu_log ( log_point_s(104), '1d-model', 'stop')
          CALL location_message( 'running 1d-model', 'finished' )
!
...
!-- file. Right now this only affects default surfaces, where roughness length, surface sensible
!-- or latent heat fluxes, or surface scalar fluxes can be initialized. Therefore, a generic
!-- routine from netcdf_data_input_mod is called to read a 2D array. Further on, the input data
!-- is used to initialize surface values at default surfaces.
    IF ( input_pids_static )  THEN
!
!--    Roughness length.
       CALL netcdf_data_input_2d_xy_real( TRIM( input_file_static ) // TRIM( coupling_char ),      &
                                          'z0', tmp_2d, var_available,                             &
                                          includes_ghost_layers = .TRUE. )

       IF ( var_available )  THEN
          CALL init_single_surface_properties( surf_def%z0, tmp_2d%var, surf_def%ns, tmp_2d%fill,  &
                                               surf_def%i, surf_def%ioff, surf_def%j,              &
                                               surf_def%joff, surf_def%k, surf_def%koff)
       ENDIF
!
!--    Surface sensible heat flux.
       CALL netcdf_data_input_2d_xy_real( TRIM( input_file_static ) // TRIM( coupling_char ),      &
                                          'shf', tmp_2d, var_available,                            &
                                          includes_ghost_layers = .TRUE. )
...
!
!--       Check consistency of boundary conditions again (like done in check_parameters before).
!--       A given surface temperature implies Dirichlet boundary condition for temperature.
!--       In this case specification of a constant heat flux is forbidden.
!--       ATTENTION: the additional condition ibc_pt_b == 3 (nested at bottom boundary) may be
!--                  removed lateron, if it turns out that surface layer fluxes are not calculated
!--                  for bottom boundary grid points in such a case (because they are actually
!--                  located in the atmosphere).
          ...
          THEN
             ...
             CALL message( 'init_3d_model', 'PAC0087', 1, 2, 0, 6, 0 )
          ENDIF
       ...
       IF ( passive_scalar )  THEN
          ...
          IF ( var_available )  THEN
             CALL init_single_surface_properties( surf_def%ssws, tmp_2d%var, surf_def%ns,          &
                                                  tmp_2d%fill, surf_def%i, surf_def%ioff,          &
                                                  surf_def%j, surf_def%joff, surf_def%k,           &
                                                  surf_def%koff, scalarflux_input_conversion )
             constant_scalarflux = .TRUE.
!
!--          Check consistency of boundary conditions again (like done in check_parameters before).
             CALL check_bc_scalars( 's', bc_s_b, ibc_s_b, 'PAC0094', 'PAC0095',                    &
...
!-- simulations, where childs do not cover the complete root domain.
    IF ( ibc_p_b == 1  .AND.  ibc_p_t == 1  .AND.  .NOT. nesting_offline                           &
         .AND. .NOT. ( child_domain  .AND.  .NOT. atmosphere_ocean_coupled_run  .AND.              &
                       ( nesting_bounds /= 'vertical_only'  .OR.                                   &
                         ( nesting_bounds == 'vertical_only' .AND.                                 &
                           .NOT. ( bc_lr_cyc .AND. bc_ns_cyc ) )                                   &
                       )                                                                           &
                     ) )                                                                           &
    THEN
...
!
!-- Initialize arrays for surface data output.
    IF ( surface_output )  THEN
       CALL surface_data_output_init
    ENDIF
!
