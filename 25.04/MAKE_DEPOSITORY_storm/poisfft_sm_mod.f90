...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE poisfft_sm_init
    ...
    INTEGER(iwp) ::  array_of_starts(4)    !<
    INTEGER(iwp) ::  array_of_subsizes(4)  !<
    INTEGER(iwp) ::  shp(4)                !<
...
!--  fftx_tri_fftx.
     ...
     shp = SHAPE( xz_buf )
     shp(4) = 1
     array_of_subsizes = (/ nnx_max, 1, nny_x_max, 1 /)
     array_of_starts   = (/ 0, 0, 0, 0 /)
     CALL MPI_TYPE_CREATE_SUBARRAY( 4, shp, array_of_subsizes, array_of_starts, MPI_ORDER_FORTRAN, &
                                    MPI_REAL, xzbuftype, ierr )
