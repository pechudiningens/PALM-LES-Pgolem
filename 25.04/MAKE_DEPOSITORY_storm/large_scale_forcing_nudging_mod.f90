...
!--------------------------------------------------------------------------------------------------!
 MODULE lsf_nudging_mod
    ...
    CHARACTER(LEN=20) ::  nudged_quantities = ''  !< string to store the quantities to which nudging is applied
    ...
    LOGICAL ::  nudge_u  = .TRUE.  !< internal switch to apply nudging for this specific quantity
    LOGICAL ::  nudge_v  = .TRUE.  !< internal switch to apply nudging for this specific quantity
    LOGICAL ::  nudge_w  = .TRUE.  !< internal switch to apply nudging for this specific quantity
    LOGICAL ::  nudge_pt = .TRUE.  !< internal switch to apply nudging for this specific quantity
    LOGICAL ::  nudge_q  = .TRUE.  !< internal switch to apply nudging for this specific quantity
...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE lsf_nudging_header ( io )
       ...
       IF ( large_scale_forcing )  THEN
          ...
          WRITE ( io, 7 )
          IF ( nudging )  THEN
             WRITE ( io, 10 )  TRIM( nudged_quantities )
          ENDIF
       ELSE
          WRITE ( io, 2 )
          WRITE ( io, 11 )
...
10 FORMAT (' --> Nudging is applied to quantities ',A)
...
!--       Save surface sensible heat flux on upward-facing default, natural and urban surfaces.
          ...
          DO  m = 1, surf_lsm%ns
             ...
             surf_lsm%shf_agg(m) = MERGE( dum_surf_flux, surf_lsm%shf_agg(m), surf_lsm%upward(m) )
          ENDDO
          DO  m = 1, surf_usm%ns
             surf_usm%shf(m) = MERGE( dum_surf_flux, surf_usm%shf(m), surf_usm%upward(m) )
          ENDDO
!
!--       In case of SLUrb, save also for the SLUrb surface (which are all upward at this point).
          IF ( slurb )  THEN
             DO  m = 1, surf_slurb%ns
                surf_slurb%shf_urb(m) = dum_surf_flux
             ENDDO
          ENDIF
...
!--       Save surface latent heat flux on upward-facing default, natural and urban surfaces.
          ...
          DO  m = 1, surf_lsm%ns
             ...
             surf_lsm%qsws_agg(m) = MERGE( dum_surf_flux, surf_lsm%qsws_agg(m), surf_lsm%upward(m) )
          ENDDO
          DO  m = 1, surf_usm%ns
...
!--       In case of SLUrb, save also for the SLUrb surface (which are all upward at this point)
          IF ( slurb )  THEN
             DO  m = 1, surf_slurb%ns
                surf_slurb%qsws_urb(m) = dum_surf_flux
             ENDDO
          ENDIF
...
!
!--    Check for each quantity, if nudging shall not be applied.
       IF ( ALL( INT( unudge(:,1:nt) )  == -999999 ) )  THEN
          nudge_u  = .FALSE.
       ELSE
          nudged_quantities = TRIM( nudged_quantities ) // ' u,'
       ENDIF
       IF ( ALL( INT( vnudge(:,1:nt) )  == -999999 ) )  THEN
          nudge_v  = .FALSE.
       ELSE
          nudged_quantities = TRIM( nudged_quantities ) // ' v,'
       ENDIF
       IF ( ALL( INT( wnudge(:,1:nt) )  == -999999 ) )  THEN
          nudge_w  = .FALSE.
       ELSE
          nudged_quantities = TRIM( nudged_quantities ) // ' w,'
       ENDIF
       IF ( ALL( INT( ptnudge(:,1:nt) ) == -999999 ) )  THEN
          nudge_pt = .FALSE.
       ELSE
          nudged_quantities = TRIM( nudged_quantities ) // ' pt,'
       ENDIF
       IF ( ALL( INT( qnudge(:,1:nt) )  == -999999 ) )  THEN
          nudge_q  = .FALSE.
       ELSE
          nudged_quantities = TRIM( nudged_quantities ) // ' q,'
       ENDIF
!
!--    Overwrite initial profiles in case of nudging
       IF ( nudging )  THEN
          IF ( nudge_pt )  pt_init = ptnudge(:,1)
          IF ( nudge_u  )  u_init  = unudge(:,1)
          IF ( nudge_v  )  v_init  = vnudge(:,1)
          IF ( humidity  )  THEN ! is passive_scalar correct???
             IF ( nudge_q )  q_init = qnudge(:,1)
          ENDIF

          WRITE( message_string, * ) 'Initial profiles of ', TRIM( nudged_quantities ),            &
...
!--    scalar Rayleigh-damping are used.
       ...
       IF ( nudge_pt )  pt_init = ptnudge(:,nt) + fac * ( ptnudge(:,nt+1) - ptnudge(:,nt) )
       IF ( nudge_q  )  q_init  = qnudge(:,nt)  + fac * ( qnudge(:,nt+1)  - qnudge(:,nt)  )
       IF ( nudge_u  )  u_init  = unudge(:,nt)  + fac * ( unudge(:,nt+1)  - unudge(:,nt)  )
       IF ( nudge_v  )  v_init  = vnudge(:,nt)  + fac * ( vnudge(:,nt+1)  - vnudge(:,nt)  )
