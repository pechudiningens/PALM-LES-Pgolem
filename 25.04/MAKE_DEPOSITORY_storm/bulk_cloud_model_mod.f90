...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE bcm_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
...
!--                the dissipation rate (diss) from m2 s-3 to cm2 s-3.
                   IF ( collision_turbulence )  THEN
                      k_cr = k_cr0 * ( 1.0_wp + 0.05_wp *                                          &
                                       SQRT( SQRT( MIN( 600.0_wp, diss(k,j,i) * 1.0E4_wp ) ) )     &
                                     )
                   ELSE
                      k_cr = k_cr0
                   ENDIF
