...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE header
...
#if defined( __parallel )
    CHARACTER (LEN=20) ::  nesting_communication  !< type of data exchange between parent <-> child
    ...
    CHARACTER (LEN=86) ::  values              !< string indicating profile-prescribed values of quantities
    ...
    REAL(wp) ::  lower_coord_z       !< z-coordinate of nest domain
#endif
...
!-- Determine kind of model run
    ...
    IF ( nested_run )  THEN
       ...
       run_classification = 'nested ' // run_classification(1:63)
    ENDIF
#endif
    IF ( ocean_mode )  THEN
...
!-- Run-identification, date, time, host
    ...
    IF ( threads_per_task == 1 )  THEN
       IF ( serial_run )  THEN
          WRITE ( io, 105 )
       ELSE
          IF ( num_acc_devices == 0 )  THEN
             WRITE ( io, 103 )  numprocs, npex, npey, TRIM( char1 )
          ELSE
             WRITE ( io, 106 )  numprocs, num_acc_devices, npex, npey, TRIM( char1 )
          ENDIF
       ENDIF
    ELSE
    ...
    IF ( nested_run  .OR.  atmosphere_ocean_coupled_run )  THEN
       ...
       ELSE
          IF ( use_one_sided_communication )  THEN
             nesting_communication = 'one-sided (MPI3)'
          ELSE
             nesting_communication = 'via ISEND/IRECV'
          ENDIF
...
!-- Numerical schemes
    ...
    IF ( psolver(1:7) == 'poisfft' )  THEN
       ...
       IF ( .NOT. bc_lr_cyc  .OR.  .NOT. bc_ns_cyc )  WRITE( io, 114 )
...
#if defined( __parallel )
       IF ( nested_run )  THEN
          CALL pmc_get_model_info( request_for_cpl_id = my_cpl_id, nest_shift_z = lower_coord_z )
          IF ( lower_coord_z /= 0.0_wp )  WRITE( io, 252 )  zw(0)
       ENDIF
#endif
...
!-- Building output strings, starting with surface value
    IF ( large_scale_subsidence )  THEN
       ...
       DO  WHILE ( subs_vertical_gradient_level_i(i) /= -9999 )
          ...
          values = TRIM( values ) // ' ' // TRIM( coor_chr )
       ...
       IF ( .NOT. large_scale_forcing )  THEN
          WRITE ( io, 426 )  TRIM( coordinates ), TRIM( values ), TRIM( gradients ), TRIM( slices )
       ENDIF
...
!-- Building output strings.
    ...
    IF ( .NOT. large_scale_forcing )  THEN
       WRITE ( io, 423 )  TRIM( coordinates ), TRIM( values ), TRIM( gradients ), TRIM( slices )
    ENDIF
...
!-- Building output strings.
    ...
    IF ( .NOT. large_scale_forcing )  THEN
       WRITE ( io, 424 )  TRIM( coordinates ), TRIM( values ), TRIM( gradients ), TRIM( slices )
    ENDIF
...
!-- Boundary conditions
    ...
    IF ( ibc_uv_b == 0 )  THEN
       r_lower = TRIM( r_lower ) // ' uv(0)     = 0                     |'
    ELSEIF ( ibc_uv_b == 3 )  THEN
       r_lower = TRIM( r_lower ) // ' uv interpolated from parent       |'
    ELSE
    ...
    ELSEIF ( ibc_uv_t == 3 )  THEN
       r_upper = TRIM( r_upper ) // ' uv interpolated from parent       |'
    ELSE
    ...
    ELSEIF( ibc_pt_t == 3 )  THEN
       r_upper  = TRIM( r_upper  ) // ' pt interpolated from parent'
    ENDIF
    ...
    IF ( ocean_mode )  THEN
       IF ( ibc_sa_b == 1 )  THEN
          r_lower = 'sa(0)    = sa(1)'
       ELSEIF ( ibc_sa_b == 3 )  THEN
          r_lower = 'sa interpolated from parent'
       ENDIF
       IF ( ibc_sa_t == 0 )  THEN
    ...
    IF ( .NOT. bc_lr_cyc  .OR.  .NOT. bc_ns_cyc )  THEN
       ...
       WRITE ( io, 299 )  outflow_damping_width, outflow_damping_factor
...
!-- Initial Profiles
    WRITE ( io, 321 )
    IF ( TRIM( initializing_actions ) == 'interpolate_from_parent' )  THEN
...
!--    Building output strings, starting with surface temperature
       ...
       IF ( .NOT. nudging )  THEN
          WRITE ( io, 420 )  TRIM( coordinates ), TRIM( values ), TRIM( gradients ), TRIM( slices )
       ELSE
          WRITE ( io, 428 )
       ENDIF
...
!--    Building output strings, starting with surface humidity
       IF ( humidity )  THEN
          ...
          IF ( .NOT. nudging )  THEN
             WRITE ( io, 421 )  TRIM( coordinates ), TRIM( values ), TRIM( gradients ),            &
                                TRIM( slices )
          ENDIF
       ENDIF
!
...
!--    Building output strings, starting with surface humidity
       IF ( passive_scalar )  THEN
          ...
          WRITE ( io, 422 )  TRIM( coordinates ), TRIM( values ), TRIM( gradients ), TRIM( slices )
       ENDIF
...
!--    Building output strings, starting with surface salinity
       IF ( ocean_mode )  THEN
          ...
          WRITE ( io, 425 )  TRIM( coordinates ), TRIM( values ), TRIM( gradients ), TRIM( slices )
       ENDIF
...
!
!--    No parallel output for profiles, hence output_format must be adjusted.
...
!          IF ( TRIM( netcdf_compression_method ) == 'none' )  THEN
!             WRITE ( io, 344 )  output_format
!          ELSE
!             WRITE ( io, 354 )  TRIM( output_format ), TRIM( netcdf_compression_method ),          &
!                                netcdf_compression_level
!             IF ( netcdf_compression_nsd /= HUGE( 1 ) )  WRITE ( io, 359 )  netcdf_compression_nsd
!          ENDIF
          ...
          IF ( domask_chr /= '' )  THEN
             ...
             ELSE
                ...
                IF ( netcdf_compression_nsd /= HUGE( 1 ) )  THEN
                   WRITE ( io, 359 )  netcdf_compression_nsd
                ENDIF
             ENDIF
...
!-- Timeseries
    IF ( dt_dots /= 9999999.9_wp )  THEN
       ...
       output_format = netcdf_data_format_string
!
!--    No parallel output for time series, hence output_format must be adjusted.
       IF ( netcdf_data_format == 5 ) output_format = 'netCDF4/HDF5'
       IF ( netcdf_data_format == 6 ) output_format = 'netCDF4/HDF5 classic'
       WRITE ( io, 344 )  output_format
...
!
!-- Restart data formats
    IF ( TRIM( initializing_actions ) == 'read_restart_data'  .OR.  write_binary )  THEN
       WRITE ( io, 355 )
       IF ( TRIM( initializing_actions ) == 'read_restart_data' )  THEN
          WRITE ( io, 356 )  TRIM( restart_data_format_input )
       ENDIF
       IF ( write_binary )  THEN
          WRITE ( io, 357 )  TRIM( restart_data_format_output )
       ENDIF
    ENDIF
...
!-- Parameters of 1D-model
    IF ( INDEX( initializing_actions, 'set_1d-model_profiles' ) /= 0 )  THEN
       ...
       IF ( implicit_diffusion_1d )  THEN
          WRITE ( io, 501 )  implicit_timestep_factor
       ENDIF
...
106 FORMAT (' Number of PEs:',10X,I6,4X,'ACC-devices per node:',I4,/                               &
              35X,'Processor grid (x,y): (',I4,',',I4,')',1X,A)
107 FORMAT (35X,'A 1d-decomposition along ',A,' is used')
108 FORMAT (35X,'Max. # of parallel I/O streams is ',I5)
109 FORMAT (35X,'Coupled atmosphere-ocean run following'/                                          &
...
114 FORMAT ('     Specific FFTs with cosine transformations are used for non-cyclic BCs')
...
125 FORMAT (' --> No turbulence scheme used (constant diffusuin coefficient)')
...
#if defined( __parallel )
252 FORMAT (' Elevated child bottom height:                             z(w) = ',F10.3,' m')
#endif
...
272 FORMAT (' Single quasi-2D street canyon of infinite length in ',A,' direction' /               &
...
299 FORMAT (/'       outflow damping layer width = ',F8.2,' m, outflow damping factor =',F7.4)
...
303 FORMAT (/' Bottom surface fluxes are used in diffusion terms above surface grid points')
...
321 FORMAT (//' Initial profiles:'/                                                                &
              ' ----------------')
322 FORMAT ('    Interpolated from parent')
...
354 FORMAT ('       Output format: ',A, '   compression method: ',A, '   compression level: ',I7)
...
359 FORMAT ('                      lossy compression with ',I2, ' significant digits')
...
472 FORMAT ('    Disturbances continued during the run from i/j =',I4,' to i/j =',I4)
473 FORMAT ('    Disturbances cease as soon as the disturbance energy exceeds',F6.3, ' m**2/s**2')
474 FORMAT ('    Random number generator used    : ',A/)
475 FORMAT ('    The surface temperature is increased (or decreased, ','respectively, if'/         &
            '    the value is negative) by ',F5.2,' K at the beginning of the',' 3D-simulation'/)
476 FORMAT ('    The surface temperature increases (or decreases, ','respectively, if'/            &
            '    the value is negative) by ',F8.4,' K/h during the',' 3D-simulation'/)
477 FORMAT ('    The surface humidity is increased (or decreased, ','respectively, if the'/        &
...
501 FORMAT ('    Diffusion tendencies are calculated via Crank-Nicolson-scheme'/                   &
            '       time step enlarged by a factor of ',F6.2)
...
600 FORMAT (/' Nesting information:'/                                                              &
            ' --------------------'/                                                               &
            ' Atmosphere-ocean run with surface-flux coupling')
601 FORMAT (/' Nesting information:'/                                                              &
            ' --------------------'/                                                               &
