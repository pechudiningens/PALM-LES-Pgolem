...
!-- everywhere.
!-- This must not be done in case of a 3d-nested child domain or in case of pure vertical nesting
...
!-- known then.
    ...
    THEN
       ...
       DO  k = nzb, nzt
          ...
          w_l_l(k) = 0.0_wp
       ENDDO
       !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE( i, j, k) PRESENT( topo_flags, w_l_l, w ) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = nzb, nzt
                !$ACC ATOMIC
       ...
       ENDDO
#endif
       !$ACC PARALLEL LOOP PRESENT( ngp_2dh_wgrid, w_l ) IF(enable_openacc)
       DO  k = 1, nzt
          w_l(k) = w_l(k) / ngp_2dh_wgrid(k)
       ENDDO
       !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE( i, j, k) PRESENT( topo_flags, w_l, w ) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = nzb, nzt
...
!-- Compute the divergence of the provisional velocity field.
    ...
    DO  i = nxl, nxr
       DO  j = nys, nyn
          DO  k = 1, nzt
!
!--          No masking in case of cut-cell topgraphy.
             d(k,j,i) = ( ( u(k,j,i+1) - u(k,j,i) ) * rho_air(k) * ddx +                           &
                          ...
                          * MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(k,j,i), 0 )  .OR.             &
                                                   cut_cell_topography )
          ENDDO
       ENDDO
    ENDDO
...
!-- Runge-Kutta substep.
    IF ( intermediate_timestep_count == intermediate_timestep_count_max  .OR.                      &
       ...
       !$ACC PRESENT(d) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = nzb+1, nzt
...
!--    Store computed perturbation pressure and set boundary condition in z-direction.
       !$OMP PARALLEL DO PRIVATE (i,j,k) SCHEDULE( STATIC )
       !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
       !$ACC PRESENT(d, tend) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = nzb+1, nzt
                tend(k,j,i) = d(k,j,i)
...
!--       Dirichlet
          !$OMP PARALLEL DO PRIVATE (i,j) SCHEDULE( STATIC )
          !$ACC PARALLEL LOOP COLLAPSE(2) PRIVATE(i, j) &
          !$ACC PRESENT(tend) DEFAULT(NONE) IF(enable_openacc)
          DO  i = nxlg, nxrg
             DO  j = nysg, nyng
...
!-- Ghost layers are added in the output routines (except sor-method: see below)
    IF ( intermediate_timestep_count <= 1 )  THEN
       !$OMP PARALLEL DO PRIVATE (i,j,k) SCHEDULE( STATIC )
       !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
       !$ACC PRESENT(p, tend) PRESENT(rho_air) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl-1, nxr+1
          DO  j = nys-1, nyn+1
             DO  k = nzb, nzt+1
...
!-- imposed at the boundaries.
    ...
    !$ACC PRESENT(u, v, w, tend, ddzu, topo_flags) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxl, nxr
       DO  j = nys, nyn
