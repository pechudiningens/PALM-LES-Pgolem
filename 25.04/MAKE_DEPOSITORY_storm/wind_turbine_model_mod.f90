...
!-- Variables that have to be saved in the binary file for restarts:
    ...
    INTERFACE wtm_header
       MODULE PROCEDURE wtm_header
    END INTERFACE wtm_header
...
! Description:
! ------------
!> Header output for wind turbine parameters
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE wtm_header( io )
...
!
!-- Write wind turbine header
    WRITE( io, 1 )
    WRITE( io, 2 )  n_turbines

1   FORMAT (//' Wind turbine settings:'/' ---------------------'/)
2   FORMAT ('    number of turbines: ',I4)

 END SUBROUTINE wtm_header
...
!--    Determine the indices of the hub height.
       ...
       k_hub(inot) = INT( ( hub_z(inot) + 0.5_wp * dz(1) - nest_shift_z ) / dz(1) )

!
!--    Check if turbine position is inside the domain.
       IF ( i_hub(inot) < 0  .OR.  i_hub(inot) > nx  .OR.  j_hub(inot) < 0  .OR.                   &
            j_hub(inot) > ny )                                                                     &
       THEN
          WRITE( message_string, '(A,I4,4(A,F8.1))' )  'wind turbine ', inot,                      &
                                       ' outside domain bounds&position(x,y) = (', hub_x(inot),    &
                                       ',', hub_y(inot), ')&domain bounds along x: 0.0 / ', nx*dx, &
                                       '&domain bounds along y: 0.0 / ', ny*dy
          CALL message( 'wtm_init', 'WTM0009', 1, 2, 0, 6, 0 )
       ENDIF
...
!--    be contained in grid box 1.
       index_nacb(inot) = INT( ( hub_z(inot) - nacelle_radius(inot) - nest_shift_z ) / dz(1) ) + 1
       index_nact(inot) = INT( ( hub_z(inot) + nacelle_radius(inot) - nest_shift_z ) / dz(1) ) + 1
!
...
!--               nacelle and consequently we can set nac_cd_surf(k,j,i) = 0.0.
                  IF ( ( k >= index_nacb(inot) )  .AND.  ( k <= index_nact(inot) ) )  THEN
!
...
!--             Interpolation of the u-component:
                ...
                kk = ( rbz(ring,rseg) + 0.5_wp * dz(1) - nest_shift_z ) / dz(1)
!
...
!--             Interpolation of the w-component:
                ii = ( rbx(ring,rseg) - 0.5_wp * dx ) * ddx
                jj = ( rby(ring,rseg) - 0.5_wp * dy ) * ddy
                kk = ( rbz(ring,rseg) - nest_shift_z ) / dz(1)
!
!--             Interpolate only if all required information is available on the current PE:
                IF ( ( ii >= nxl )  .AND.  ( ii <= nxr ) )  THEN
                   IF ( ( jj >= nys )  .AND.  ( jj <= nyn ) )  THEN
...
!--                      each rotor area segment:
                         dist_u_3d = ( i * dx               - rbx(ring,rseg) )**2 +                &
                                     ...
                                     ( k * dz(1) - 0.5_wp * dz(1) + nest_shift_z                   &
                                                            - rbz(ring,rseg) )**2
