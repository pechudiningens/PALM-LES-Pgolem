...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE poisfft( ar )
    ...
    !$ACC UPDATE HOST(ar) IF(enable_openacc)
#endif
...
!-- Two-dimensional Fourier Transformation in x- and y-direction.
    ...
    !$ACC DATA CREATE(xy_in, zx_in, zx_out, xy_out, yz_in, yz_out) IF(enable_openacc)
!
...
!-- Transposition x --> z.
    ...
    !$ACC END DATA
    DEALLOCATE(xy_in, zx_in, zx_out, xy_out, yz_in, yz_out)

#if ! defined( __cuda_fft )
    !$ACC UPDATE DEVICE(ar) IF(enable_openacc)
#endif
