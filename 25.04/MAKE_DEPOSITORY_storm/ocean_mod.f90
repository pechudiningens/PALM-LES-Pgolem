...
!--------------------------------------------------------------------------------------------------!
 MODULE ocean_mod
    ...
    CHARACTER (LEN=20) ::  bc_sa_b = 'neumann'  !< default bottom boundary condition for salinity (no namelist parameter!)
    ...
    INTEGER(iwp) ::  sa_vertical_gradient_level_ind(12) = -9999  !< grid index values of sa_vertical_gradient_level(s)
    ...
    REAL(wp) ::  sa_vertical_gradient(12) = 0.0_wp                !< namelist parameter
    REAL(wp) ::  sa_vertical_gradient_level(12) = HUGE( 1.0_wp )  !< namelist parameter
    ...
    INTERFACE ocean_exchange_horiz
       MODULE PROCEDURE ocean_exchange_horiz
    END INTERFACE ocean_exchange_horiz
    ...
    INTERFACE ocean_rrd_global
       MODULE PROCEDURE ocean_rrd_global_ftn
       MODULE PROCEDURE ocean_rrd_global_mpi
    END INTERFACE ocean_rrd_global

    INTERFACE ocean_rrd_local
       MODULE PROCEDURE ocean_rrd_local_ftn
       MODULE PROCEDURE ocean_rrd_local_mpi
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE eqn_state_seawater
    ...
    INTEGER(iwp) ::  ioff    !< offset index used to access topography point
    INTEGER(iwp) ::  j       !< running index y direction
    INTEGER(iwp) ::  joff    !< offset index used to access topography point
    INTEGER(iwp) ::  k       !< running index z direction
    INTEGER(iwp) ::  koff    !< offset index used to access topography point
...
!
!--       Neumann conditions are assumed at top boundary (interface to atmosphere) and at bottom
!--       boundary, too (see further below).
    ...
    ENDDO
!
!-- Neumann conditions at surfaces.
    !$OMP PARALLEL DO PRIVATE( i, ioff, j, joff, k, koff )
    DO  m = 1, bc_hv%ns
       i = bc_hv%i(m)
       j = bc_hv%j(m)
       k = bc_hv%k(m)
       ...
       prho(k+koff,j+joff,i+ioff)      = prho(k,j,i)
       rho_ocean(k+koff,j+joff,i+ioff) = rho_ocean(k,j,i)
    ENDDO
!
!-- Also set Neumann conditions, if no surfaces are defined at the bottom boundary (open boundary).
    prho(nzb,:,:) = prho(nzb+1,:,:)
    rho_ocean(nzb,:,:) = rho_ocean(nzb+1,:,:)
...
!--    In-situ density
    ...
    ENDDO
!
!-- Neumann conditions at surfaces within grid column (j,i).
...
!
!-- Neumann condition are always assumed at the top boundary (interface to atmosphere).
    prho(nzt+1,j,i)      = prho(nzt,j,i)
    rho_ocean(nzt+1,j,i) = rho_ocean(nzt,j,i)
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE ocean_parin
    ...
    NAMELIST /ocean_parameters/  bc_sa_t,                                                          &
                                 bottom_salinityflux,                                              &
                                 salinity,                                                         &
                                 sa_surface,                                                       &
                                 sa_vertical_gradient,                                             &
...
!
!-- Move to the beginning of the namelist file and try to find and read the namelist called
!-- ocean_parameters.
    REWIND( 11 )
    READ( 11, ocean_parameters, IOSTAT=io_status )

!
!-- Action depending on the READ status
    IF ( io_status == 0 )  THEN
!
!--    ocean_parameters namelist was found and read correctly. Set switch that enables PALM's ocean
!--    mode.
       IF ( .NOT. switch_off_module )  ocean_mode = .TRUE.

    ELSEIF ( io_status > 0 )  THEN
!
!--    ocean_parameters namelist was found but contained errors. Print an error message including
...
!-- Check for invalid combinations
    IF ( nested_run  .AND.  nest_shift_z /= 0.0 )  THEN
       message_string = 'ocean mode does not allow vertically shifted childs'
       CALL message( 'ocean_check_parameters', 'OCM0001', 3, 2, 0, 6, 0 )
    ENDIF

    IF ( cyclic_fill_initialization )  THEN
       message_string = 'ocean mode does not allow cyclic-fill initialization'
       CALL message( 'ocean_check_parameters', 'OCM0002', 1, 2, 0, 6, 0 )
    ENDIF

    IF ( homogeneous_initialization_child )  THEN
       message_string = 'ocean mode does not allow homogeneous_initialization_child = .T.'
       CALL message( 'ocean_check_parameters', 'OMC0012', 3, 2, 0, 6, 0 )
    ENDIF

    IF ( rans_mode )  THEN
       message_string = 'RANS turbulence parameterization not adapted for ocean mode'
       CALL message( 'ocean_check_parameters', 'OMC0013', 0, 1, 0, 6, 0 )
    ENDIF

!
!-- Check ocean setting
    IF ( .NOT. atmosphere_ocean_coupled_run  .AND.  TRIM( coupling_char ) == '_O' .AND.            &
         .NOT. ocean_mode )  THEN
...
!--    is done via palmrun-option -y)
       ...
       CALL message( 'ocean_check_parameters', 'OCM0003', 1, 2, 0, 6, 0 )
...
!-- Ocean version must use flux boundary conditions at the top
    IF ( .NOT. use_top_fluxes )  THEN
       ...
       CALL message( 'ocean_check_parameters', 'OCM0004', 1, 2, 0, 6, 0 )
    ENDIF
...
!-- Boundary conditions for salinity
    ...
    ELSE
       ...
       CALL message( 'ocean_check_parameters', 'OCM0005', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_sa_b == 'neumann' )  THEN
       ibc_sa_b = 1
    ELSEIF ( bc_sa_b == 'nested' )  THEN
       ibc_sa_b = 3
    ENDIF
    ...
    IF ( .NOT. salinity )  THEN
       ...
       THEN
          ...
          CALL message( 'ocean_check_parameters', 'OCM0006', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF

    IF ( ibc_sa_t == 1  .AND.  top_salinityflux == 9999999.9_wp  .AND.  salinity )  THEN
       message_string = 'boundary condition: bc_sa_t = "' // TRIM( bc_sa_t ) //                    &
                        '" requires to set top_salinityflux'
       CALL message( 'ocean_check_parameters', 'OCM0007', 1, 2, 0, 6, 0 )
    ENDIF
...
!-- specification of a constant salinity flux is forbidden.
    IF ( ibc_sa_t == 0  .AND.  constant_top_salinityflux  .AND.  top_salinityflux /= 0.0_wp )  THEN
       ...
       CALL message( 'ocean_check_parameters', 'OCM0008', 1, 2, 0, 6, 0 )
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE ocean_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE ocean_exchange_horiz( location )
    ...
    END SUBROUTINE ocean_exchange_horiz
...
2   FORMAT ('    --> Craik-Leibovich vortex force and Stokes drift switched',' on'/                &
...
!--    condition. This is not realized for the ocean mode, therefore a dummy argument is used.
       IF ( salinity )  THEN
          CALL init_vertical_profiles( sa_vertical_gradient_level_ind, sa_vertical_gradient_level, &
                                       sa_vertical_gradient, sa_init, sa_surface, dum, 'sa' )
       ENDIF
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE ocean_actions_ij( i, j, location )
    ...
    INTEGER(iwp)             ::  dummy     !< dummy to avoid compiler error concerning unused variable
...
!--    sa-tendency terms with no communication
       ...
       CALL diffusion_s( sa, surf_top%sasws, surf_def%sasws, surf_lsm%sasws, surf_usm%sasws )
...
!-- First, start with tendency-terms for salinity.
    IF ( salinity )  THEN
       ...
       ENDIF
       CALL diffusion_s( i, j, sa, surf_top%sasws, surf_def%sasws, surf_lsm%sasws, surf_usm%sasws )
...
!-- Boundary conditions for salinity.
    IF ( ibc_sa_b == 1 )  THEN
       ...
       DO  m = 1, bc_hv%ns
          ...
          j = bc_hv%j(m)
          k = bc_hv%k(m)
          sa_p(k+bc_hv%koff(m),j+bc_hv%joff(m),i+bc_hv%ioff(m)) = sa_p(k,j,i)
       ENDDO
    ENDIF
!
...
!--------------------------------------------------------------------------------------------------!
 ...
 END SUBROUTINE ocean_rrd_global_ftn
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE ocean_rrd_global_mpi
    ...
    CALL rrd_mpi_io( 'bc_sa_t', bc_sa_t )
    CALL rrd_mpi_io( 'bottom_salinityflux', bottom_salinityflux )
    CALL rrd_mpi_io( 'salinity', salinity )
    CALL rrd_mpi_io_global_array( 'sa_init', sa_init )
    CALL rrd_mpi_io( 'sa_surface', sa_surface )
    CALL rrd_mpi_io_global_array( 'sa_vertical_gradient', sa_vertical_gradient )
    CALL rrd_mpi_io_global_array( 'sa_vertical_gradient_level', sa_vertical_gradient_level )
    CALL rrd_mpi_io_global_array( 'sa_vertical_gradient_level_ind', sa_vertical_gradient_level_ind )
    CALL rrd_mpi_io( 'stokes_waveheight', stokes_waveheight )
    CALL rrd_mpi_io( 'stokes_wavelength', stokes_wavelength )
    CALL rrd_mpi_io( 'surface_cooling_spinup_time', surface_cooling_spinup_time )
    CALL rrd_mpi_io( 'top_salinityflux', top_salinityflux )
    CALL rrd_mpi_io( 'wave_breaking', wave_breaking )

 END SUBROUTINE ocean_rrd_global_mpi
...
!--------------------------------------------------------------------------------------------------!
 ...
 END SUBROUTINE ocean_rrd_local_ftn
...
!-- averaged data is useless and can lead to faulty averaging.
    ...
    CALL rrd_mpi_io( 'sa', sa )

    CALL rd_mpi_io_check_array( 'sa_av' , found = array_found )
    IF ( array_found )  THEN
       IF ( .NOT. ALLOCATED( sa_av ) )  ALLOCATE( sa_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
       CALL rrd_mpi_io( 'sa_av', sa_av )
    ENDIF

 END SUBROUTINE ocean_rrd_local_mpi
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE ocean_wrd_global
    ...
    ELSEIF ( restart_data_format_output(1:3) == 'mpi' )  THEN
       ...
       CALL wrd_mpi_io( 'bc_sa_t', bc_sa_t )
       CALL wrd_mpi_io( 'bottom_salinityflux', bottom_salinityflux )
       CALL wrd_mpi_io( 'salinity', salinity )
       CALL wrd_mpi_io_global_array( 'sa_init', sa_init )
       CALL wrd_mpi_io( 'sa_surface', sa_surface )
       ...
       CALL wrd_mpi_io( 'surface_cooling_spinup_time', surface_cooling_spinup_time )
       CALL wrd_mpi_io( 'top_salinityflux', top_salinityflux )
       CALL wrd_mpi_io( 'wave_breaking', wave_breaking )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE ocean_wrd_local
    ...
    ELSEIF ( restart_data_format_output(1:3) == 'mpi' )  THEN
       ...
       IF ( ALLOCATED( rho_ocean_av ) )  CALL wrd_mpi_io( 'rho_ocean_av', rho_ocean_av )
       CALL wrd_mpi_io( 'sa', sa )
       IF ( ALLOCATED( sa_av ) )  CALL wrd_mpi_io( 'sa_av', sa_av )
...
!--       Precalculate cosine and sine of rotation angle
       ...
       CASE DEFAULT
          ...
          CALL message( 'stokes_drift_terms', 'OCM0010', 1, 2, 0, 6, 0 )
...
!--    v-component
       ...
       CASE DEFAULT
          ...
          CALL message( 'stokes_drift_terms', 'OCM0011', 1, 2, 0, 6, 0 )
