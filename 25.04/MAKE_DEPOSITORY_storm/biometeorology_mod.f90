...
! Description:
! ------------
!> Human thermal comfort module calculating thermal perception of a sample human being under the
!> current meteorological conditions.
!>
!> @todo Alphabetical sorting of variable declarations
             ...
             IF ( .NOT. do_average_mrt  .AND.  ALLOCATED( mrt_av_grid ) )  THEN
                ...
                IF ( mrt_include_sw )  THEN
                   DO  l = 1, nmrtbl
                      ...
                      mrt_av_grid(k,j,i) = mrt_av_grid(k,j,i)  +                                   &
                                           SQRT( SQRT( ( human_absorb * mrtinsw(l) + mrtinlw(l) ) /&
                ...
                ELSE
                   DO  l = 1, nmrtbl
                      ...
                      k = mrtbl(iz,l)
                      mrt_av_grid(k,j,i) = mrt_av_grid(k,j,i)  +                                   &
                                           SQRT( SQRT( mrtinlw(l) / ( human_emiss * sigma_sb ) )   &
                                               ) - degc_to_k
                   ENDDO
                ENDIF
             ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE bio_check_data_output( var, unit, j )
...
!--    bio_calculate_thermal_index_maps).
       CASE ( 'bio_mrt', 'bio_mrt*' )
          ...
          IF ( .NOT. ALLOCATED( tmrt_grid ) )  THEN
             ...
             IF ( .NOT. data_output_raw )  tmrt_grid = output_fill_value
          ENDIF
       ...
       CASE ( 'bio_perct*' )
          ...
          IF ( j == 0 )  THEN                !< if instantaneous input
             ...
             IF ( .NOT. ALLOCATED( perct ) )  THEN
                ...
                IF ( .NOT. data_output_raw )  perct = output_fill_value
             ENDIF
       ...
       CASE ( 'bio_utci*' )
          ...
          IF ( j == 0 )  THEN
             ...
             IF ( .NOT. ALLOCATED( utci ) )  THEN
                ...
                IF ( .NOT. data_output_raw )  utci = output_fill_value
             ENDIF
          ELSE
       ...
       CASE ( 'bio_pet*' )
          ...
          IF ( j == 0 )  THEN
             ...
             IF ( .NOT. ALLOCATED( pet ) )  THEN
                ...
                IF ( .NOT. data_output_raw )  pet = output_fill_value
             ENDIF
          ELSE
...
!--    Break if required modules "radiation" is not available.
       IF ( .NOT.  radiation )  THEN
          ...
          CALL message( 'check_parameters', 'BIO0001', 1, 2, 0, 6, 0 )
          unit = 'illegal'
       ENDIF
!
...
!-- Output variables
    ...
    LOGICAL, INTENT(OUT)           ::  two_d  !< Flag parameter that indicates 2D variables,
                                              !< horizontal cross sections, must be .TRUE. for thermal indices
    ...
    SELECT CASE ( TRIM( variable ) )
        ...
        CASE ( 'bio_mrt_xy' )
           ...
           DO  l = 1, nmrtbl
              ...
              IF ( av == 0 )  THEN
                 IF ( mrt_include_sw )  THEN
                    local_pf(i,j,k) = SQRT( SQRT( ( human_absorb * mrtinsw(l) + mrtinlw(l) ) /     &
                                                  ( human_emiss * sigma_sb ) )                     &
                                          ) - degc_to_k
                 ELSE
                    local_pf(i,j,k) = SQRT( SQRT( mrtinlw(l) / ( human_emiss * sigma_sb ) )        &
                                          ) - degc_to_k
                 ENDIF
              ELSE
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE bio_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ...
    ELSE
       ...
       THEN
          WRITE( message_string, * ) 'combination of mrt_minlevel = ', mrt_minlevel,               &
                                     ...
                                     ' body 1,1 m)'
          CALL message( 'bio_init', 'BIO0003', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE bio_init_checks
    ...
    IF ( ( .NOT. radiation_interactions )  .AND.  ( thermal_comfort ) )  THEN
       ...
       CALL message( 'bio_init_checks', 'BIO0004', 1, 2, 0, 6, 0 )
    ENDIF
...
!-- Internal variables
    ...
    NAMELIST /biometeorology_parameters/  switch_off_module,                                       &
                                          thermal_comfort
...
!--    bio_check_data_output yet.
       ...
       DO  l = 1, nmrtbl
          ...
          IF ( k - topo_top_ind(j,i,0) == bio_cell_level + 1 )  THEN
             IF ( mrt_include_sw )  THEN
                tmrt_grid(j,i) = SQRT( SQRT( ( human_absorb * mrtinsw(l) + mrtinlw(l) ) /          &
                                             ( human_emiss * sigma_sb ) )                          &
                                     ) - degc_to_k
             ELSE
                tmrt_grid(j,i) = SQRT( SQRT( mrtinlw(l) / ( human_emiss * sigma_sb ) ) ) - degc_to_k
             ENDIF
          ENDIF
       ENDDO
    ENDIF
