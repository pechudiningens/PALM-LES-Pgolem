...
!-- Data type comprises u, v, w, pt, and q at lateral and top boundaries.
    ...
    TYPE(nest_offl_type) ::  nest_offl  !< data structure for data input at lateral and top boundaries (provided by dynamic file)
...
!-- Public subroutines
    ...
    INTERFACE nesting_offl_init_modules
       MODULE PROCEDURE nesting_offl_init_modules
    END INTERFACE nesting_offl_init_modules
...
!
!-- Initialize arrays for the boundary conditions at the first call.
    ...
    ELSE
       IF ( bc_dirichlet_l )  THEN
          ...
          IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
             DO  n = 1, UBOUND( nest_offl%var_names_chem_l, 1 )
                IF ( check_existence( nest_offl%var_names, nest_offl%var_names_chem_l(n) ) )  THEN
                   CALL get_variable( pids_id, TRIM( nest_offl%var_names_chem_l(n) ),              &
       ...
       IF ( bc_dirichlet_r )  THEN
          ...
          IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
             DO  n = 1, UBOUND( nest_offl%var_names_chem_r, 1 )
                IF ( check_existence( nest_offl%var_names, nest_offl%var_names_chem_r(n) ) )  THEN
                   CALL get_variable( pids_id, TRIM( nest_offl%var_names_chem_r(n) ),              &
       ...
       IF ( bc_dirichlet_n )  THEN
          ...
          IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
             DO  n = 1, UBOUND( nest_offl%var_names_chem_n, 1 )
                IF ( check_existence( nest_offl%var_names, nest_offl%var_names_chem_n(n) ) )  THEN
                   CALL get_variable( pids_id, TRIM( nest_offl%var_names_chem_n(n) ),              &
       ...
       IF ( bc_dirichlet_s )  THEN
          ...
          IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
             DO  n = 1, UBOUND( nest_offl%var_names_chem_s, 1 )
                IF ( check_existence( nest_offl%var_names, nest_offl%var_names_chem_s(n) ) )  THEN
                   CALL get_variable( pids_id, TRIM( nest_offl%var_names_chem_s(n) ),              &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE nesting_offl_bc
    ...
    INTEGER(iwp), DIMENSION(nzb+1:nzt+1) ::  ngp_s  !< total number of boundary grid points on u-grid
    INTEGER(iwp), DIMENSION(nzb+1:nzt+1) ::  ngp_u  !< total number of boundary grid points on u-grid
    INTEGER(iwp), DIMENSION(nzb+1:nzt+1) ::  ngp_v  !< total number of boundary grid points on v-grid
...
!--       Pre-interpolate profile before mapping onto the boundaries.
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%u_l(0,k,1),                                 &
                                             nest_offl%u_l(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  j = nys, nyn
...
!--    w-component
       ...
       ELSE
          DO  k = nzb+1, nzt-1
             var_1d(k) = interpolate_linear( nest_offl%w_l(0,k,1),                                 &
                                             nest_offl%w_l(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  j = nys, nyn
...
!--    v-component
       ...
       ELSE
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%v_l(0,k,1),                                 &
                                             nest_offl%v_l(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  j = nysv, nyn
...
!--    Potential temperature
       IF ( .NOT. neutral )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%pt_l(0,k,1),                             &
...
!--    Humidity
       IF ( humidity )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%q_l(0,k,1),                              &
                                                nest_offl%q_l(1,k,1),                              &
                                                fac_dt )
             ENDDO
             DO  j = nys, nyn
       ...
       IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
          DO  n = 1, UBOUND( chem_species, 1 )
             IF ( nest_offl%chem_from_file_l(n) )  THEN
                ...
                ELSE
                   DO  k = nzb+1, nzt
                      var_1d(k) = interpolate_linear( nest_offl%chem_l(0,k,1,n),                   &
...
!--    u-component
       ...
       ELSE
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%u_r(0,k,1),                                 &
                                             nest_offl%u_r(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  j = nys, nyn
...
!--    w-component
       ...
       ELSE
          DO  k = nzb+1, nzt-1
             var_1d(k) = interpolate_linear( nest_offl%w_r(0,k,1),                                 &
                                             nest_offl%w_r(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  j = nys, nyn
...
!--    v-component
       ...
       ELSE
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%v_r(0,k,1),                                 &
                                             nest_offl%v_r(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  j = nysv, nyn
...
!--    Potential temperature
       IF ( .NOT. neutral )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%pt_r(0,k,1),                             &
...
!--    Humidity
       IF ( humidity )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%q_r(0,k,1),                              &
                                                nest_offl%q_r(1,k,1),                              &
                                                fac_dt )
             ENDDO
             DO  j = nys, nyn
       ...
       IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
          DO  n = 1, UBOUND( chem_species, 1 )
             IF ( nest_offl%chem_from_file_r(n) )  THEN
                ...
                ELSE
                   DO  k = nzb+1, nzt
                      var_1d(k) = interpolate_linear( nest_offl%chem_r(0,k,1,n),                   &
...
!--    v-component
       ...
       ELSE
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%v_n(0,k,1),                                 &
                                             nest_offl%v_n(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  i = nxl, nxr
...
!--    w-component
       ...
       ELSE
          DO  k = nzb+1, nzt-1
             var_1d(k) = interpolate_linear( nest_offl%w_n(0,k,1),                                 &
                                             nest_offl%w_n(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  i = nxl, nxr
...
!--    u-component
       ...
       ELSE
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%u_n(0,k,1),                                 &
                                             nest_offl%u_n(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  i = nxlu, nxr
...
!--    Potential temperature
       IF ( .NOT. neutral )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%pt_n(0,k,1),                             &
...
!--    Humidity
       IF ( humidity )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%q_n(0,k,1),                              &
                                                nest_offl%q_n(1,k,1),                              &
                                                fac_dt )
             ENDDO
             DO  i = nxl, nxr
       ...
       IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
          DO  n = 1, UBOUND( chem_species, 1 )
             IF ( nest_offl%chem_from_file_n(n) )  THEN
                ...
                ELSE
                   DO  k = nzb+1, nzt
                      var_1d(k) = interpolate_linear( nest_offl%chem_n(0,k,1,n),                   &
...
!--    v-component
       ...
       ELSE
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%v_s(0,k,1),                                 &
                                             nest_offl%v_s(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  i = nxl, nxr
...
!--    w-component
       ...
       ELSE
          DO  k = nzb+1, nzt-1
             var_1d(k) = interpolate_linear( nest_offl%w_s(0,k,1),                                 &
                                             nest_offl%w_s(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  i = nxl, nxr
...
!--    u-component
       ...
       ELSE
          DO  k = nzb+1, nzt
             var_1d(k) = interpolate_linear( nest_offl%u_s(0,k,1),                                 &
                                             nest_offl%u_s(1,k,1),                                 &
                                             fac_dt )
          ENDDO
          DO  i = nxlu, nxr
...
!--    Potential temperature
       IF ( .NOT. neutral )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%pt_s(0,k,1),                             &
...
!--    Humidity
       IF ( humidity )  THEN
          ...
          ELSE
             DO  k = nzb+1, nzt
                var_1d(k) = interpolate_linear( nest_offl%q_s(0,k,1),                              &
                                                nest_offl%q_s(1,k,1),                              &
                                                fac_dt )
             ENDDO
             DO  i = nxl, nxr
       ...
       IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
          DO  n = 1, UBOUND( chem_species, 1 )
             IF ( nest_offl%chem_from_file_s(n) )  THEN
                ...
                ELSE
                   DO  k = nzb+1, nzt
                      var_1d(k) = interpolate_linear( nest_offl%chem_s(0,k,1,n),                   &
...
!-- u-component
    IF ( lod == 2 )  THEN
       DO  i = nxlu, nxr
          DO  j = nys, nyn
             u(nzt+1,j,i) = interpolate_linear( nest_offl%u_top(0,j,i),                            &
                                                nest_offl%u_top(1,j,i),                            &
                                                fac_dt ) *                                         &
                            MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(nzt+1,j,i), 1 ) )
             u_ref_l(nzt+1) = u_ref_l(nzt+1) + u(nzt+1,j,i) *                                      &
                                               MERGE( 1.0_wp, 0.0_wp,                              &
                                                      BTEST( topo_flags(nzt+1,j,i), 1 ) )
             ngp_u(nzt+1) = ngp_u(nzt+1) + MERGE( 1, 0, BTEST( topo_flags(nzt+1,j,i), 1 ) )
          ENDDO
       ENDDO
    ELSE
       var_1d(nzt+1) = interpolate_linear( nest_offl%u_top(0,1,1),                                 &
...
!-- v-component
    IF ( lod == 2 )  THEN
       DO  i = nxl, nxr
          DO  j = nysv, nyn
             v(nzt+1,j,i) = interpolate_linear( nest_offl%v_top(0,j,i),                            &
                                                nest_offl%v_top(1,j,i),                            &
                                                fac_dt ) *                                         &
                            MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(nzt+1,j,i), 2 ) )
             v_ref_l(nzt+1) = v_ref_l(nzt+1) + v(nzt+1,j,i) *                                      &
                                               MERGE( 1.0_wp, 0.0_wp,                              &
                                                      BTEST( topo_flags(nzt+1,j,i), 2 ) )
             ngp_v(nzt+1) = ngp_v(nzt+1) + MERGE( 1, 0, BTEST( topo_flags(nzt+1,j,i), 2 ) )
          ENDDO
       ENDDO
    ELSE
       var_1d(nzt+1) = interpolate_linear( nest_offl%v_top(0,1,1),                                 &
       ...
       v_ref_l(nzt+1) = v_ref_l(nzt+1) + SUM( v(nzt+1,nysv:nyn,nxl:nxr) *                          &
                                         MERGE( 1.0_wp, 0.0_wp,                                    &
                                                BTEST( topo_flags(nzt+1,nysv:nyn,nxl:nxr), 2 ) ) )
       ngp_v(nzt+1) = ngp_v(nzt+1) +                                                               &
                      SUM( MERGE( 1, 0, BTEST( topo_flags(nzt+1,nys:nyn,nxlu:nxr), 2 ) ) )
    ENDIF
!
...
!-- w-component
    IF ( lod == 2 )  THEN
       DO  i = nxl, nxr
          DO  j = nys, nyn
             w(nzt,j,i) = interpolate_linear( nest_offl%w_top(0,j,i),                              &
       ...
       ENDDO
    ELSE
       var_1d(nzt) = interpolate_linear( nest_offl%w_top(0,1,1),                                   &
...
!-- Potential temperature
    IF ( .NOT. neutral )  THEN
       IF ( lod == 2 )  THEN
          DO  i = nxl, nxr
             DO  j = nys, nyn
                pt(nzt+1,j,i) = interpolate_linear( nest_offl%pt_top(0,j,i),                       &
                                                    nest_offl%pt_top(1,j,i),                       &
                                                    fac_dt )
                pt_ref_l(nzt+1) = pt_ref_l(nzt+1) + pt(nzt+1,j,i) *                                &
                                                    MERGE( 1.0_wp, 0.0_wp,                         &
                                                           BTEST( topo_flags(nzt+1,j,i), 0 ) )
                ngp_s(nzt+1) = ngp_s(nzt+1) + MERGE( 1, 0, BTEST( topo_flags(nzt+1,j,i), 0 ) )
             ENDDO
          ENDDO
       ELSE
          var_1d(nzt+1) = interpolate_linear( nest_offl%pt_top(0,1,1),                             &
                                              nest_offl%pt_top(1,1,1),                             &
                                              fac_dt )
          pt(nzt+1,nys:nyn,nxl:nxr) = var_1d(nzt+1)
          pt_ref_l(nzt+1) = pt_ref_l(nzt+1) + SUM( pt(nzt+1,nys:nyn,nxl:nxr) *                     &
                                              MERGE( 1.0_wp, 0.0_wp,                               &
                                                   BTEST( topo_flags(nzt+1,nys:nyn,nxl:nxr), 0 ) ) )
          ngp_s(nzt+1) = ngp_s(nzt+1) +                                                            &
                         SUM( MERGE( 1, 0, BTEST( topo_flags(nzt+1,nys:nyn,nxlu:nxr), 0 ) ) )
       ENDIF
    ENDIF
!
!--    humidity
    IF ( humidity )  THEN
       IF ( lod == 2 )  THEN
          DO  i = nxl, nxr
             DO  j = nys, nyn
                q(nzt+1,j,i) = interpolate_linear( nest_offl%q_top(0,j,i),                         &
          ...
          ENDDO
       ELSE
          var_1d(nzt+1) = interpolate_linear( nest_offl%q_top(0,1,1),                              &
       ...
       DO  n = 1, UBOUND( chem_species, 1 )
          IF ( nest_offl%chem_from_file_t(n) )  THEN
             IF ( lod == 2 )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      chem_species(n)%conc(nzt+1,j,i) = interpolate_linear(                        &
                ...
                ENDDO
             ELSE
                var_1d(nzt+1) = interpolate_linear( nest_offl%chem_top(0,1,1,n),                   &
...
!-- averaging is required.
    ...
    ENDIF
!
!-- Sum-up number of boundary grid points being not topography. These are used to compute
!-- correct averages of the reference profiles, which in turn are mapped on the _init profiles.
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, ngp_u, nzt+1-nzb, MPI_INTEGER, MPI_SUM, comm2d, ierr )
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, ngp_v, nzt+1-nzb, MPI_INTEGER, MPI_SUM, comm2d, ierr )
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, ngp_s, nzt+1-nzb, MPI_INTEGER, MPI_SUM, comm2d, ierr )
#else
    u_ref  = u_ref_l
    v_ref  = v_ref_l
...
#endif
!
!-- Average data. Note, reference profiles up to nzt are derived from lateral boundaries, but at the
!-- model top they are derived from the whole top boundary layer. Thus, the number of input values
!-- as given by ngp_** is much smaller for nzb+1:nzt than for level nzt+1.
!-- In case of nesting, under specific circumstances all parent nzb+1 levels may be located within
!-- topography, so the case ngp_** = 0 needs to be taken care of.
    WHERE ( ngp_u(nzb+1:nzt+1) /= 0 )
       u_ref(nzb+1:nzt+1) = u_ref(nzb+1:nzt+1) / REAL( ngp_u(nzb+1:nzt+1), KIND = wp )
    ENDWHERE
    WHERE ( ngp_v(nzb+1:nzt+1) /= 0 )
       v_ref(nzb+1:nzt+1) = v_ref(nzb+1:nzt+1) / REAL( ngp_v(nzb+1:nzt+1), KIND = wp )
    ENDWHERE
    IF ( humidity )  THEN
       WHERE ( ngp_s(nzb+1:nzt+1) /= 0 )
          q_ref(nzb+1:nzt+1) = q_ref(nzb+1:nzt+1) / REAL( ngp_s(nzb+1:nzt+1), KIND = wp )
       ELSEWHERE
!
!--       In order to avoid errors in cases where the mixing ratio is accessed at locations within
!--       topography.
          q_ref(nzb+1:nzt+1) = q_surface
       ENDWHERE
    ENDIF
    IF ( .NOT. neutral )  THEN
       WHERE ( ngp_s(nzb+1:nzt+1) /= 0 )
          pt_ref(nzb+1:nzt+1) = pt_ref(nzb+1:nzt+1) / REAL( ngp_s(nzb+1:nzt+1), KIND = wp )
       ELSEWHERE
!
!--       In order to avoid errors in cases where the temperature is accessed at locations within
!--       topography.
          pt_ref(nzb+1:nzt+1) = pt_surface
       ENDWHERE
    ENDIF
    IF ( air_chemistry  .AND.  nesting_offline_chem )  THEN
       DO  n = 1, UBOUND( chem_species, 1 )
          WHERE ( ngp_s(nzb+1:nzt+1) /= 0 )
             ref_chem(nzb+1:nzt+1,n) = ref_chem(nzb+1:nzt+1,n) /                                   &
                                       REAL( ngp_s(nzb+1:nzt+1), KIND = wp )
          ENDWHERE
       ENDDO
    ENDIF
!
...
!
!-- Further, adjust Rayleigh damping height in case of time-changing boundary layer heights.
!-- Note, this is only possible when potential temperature is available.
    IF ( .NOT. neutral )  CALL adjust_sponge_layer
...
! Description:
! ------------
!> Set the lateral and top boundary conditions for other modules.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE nesting_offl_bc_modules
...
!> criterion.
!--------------------------------------------------------------------------------------------------!
 FUNCTION nesting_offl_calc_zi() RESULT( zi_ribulk )
    ...
    REAL(wp) ::  zi_ribulk               !< boundary-layer depth according to bulk Richardson criterion,
                                         !< i.e. the height where Ri_bulk exceeds the critical bulk Richardson number ri_bulk_crit
...
!--          the wind profile in order to detect also ABL heights in the stable boundary layer.
 ...
 END FUNCTION nesting_offl_calc_zi
...
!--    Assure that rayleigh damping starts well above the boundary layer.
       rdh = MIN( MAX( nesting_offl_calc_zi() * 1.3_wp, 10.0_wp * dz(1) ),                         &
...
!-- dynamic driver. If not, discrepancies between the initial data and the imposed lateral boundary
...
!-- given, are working, do not abort the run but assume LOD2 forcing.
    CALL get_attribute( pids_id, char_lod, nest_offl%lod_east_pt,  .FALSE., 'ls_forcing_left_pt',  &
                        ignore_error = .TRUE. )
    CALL get_attribute( pids_id, char_lod, nest_offl%lod_east_qv,  .FALSE., 'ls_forcing_left_qv',  &
                        ignore_error = .TRUE. )
    CALL get_attribute( pids_id, char_lod, nest_offl%lod_east_u,   .FALSE., 'ls_forcing_left_u',   &
                        ignore_error = .TRUE. )
    CALL get_attribute( pids_id, char_lod, nest_offl%lod_east_v,   .FALSE., 'ls_forcing_left_v',   &
                        ignore_error = .TRUE. )
...
#endif
!
!-- Temporary workaround until most of the dynamic drivers contain a LOD attribute. Some proprocessor
!-- tools did not provide the LOD attribute in the past. In order to still use these older dynamic drivers,
!-- a temporary workaround is used. If the LOD is not given, a NetCDF interal error will occur but the
!-- simulation will not be aborted since the ignore_error attribute is passed. However, the respective
...
!
!-- In case no humidity or potential temperature equation is solved, make sure that their default
!-- LOD settings do not trigger the following error message. To guarantee this, just set their
!-- LOD value to the one of the u-component.
    IF ( neutral )  THEN
       nest_offl%lod_east_pt  = nest_offl%lod_east_u
       nest_offl%lod_north_pt = nest_offl%lod_north_u
       nest_offl%lod_south_pt = nest_offl%lod_south_u
       nest_offl%lod_west_pt  = nest_offl%lod_west_u
       nest_offl%lod_top_pt   = nest_offl%lod_top_u
    ENDIF
    IF ( .NOT. humidity )  THEN
       nest_offl%lod_east_qv  = nest_offl%lod_east_u
       nest_offl%lod_north_qv = nest_offl%lod_north_u
       nest_offl%lod_south_qv = nest_offl%lod_south_u
       nest_offl%lod_west_qv  = nest_offl%lod_west_u
       nest_offl%lod_top_qv   = nest_offl%lod_top_u
    ENDIF
!
...
!-- either nxl:nxr, or nys:nyn (for LOD=2), or it reduces to one element for LOD=1. For the inner
!-- subdomains, which have no lateral boundaries, allocate a dummy array as well, in order to
!-- enable netcdf parallel access. This case, arrays will be allocated with dimension length zero.
    IF ( bc_dirichlet_l )  THEN
...
!-- in order to interpolate in between. For the inner subdomains, which have no lateral boundaries,
!-- allocate a dummy array as well, in order to enable netcdf parallel access. This case, arrays
!-- will be allocated with dimension length zero.
    IF ( bc_dirichlet_n )  THEN
...
! Description:
!--------------------------------------------------------------------------------------------------!
!> Initialize mesoscale offline nesting for all modules that do not alter the pressure solution.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE nesting_offl_init_modules

    IF ( salsa )  CALL salsa_nesting_offl_init

 END SUBROUTINE nesting_offl_init_modules
