...
!-- ToDo: move all of them to respective module or a dedicated central module
    ...
    USE surface_data_handling,                                                                     &
        ONLY:  init_surface_grid_information
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_check_parameters
    ...
    CALL netcdf_check_parameters
    CALL tcm_check_parameters

    IF ( bulk_cloud_model )     CALL bcm_check_parameters
    IF ( air_chemistry )        CALL chem_check_parameters
    IF ( det_enabled )          CALL det_check_parameters
    IF ( cut_cell_topography )  CALL cct_check_parameters
    IF ( dmp_enabled )          CALL dmp_check_parameters
    IF ( fastv8_coupler_enabled )  CALL f8c_check_parameters
    IF ( fct_enabled )          CALL fct_check_parameters
...
!-- Note that the call order here must fit with the call order in module_interface_statistics
    ...
    IF ( det_enabled )       CALL det_check_data_output_ts( dots_max, dots_num, dots_label, dots_unit )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_check_data_output_pr( variable, var_count, unit, dopr_unit )
    ...
    IF ( unit == 'illegal'  .AND.  det_enabled )  THEN
       CALL det_check_data_output_pr( variable, var_count, unit, dopr_unit )
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_check_data_output( variable, unit, i, j, ilen, k )
    ...
    IF ( unit == 'illegal'  .AND.  biometeorology )  THEN
       CALL bio_check_data_output( variable, unit, j )
    ENDIF
    ...
    IF ( unit == 'illegal' .AND.  det_enabled ) THEN
       CALL det_check_data_output( variable, unit )
    ENDIF
    ...
    IF ( unit == 'illegal'  .AND.  slurb )  THEN
      CALL slurb_check_data_output( variable, unit )
   ENDIF
    ...
    IF ( unit == 'illegal'  .AND.  uv_radiation )  THEN
       unit = ''
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_check_data_output_surf( trimvar, unit, av )
    ...
    IF ( uv_radiation )  CALL uv_radiation_check_data_output_surf( trimvar, unit, av )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_init_arrays
    ...
    IF ( det_enabled            )  CALL det_init_arrays
    IF ( dmp_enabled            )  CALL dmp_init_arrays
    IF ( fastv8_coupler_enabled )  CALL f8c_init_arrays
    IF ( fct_enabled            )  CALL fct_init_arrays
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_init_after_pressure_solver
    ...
    IF ( uv_radiation        )  CALL uv_radiation_init
...
!
!-- Initialize surface grid information, e.g. count the number of surface elements,
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_header( io )
    ...
    IF ( det_enabled         )  CALL det_header( io )
    IF ( dmp_enabled         )  CALL dmp_header( io )
    IF ( fastv8_coupler_enabled )  CALL f8c_header ( io )
    IF ( fct_enabled         )  CALL fct_header( io )
    ...
    IF ( uv_radiation        )  CALL uv_radiation_header( io )
    IF ( wind_turbine        )  CALL wtm_header( io )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_actions( location )
    ...
    IF ( det_enabled            )  CALL det_actions( location )
    IF ( dmp_enabled            )  CALL dmp_actions( location )
    IF ( fastv8_coupler_enabled )  CALL f8c_actions( location )
    IF ( fct_enabled            )  CALL fct_actions( location )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_actions_ij( i, j, location )
    ...
    IF ( det_enabled            )  CALL det_actions( i, j, location )
    IF ( dmp_enabled            )  CALL dmp_actions( i, j, location )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_non_advective_processes
    ...
    IF ( det_enabled      )  CALL det_non_advective_processes
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_non_advective_processes_ij( i, j )
    ...
    IF ( det_enabled      )  CALL det_non_advective_processes( i, j )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_exchange_horiz( location )
    ...
    IF ( det_enabled           )  CALL det_exchange_horiz( location )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_prognostic_equations
    ...
    IF ( det_enabled         )  CALL det_prognostic_equations
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_prognostic_equations_ij( i, j, i_omp_start, tn )
    ...
    IF ( det_enabled         )  CALL det_prognostic_equations( i, j, i_omp_start, tn )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_boundary_conditions
    ...
    IF ( det_enabled           )  CALL det_boundary_conditions
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_swap_timelevel ( swap_mode )
    ...
    IF ( det_enabled           )  CALL det_swap_timelevel( swap_mode )
    ...
    IF ( slurb                 )  CALL slurb_swap_timelevel( swap_mode )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_3d_data_averaging( mode, variable )
    ...
    IF ( det_enabled           )  CALL det_3d_data_averaging( mode, variable )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_data_output_2d( av, variable, found, grid, mode, local_pf, two_d,     &
    ...
    IF ( .NOT. found  .AND.  det_enabled )  THEN
       CALL det_data_output_2d( av, variable, found, grid, mode, local_pf, two_d, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found )  THEN
       CALL doq_output_2d( av, variable, found, grid, mode, local_pf, two_d, flag_nr, nzb_do,      &
                           nzt_do )
    ENDIF
    ...
    IF ( .NOT. found  .AND.  slurb )  THEN
       CALL slurb_data_output_2d( av, variable, found, grid, mode, local_pf, two_d, nzb_do, nzt_do )
    ENDIF
    ...
    IF ( .NOT. found  .AND.  uv_radiation )  THEN
       CALL uv_radiation_data_output_2d( av, variable, found, grid, local_pf, two_d, nzb_do, nzt_do )
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_data_output_3d( av, variable, found, local_pf, resorted, flag_nr,     &
                                             mask_topography, nzb_do, nzt_do )
    ...
    CALL dynamics_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )

    IF ( .NOT. found )  THEN
       CALL tcm_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  biometeorology )  THEN
       CALL bio_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  bulk_cloud_model )  THEN
       CALL bcm_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  air_chemistry )  THEN
       CALL chem_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  dcep )  THEN
       CALL dcep_data_output_3d( av, variable, found, local_pf, resorted, mask_topography, nzb_do, &
                                 nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  det_enabled )  THEN
       CALL det_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found )  THEN
       CALL doq_output_3d( av, variable, found, local_pf, resorted, flag_nr, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  fastv8_coupler_enabled )  THEN
       CALL f8c_data_output_3d( av, variable, found, local_pf, resorted, flag_nr, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  gust_module_enabled )  THEN
       CALL gust_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  indoor_model )  THEN
       CALL im_data_output_3d( av, variable, found, local_pf, resorted, mask_topography, nzb_do,   &
                               nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  ocean_mode )  THEN
       CALL ocean_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  plant_canopy )  THEN
       CALL pcm_data_output_3d( av, variable, found, local_pf, resorted, mask_topography, nzb_do,  &
                                nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  radiation )  THEN
       CALL radiation_data_output_3d( av, variable, found, local_pf, resorted, mask_topography,    &
                                      nzb_do, nzt_do )
    ENDIF

    IF ( .NOT. found  .AND.  salsa )  THEN
       CALL salsa_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_data_output_surf( av, trimvar, found )
    ...
    IF ( uv_radiation )  CALL uv_radiation_data_output_surf( av, trimvar, found )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_statistics( mode, sr, tn, dots_max )
    ...
    IF ( det_enabled           )  CALL det_statistics( mode, sr, tn )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_rrd_global_ftn( found )
    ...
    IF ( .NOT. found )  CALL det_rrd_global( found ) ! ToDo: change interface to pass variable
    IF ( .NOT. found )  CALL dmp_rrd_global( found ) ! ToDo: change interface to pass variable
    IF ( .NOT. found )  CALL fct_rrd_global( found ) ! ToDo: change interface to pass variable
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_rrd_global_mpi
    ...
    IF ( det_enabled )          CALL det_rrd_global
    IF ( dmp_enabled )          CALL dmp_rrd_global
    IF ( fct_enabled )          CALL fct_rrd_global
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_wrd_global
    ...
    IF ( det_enabled )          CALL det_wrd_global
    IF ( dmp_enabled )          CALL dmp_wrd_global
    IF ( fct_enabled )          CALL fct_wrd_global
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_rrd_local_ftn( map_index,                                             &
    ...
    IF ( .NOT. found )  CALL det_rrd_local( map_index,                                             &
                                            nxlf, nxlc, nxl_on_file,                               &
                                            nxrf, nxrc, nxr_on_file,                               &
                                            nynf, nync, nyn_on_file,                               &
                                            nysf, nysc, nys_on_file,                               &
    ...
    IF ( .NOT. found )  CALL slurb_rrd_local( map_index,                                           &
                                              nxlf, nxlc, nxl_on_file,                             &
                                              nxrf, nxrc, nxr_on_file,                             &
                                              nynf, nync, nyn_on_file,                             &
                                              nysf, nysc, nys_on_file,                             &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_rrd_local_mpi
    ...
    IF ( det_enabled )          CALL det_rrd_local
    CALL doq_rrd_local
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE module_interface_wrd_local
    ...
    IF ( det_enabled )          CALL det_wrd_local
    CALL doq_wrd_local
