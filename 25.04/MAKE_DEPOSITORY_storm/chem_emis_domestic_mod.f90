...
!-- Transfer data from static file.
    ...
    CALL get_variable( ncid, 'stack_building_volume', stack_volume, nxl, nxr, nys, nyn, nbgp=0 )
    CALL get_variable( ncid, 'buildings_2d',  building_height, nxl, nxr, nys, nyn, nbgp=0 )
    CALL get_variable( ncid, 'building_type', building_type,   nxl, nxr, nys, nyn , nbgp=0 )
    CALL close_input_file( ncid )
