...
!--------------------------------------------------------------------------------------------------!
 MODULE synthetic_turbulence_generator_mod
    ...
    LOGICAL ::  zi_warning_given              = .FALSE.  !< flag to check if warning concerning zi has been already given
...
!--    parametrized. In the latter case, a prescribed boundary-layer depth > 0 is required.
       IF ( neutral )  THEN
          INQUIRE( FILE = 'STG_PROFILES' // TRIM( coupling_char ), EXIST = file_stg_exist  )
          IF ( .NOT. file_stg_exist  .AND.  boundary_layer_depth <= 0.0_wp  .OR.                   &
                                            boundary_layer_depth >= zw(nzt-5) )  THEN
             WRITE ( message_string, * )  'boundary_layer_depth = ', boundary_layer_depth,         &
                                          ' <= 0.0 or almost approaches the domain top'
...
!
!--    Check if zi exceeds or is in the range of the domain top level. This case, give a warning as
!--    this can lead to blow-ups of the velocity perturbations.
       IF ( zi >= zw(nzt-5)  .AND.  .NOT. zi_warning_given )  THEN
          WRITE ( message_string, * )  'boundary layer depth exceeds the model top'
          CALL message( 'stg_adjust', 'STG0012', 0, 1, 0, 6, 0 )
          zi_warning_given = .TRUE.
       ENDIF
    ENDIF
!
...
!-- model domain. Only average over upward facing surfaces.
    ...
    DO  m = 1, surf_def%ns
       IF ( surf_def%upward(m) )  THEN
          ...
          shf_mean_l     = shf_mean_l      + surf_def%shf(m) * drho_air_zw(k)
    ...
    DO  m = 1, surf_lsm%ns
       IF ( surf_lsm%upward(m) )  THEN
          ...
          shf_mean_l     = shf_mean_l      + surf_lsm%shf_agg(m) * drho_air_zw(k)
    ...
    DO  m = 1, surf_usm%ns
       IF ( surf_usm%upward(m) )  THEN
          ...
          shf_mean_l     = shf_mean_l      + surf_usm%shf(m) * drho_air_zw(k)
