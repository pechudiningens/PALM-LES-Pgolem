...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE init_grid
    ...
    LOGICAL ::  bc_cs_b_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_cs_t_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_pt_b_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_pt_t_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_q_b_set_internally  = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_q_t_set_internally  = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_s_b_set_internally  = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_s_t_set_internally  = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_uv_b_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_uv_t_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
...
!--       Bottom condition for elevated (shifted) childs. Then the bottom boundary is "open".
          IF ( nest_shift_z /= 0.0_wp )  THEN
             ...
             bc_s_b  = 'nested'
             bc_uv_b = 'nested'
             bc_cs_b_set_internally = .TRUE.
             bc_pt_b_set_internally = .TRUE.
             bc_q_b_set_internally  = .TRUE.
             bc_s_b_set_internally  = .TRUE.
             bc_uv_b_set_internally = .TRUE.
          ENDIF
!
...
!> TODO:  Should be adjusted for the case if the top of the child matches the top of the root.
          ...
          bc_s_t   = 'nested'
          bc_uv_t  = 'nested'
          bc_cs_t_set_internally = .TRUE.
          bc_pt_t_set_internally = .TRUE.
          bc_q_t_set_internally  = .TRUE.
          bc_s_t_set_internally  = .TRUE.
          bc_uv_t_set_internally = .TRUE.
...
!-- Check if nested boundary conditions have been set for non-allowed cases.
    IF ( bc_cs_b == 'nested'  .AND.  .NOT. bc_cs_b_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_cs_b ="' // TRIM( bc_cs_b ) // '"'
       CALL message( 'init_grid', 'CHM0007', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_cs_t == 'nested'  .AND.  .NOT. bc_cs_t_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_cs_t ="' // TRIM( bc_cs_t ) // '"'
       CALL message( 'init_grid', 'CHM0007', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_pt_b == 'nested'  .AND.  .NOT. bc_pt_b_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_pt_b = "' // TRIM( bc_pt_b ) // '"'
       CALL message( 'init_grid', 'PAC0082', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_pt_t == 'nested'  .AND.  .NOT. bc_pt_t_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_pt_t = "' // TRIM( bc_pt_t ) // '"'
       CALL message( 'init_grid', 'PAC0083', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_q_b == 'nested'  .AND.  .NOT. bc_q_b_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_q_b = "' // TRIM( bc_q_b ) // '"'
       CALL message( 'init_grid', 'PAC0092', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_q_t == 'nested'  .AND.  .NOT. bc_q_t_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_q_t = "' // TRIM( bc_q_t ) // '"'
       CALL message( 'init_grid', 'PAC0093', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_s_b == 'nested'  .AND.  .NOT. bc_s_b_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_s_b = "' // TRIM( bc_s_b ) // '"'
       CALL message( 'init_grid', 'PAC0092', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_s_t == 'nested'  .AND.  .NOT. bc_s_t_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_s_t = "' // TRIM( bc_s_t ) // '"'
       CALL message( 'init_grid', 'PAC0093', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_uv_b == 'nested'  .AND.  .NOT. bc_uv_b_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_uv_b = "' // TRIM( bc_uv_b ) // '"'
       CALL message( 'init_grid', 'PAC0099', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_uv_t == 'nested'  .AND.  .NOT. bc_uv_t_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_uv_t = "' // TRIM( bc_uv_t ) // '"'
       CALL message( 'init_grid', 'PAC0100', 1, 2, 0, 6, 0 )
    ENDIF
