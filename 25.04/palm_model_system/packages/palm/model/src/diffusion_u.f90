...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE diffusion_u
       ...
       !$ACC PRESENT(tend) &
       !$ACC PRESENT(surf_u, surf_u%southward, surf_u%northward, surf_u%downward, surf_u%upward) &
       !$ACC PRESENT(surf_u%start_index, surf_u%end_index, surf_u%k, surf_u%usvs, surf_u%usws) &
...
!--          For the u-component tendency, u'v' is added at
...
!--             Cut-cell branch.
                ELSE
                   ...
                   surf_s = surf_u%start_index(j,i)
                   surf_e = surf_u%end_index(j,i)
                   DO  m = surf_s, surf_e
                      k         = surf_u%k(m)

                      flux_n(k) = MERGE( surf_u%usvs(m), flux_n(k), surf_u%southward(m) )
                      flux_s(k) = MERGE( surf_u%usvs(m), flux_s(k), surf_u%northward(m) )
                      flux_t(k) = MERGE( surf_u%usws(m), flux_t(k), surf_u%downward(m)  )
                      flux_d(k) = MERGE( surf_u%usws(m), flux_d(k), surf_u%upward(m)    )
                   ENDDO
                ENDIF
             ENDIF
!
