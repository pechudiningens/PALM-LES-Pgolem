...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE count_output_particles( pcount, nr_prts_in_gridbox )
    ...
    INTEGER(iwp) ::  k       !<
    INTEGER(iwp) ::  n       !<
    INTEGER(idp) ::  n_all   !< count all particles for MOD function
...
!-- Decode the particle id in i,j,k,n.
    ...
    IF ( nr_out_prts == 0 )  THEN
       message_string = 'no valid IDs found in particle ID file ..._prtids'
       CALL message( 'dop_mark_ids_scheduled_for_output_via_file', 'LPM0036', 3, 2, 0, 6, 0 )
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE dop_mark_ids_scheduled_for_output
    ...
    INTEGER(iwp) ::  k                    !<
    INTEGER(iwp) ::  n                    !<
    INTEGER(idp) ::  n_all                !< particle counter for MOD function
