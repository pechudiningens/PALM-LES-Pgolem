...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmcp_g_init
    ...
    LOGICAL ::  bc_par_b_set_internally  = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_par_lr_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_par_ns_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_par_t_set_internally  = .FALSE.  !< flag to indicate if boundary condition has been set internally
...
!-- Check if the current model has child models.
    ...
    ENDIF
!
!-- Set the boundary conditions to nested for childs.
    IF ( .NOT. root_model )  THEN

       IF ( particle_coupling )  THEN

!
!--       Elevated atmospheric childs have on open boundary at the bottom. Ocean childs are always
!--       open at the bottom.
          IF ( ( nest_shift_z > 0.0  .OR.  ocean_mode ) )  THEN
             bc_par_b = 'nested'
             bc_par_b_set_internally = .TRUE.
             ibc_par_b = 3
          ENDIF

          IF ( .NOT. ocean_mode )  THEN
             bc_par_t = 'nested'
             bc_par_t_set_internally = .TRUE.
             ibc_par_t  = 3
          ENDIF

          IF ( .NOT. ( nesting_bounds == 'vertical_only'  .OR.                                     &
                       nesting_bounds == 'cyclic_along_x' ) )                                      &
          THEN
             bc_par_lr = 'nested'
             bc_par_lr_set_internally = .TRUE.
             ibc_par_lr = 3
          ENDIF
          IF ( .NOT. ( nesting_bounds == 'vertical_only'  .OR.                                     &
                       nesting_bounds == 'cyclic_along_y' ) )                                      &
          THEN
             bc_par_ns = 'nested'
             bc_par_ns_set_internally = .TRUE.
             ibc_par_ns = 3
          ENDIF
...
!-- Check if nested boundary conditions have been set for non-allowed cases.
    IF ( bc_par_b == 'nested'  .AND.  .NOT. bc_par_b_set_internally )  THEN
       WRITE( message_string, * )  'unknown boundary condition ',                                  &
                                   'bc_par_b = "', TRIM( bc_par_b ), '"'
       CALL message( 'pmcp_g_init', 'LPM0009', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_par_t == 'nested'  .AND.  .NOT. bc_par_t_set_internally )  THEN
       WRITE( message_string, * )  'unknown boundary condition ',                                  &
                                   'bc_par_t = "', TRIM( bc_par_t ), '"'
       CALL message( 'pmcp_g_init', 'LPM0010', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_par_lr == 'nested'  .AND.  .NOT. bc_par_lr_set_internally )  THEN
       WRITE( message_string, * )  'unknown boundary condition ',                                  &
                                   'bc_par_lr = "', TRIM( bc_par_lr ), '"'
       CALL message( 'pmcp_g_init', 'LPM0011', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_par_ns == 'nested'  .AND.  .NOT. bc_par_ns_set_internally )  THEN
       WRITE( message_string, * )  'unknown boundary condition ',                                  &
                                   'bc_par_ns = "', TRIM( bc_par_ns ), '"'
       CALL message( 'pmcp_g_init', 'LPM0012', 1, 2, 0, 6, 0 )
    ENDIF
!
!-- Data transfer via ISEND/IRECV not realized for the LPM.
    IF ( particle_coupling )  THEN
       IF ( .NOT. use_one_sided_communication )  THEN
          message_string = 'setting of use_one_sided_communication = .F. not allowed'
          CALL message( 'pmcp_g_init', 'PMC0038', 1, 2, 0, 6, 0 )
       ENDIF
       IF ( wp == sp )  THEN
          message_string = 'illegal working precision for using particle coupling'
          CALL message( 'pmcp_g_init', 'PMC0039', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
...
!--    pmc_c_getbuffer and pmc_p_fillbuffer on the parent side.
          ...
          DO  ij = 1, ape%nrele
              ...
              IF ( nr > 0 )  THEN
                 ...
                 CALL MPI_GET( coarse_particles(j,i)%parent_particles, nr * parsize, MPI_BYTE,     &
                               ip-1, target_disp, nr * parsize, MPI_BYTE, particle_win_child, ierr )
 ...
 SUBROUTINE pmcp_p_fill_particle_win
    ...
    DO  m = 1, get_number_of_children()
       ...
       IF ( lfirst  .AND.  debug_output )  THEN
          WRITE( debug_string, '(A,5(1X,F10.2))' )                                                 &
...
!--                to be transferred to the parent.
                   ...
                   ELSE
                      do_transfer = ( ( x > lx_coord  .AND.  x < rx_coord )  .AND.                 &
                                      ( y > sy_coord  .AND.  y < ny_coord )  .AND.                 &
                                      ( z > child_nest_shift_z+0.000000001  .AND.  z < uz_coord )  &
                                    )
                   ENDIF
...
!--                   Change particle positions and origin relative to global origin.
       ...
       CALL pmc_p_fillbuffer( child_id, particle_transfer = .TRUE. )
...
!--    Therefore p_copy_particle_to_org_grid is done for one child process per call.
       DO  ip = 1, pmc_p_get_child_npes( child_id )

          nr_part  = 0
          part_adr = 0

          CALL pmc_p_getbuffer( child_id, particle_transfer = .TRUE., child_process_nr = ip )
...
! Description:
! ------------
!> After the transfer mark all parent particles that are still inside one of the child areas for
       ...
       DO  jc = jps, jpn
          ...
          IF ( nr > 0 )  THEN
             DO  n = 1, nr
                ...
                jp = yc / dy
                IF ( ocean_mode )  THEN
                   kp = INT( ( zc - zw(nzb) ) / ( zw(nzb+1) - zw(nzb) ) ) + 1
                ELSE
                   kp = INT( ( zc - nest_shift_z ) / ( zw(nzb+1) - zw(nzb) ) ) + 1
                ENDIF
          ...
          DO  k = nzb + 1, nzt
             ...
             DO  n = 1, number_of_particles
                IF ( particles(n)%particle_mask )  THEN
                   ...
                   IF ( ocean_mode )  THEN
                      boundary_particle = ( boundary_particle  .OR.  z <= zw(nzb) )
                   ELSE
                      boundary_particle = ( boundary_particle .OR.                                 &
                                            ( z < nest_shift_z .OR. z > zw(nzt) ) )
                   ENDIF
