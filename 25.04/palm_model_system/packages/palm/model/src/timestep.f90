...
!--       Consider each velocity component individually
          ...
          !$ACC PRESENT(u, v, w, dzu) DEFAULT(NONE) IF(enable_openacc)
...
!--       !>       workaround so far. This has to be changed in the future.
          ...
          !$ACC PRESENT(u, v, w, dzu) IF(enable_openacc)
          !$OMP PARALLEL DO PRIVATE(i,j,k) &
...
!--    subgrid model, negative km are possible.
       ...
       !$ACC PARALLEL LOOP PRESENT(dxyz2_min, dzw) DEFAULT(NONE) IF(enable_openacc)
       DO  k = nzb+1, nzt
       ...
       !$ACC PRESENT(dxyz2_min, kh, km) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = nzb+1, nzt
...
!
!--    Advection.
       ELSEIF ( MIN( dt_u, dt_v, dt_w ) < MIN( dt_diff, dt_precipitation, dt_lsm, dt_usm,          &
                     dt_slurb ) )                                                                  &
       THEN
          timestep_reason = 'A'
!
!--    Diffusion.
       ELSEIF ( dt_diff < MIN( dt_u, dt_v, dt_w, dt_precipitation, dt_lsm, dt_usm, dt_slurb ) )    &
       THEN
          timestep_reason = 'D'
!
!--    Surface energy balance models.
       ELSEIF ( MIN( dt_lsm, dt_usm, dt_slurb ) < MIN( dt_u, dt_v, dt_w, dt_precipitation,         &
                dt_diff ) )                                                                        &
       THEN
          timestep_reason = 'S'
!
!--    Precipitation.
       ELSEIF ( bulk_cloud_model  .AND.  dt_precipitation < MIN( dt_u, dt_v, dt_w, dt_diff,        &
                dt_lsm, dt_usm, dt_slurb ) )                                                       &
       THEN
          timestep_reason = 'P'
       ENDIF
!
