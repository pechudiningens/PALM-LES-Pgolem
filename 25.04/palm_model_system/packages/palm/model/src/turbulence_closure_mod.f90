...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE tcm_boundary_conds
    ...
    REAL(wp) ::  limit_value  !< absolute minimum value for dissipation
...
!--    Note, only TKE is prognostic in this case and dissipation is only a diagnostic quantity.
       IF ( .NOT. rans_mode )  THEN
          ...
          !$ACC PRESENT(bc_hv, e_p) DEFAULT(NONE) IF(enable_openacc)
          DO  m = 1, bc_hv%ns
             i = bc_hv%i(m)
             j = bc_hv%j(m)
             k = bc_hv%k(m)
...
!--       Urban surfaces
       ...
       ENDIF
!
!--    In ocean setups, a Neumann BC for the SGS-TKE is always assumed at the bottom boundary,
!--    independent if there is an open boundary or a surface.
       IF ( ocean_mode )  e_p(nzb,:,:) = e_p(nzb+1,:,:)
!
!--    Set Neumann boundary condition for TKE at model top. Do this also in case of a nested run.
       !$ACC KERNELS PRESENT(diss_p,e_p) DEFAULT(NONE) IF(enable_openacc)
       e_p(nzt+1,:,:) = e_p(nzt,:,:)
...
!-- Wall boundary conditions for TKE dissipation rate in RANS mode.
    IF ( rans_tke_e )  THEN
!
!--    In ocean setups, a Neumann BC for the dissipation is always assumed at the bottom boundary,
!--    independent if there is an open boundary or a surface. At surfaces, the boundary value
!--    might be overwritten later on.
       IF ( ocean_mode )  diss_p(nzb,:,:) = diss_p(nzb+1,:,:)
!
!--    Use wall function within constant-flux layer
...
!--    The minimum value is different for atmosphere and ocean simulations.
       limit_value = MERGE( 0.0001_wp, 10E-15_wp, .NOT. ocean_mode )
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = nzb, nzt+1
                diss_p(k,j,i) = MAX( MIN( diss_p(k,j,i), 2.0_wp * diss(k,j,i) ),                   &
                                     0.1_wp * diss(k,j,i), limit_value )
             ENDDO
          ENDDO
       ENDDO
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE tcm_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
...
!--    Calculate mixing length according to Blackadar (1962).
       ...
       ENDIF
!
!--    WARNING: Formally, the computation also works for ocean mode (because atmos_ocean_sign is
!--    included), but the calculated mixing length will not be suitable in the ocean!
       DO  k = nzb, nzt
          ml_blackadar(k) = kappa * atmos_ocean_sign * zu(k) /                                     &
                            ( 1.0_wp + kappa * atmos_ocean_sign * zu(k) / length_scale_max )
...
!--                         Reduce search along x by already found distance
       ...
       !$ACC ENTER DATA COPYIN(ml_blackadar(nzb:nzt+1)) IF(enable_openacc)

    ENDIF  !LES or RANS mode
    !$ACC ENTER DATA COPYIN(delta(nzb:nzt+1,nys:nyn,nxl:nxr), distance_to_wall) IF(enable_openacc)
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE production_e_init
    ...
    !$ACC DATA CREATE( phi_m_def, phi_m_lsm, phi_m_usm ) IF(enable_openacc)
...
!--    phi_m is used within the MERGE construct.
       ...
       !$ACC PRESENT(surf_def) DEFAULT(NONE) IF(enable_openacc)
       DO  m = 1, surf_def%ns
          phi_m_def(m) = phi_m( surf_def%z_mo(m) / surf_def%ol(m) )
       ENDDO
       !$OMP PARALLEL DO PRIVATE(m)
       !$ACC PARALLEL LOOP PRIVATE(m) &
       !$ACC PRESENT(surf_lsm) DEFAULT(NONE) IF(enable_openacc)
       DO  m = 1, surf_lsm%ns
          phi_m_lsm(m) = phi_m( surf_lsm%z_mo(m) / surf_lsm%ol(m) )
       ENDDO
       !$OMP PARALLEL DO PRIVATE(m)
       !$ACC PARALLEL LOOP PRIVATE(m) &
       !$ACC PRESENT(surf_usm) DEFAULT(NONE) IF(enable_openacc)
       DO  m = 1, surf_usm%ns
...
!--    Default surfaces.
       ...
       !$ACC PRESENT(surf_def, u, v, drho_air_zw, zu) PRESENT(km) DEFAULT(NONE) IF(enable_openacc)
       DO  m = 1, surf_def%ns
...
!--       Multiply with zero for vertical surfaces (offset in z is zero this case).
          surf_def%u_0(m) = u(kref,j,i) + surf_def%usws(m) * SIGN( 1.0_wp, surf_def%n_eff(m) )     &
                          * drho * ( zu(k+1) - zu(k-1) ) / km_sfc * flag
          surf_def%v_0(m) = v(kref,j,i) + surf_def%vsws(m) * SIGN( 1.0_wp, surf_def%n_eff(m) )     &
                          * drho * ( zu(k+1) - zu(k-1) ) / km_sfc * flag
!
!--       upward: |u(k+1) - u_0|, downward: |u_0 - u(k-1)|
          IF ( ABS( ( u(kref,j,i) - surf_def%u_0(m) ) * SIGN( 1.0_wp, surf_def%n_eff(m) ) ) >      &
               ABS( u(k+1,j,i) - u(k-1,j,i) ) )                                                    &
          THEN
             surf_def%u_0(m) = u(k+koff,j,i)
          ENDIF

          IF ( ABS( ( v(kref,j,i) - surf_def%v_0(m) ) * SIGN( 1.0_wp, surf_def%n_eff(m) ) ) >      &
...
!--    Natural surfaces.
       !$OMP PARALLEL DO PRIVATE(i, j, k, m, drho, km_sfc, koff, kref, flag)
       !$ACC PARALLEL LOOP PRIVATE(i, j, k, m, drho, km_sfc, koff, kref, flag) &
       !$ACC PRESENT(surf_lsm, u, v, drho_air_zw, zu) PRESENT(km) DEFAULT(NONE) IF(enable_openacc)
       DO  m = 1, surf_lsm%ns
...
!--       for downward-facing surfaces this is: u(k-1) - usws * drho * 2 * dz / km.
!--       Multiply with zero for vertical surfaces (offset in z is zero this case).
          surf_lsm%u_0(m) = u(kref,j,i) + surf_lsm%usws(m) * SIGN( 1.0_wp, surf_lsm%n_eff(m) )     &
                          * drho * ( zu(k+1) - zu(k-1) ) / km_sfc * flag
          surf_lsm%v_0(m) = v(kref,j,i) + surf_lsm%vsws(m) * SIGN( 1.0_wp, surf_lsm%n_eff(m) )     &
                          * drho * ( zu(k+1) - zu(k-1) ) / km_sfc * flag
!
!--       upward: |u(k+1) - u_0|, downward: |u_0 - u(k-1)|
          IF ( ABS( ( u(kref,j,i) - surf_lsm%u_0(m) ) * SIGN( 1.0_wp, surf_lsm%n_eff(m) ) ) >      &
               ABS( u(k+1,j,i) - u(k-1,j,i) ) )                                                    &
          THEN
             surf_lsm%u_0(m) = u(k+koff,j,i)
          ENDIF

          IF ( ABS( ( v(kref,j,i) - surf_lsm%v_0(m) ) * SIGN( 1.0_wp, surf_lsm%n_eff(m) ) ) >      &
               ABS( v(k+1,j,i) - v(k-1,j,i) ) )                                                    &
          THEN
...
!--    Urban surfaces
       !$OMP PARALLEL DO PRIVATE(i, j, k, m, drho, km_sfc, koff, kref, flag)
       !$ACC PARALLEL LOOP PRIVATE(i, j, k, m, drho, km_sfc, koff, kref, flag) &
       !$ACC PRESENT(surf_usm, u, v, drho_air_zw, zu) PRESENT(km) DEFAULT(NONE) IF(enable_openacc)
       DO  m = 1, surf_usm%ns
...
!--       for downward-facing surfaces this is: u(k-1) - usws * drho * 2 * dz / km.
!--       Multiply with zero for vertical surfaces (offset in z is zero this case).
          surf_usm%u_0(m) = u(kref,j,i) + surf_usm%usws(m) * SIGN( 1.0_wp, surf_usm%n_eff(m) )     &
                          * drho * ( zu(k+1) - zu(k-1) ) / km_sfc * flag
          surf_usm%v_0(m) = v(kref,j,i) + surf_usm%vsws(m) * SIGN( 1.0_wp, surf_usm%n_eff(m) )     &
                          * drho * ( zu(k+1) - zu(k-1) ) / km_sfc * flag
!
!--       upward: |u(k+1) - u_0|, downward: |u_0 - u(k-1)|
          IF ( ABS( ( u(kref,j,i) - surf_usm%u_0(m) ) * SIGN( 1.0_wp, surf_usm%n_eff(m) ) ) >      &
               ABS( u(k+1,j,i) - u(k-1,j,i) ) )                                                    &
          THEN
             surf_usm%u_0(m) = u(k+koff,j,i)
          ENDIF

          IF ( ABS( ( v(kref,j,i) - surf_usm%v_0(m) ) * SIGN( 1.0_wp, surf_usm%n_eff(m) ) ) >      &
               ABS( v(k+1,j,i) - v(k-1,j,i) ) )                                                    &
          THEN
...
!-- If required, compute prognostic equation for turbulent kinetic energy (TKE)
    IF ( .NOT. constant_diffusion )  THEN
       ...
       CALL cpu_log( log_point(77), 'tke-equation', 'start' )
...
!--    TKE-tendency terms with no communication
       IF ( scalar_advec /= 'bc-scheme'  .OR.  use_upstream_for_tke )  THEN
          IF ( use_upstream_for_tke )  THEN
             ...
             CALL advec_s_up( e )
          ELSE
             !$ACC KERNELS PRESENT(tend) DEFAULT(NONE) IF(enable_openacc)
             tend = 0.0_wp
             !$ACC END KERNELS
             IF ( timestep_scheme(1:5) == 'runge' )  THEN
...
!--    integration. In such cases the old TKE value is reduced by 90%.
       ...
       !$ACC PRESENT(tsc(3:3)) &
       !$ACC PRESENT(e_p) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
...
!--    Calculate tendencies for the next Runge-Kutta step
       IF ( timestep_scheme(1:5) == 'runge' )  THEN
          IF ( intermediate_timestep_count == 1 )  THEN
             !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
             !$ACC PRESENT(tend, te_m) DEFAULT(NONE) IF(enable_openacc)
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb+1, nzt
          ...
          ELSEIF ( intermediate_timestep_count < intermediate_timestep_count_max )  THEN
             !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
             !$ACC PRESENT(tend, te_m) IF(enable_openacc)
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb+1, nzt
       ...
       CALL cpu_log( log_point(77), 'tke-equation', 'stop' )
...
!-- If required, compute prognostic equation for TKE dissipation rate
    IF ( rans_tke_e )  THEN
       ...
       CALL cpu_log( log_point(78), 'diss-equation', 'start' )
...
!--    Calculate tendencies for the next Runge-Kutta step
       ...
       CALL cpu_log( log_point(78), 'diss-equation', 'stop' )
...
!--             Fluxes used for east/west-facing surfaces (offset index in x is non-zero)
                ...
                km_neutral = kappa * SQRT( SQRT( usvs**2 + wsvs**2 + vsus**2 + wsus**2 ) ) *       &
                             surf_def%z_mo(m)
!
...
!--             wall type. The effect of this missing interpolation is negligible.
                dudz(k) = MERGE( SIGN( 1.0_wp, surf_def%n_eff(m) )                                 &
                                 * ( u(kref,j,i) - surf_def%u_0(m) ) * dd2zu(k),                   &
                                 dudz(k),                                                          &
                                 surf_def%upward(m)  .OR.  surf_def%downward(m) )
                dvdz(k) = MERGE( SIGN( 1.0_wp, surf_def%n_eff(m) )                                 &
                                 * ( v(kref,j,i) - surf_def%v_0(m) ) * dd2zu(k),                   &
                                 dvdz(k),                                                          &
                                 surf_def%upward(m)  .OR.  surf_def%downward(m) )
             ENDDO
!
...
!--             to maintain the already calculated gradient if it is not a north/south facing
!--             surface but a different one.
                dudy(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * usvs                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
                                 dudy(k), surf_lsm%northward(m)  .OR.  surf_lsm%southward(m) )
                dwdy(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * wsvs                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
                                 dwdy(k), surf_lsm%northward(m)  .OR.  surf_lsm%southward(m) )
!
!--             -1.0 for east-facing wall, 1.0 for west-facing wall
                dvdx(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * vsus                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
                                 dvdx(k), surf_lsm%eastward(m)  .OR.  surf_lsm%westward(m) )
                dwdx(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * wsus                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
...
!--             required. However, this would require several data transfers between 2D-grid and
!--             wall type. The effect of this missing interpolation is negligible.
                dudz(k) = MERGE( SIGN( 1.0_wp, surf_lsm%n_eff(m) )                                 &
                                 * ( u(kref,j,i) - surf_lsm%u_0(m) ) * dd2zu(k),                   &
                                 dudz(k), surf_lsm%upward(m)  .OR.  surf_lsm%downward(m) )
                dvdz(k) = MERGE( SIGN( 1.0_wp, surf_lsm%n_eff(m) )                                 &
                                 * ( v(kref,j,i) - surf_lsm%v_0(m) ) * dd2zu(k),                   &
...
!--             to maintain the already calculated gradient if it is not a north/south facing
!--             surface but a different one.
                dudy(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * usvs                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
                                 dudy(k), surf_usm%northward(m)  .OR.  surf_usm%southward(m) )
                dwdy(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * wsvs                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
                                 dwdy(k), surf_usm%northward(m)  .OR.  surf_usm%southward(m) )
!
!--             -1.0 for east-facing wall, 1.0 for west-facing wall
                dvdx(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * vsus                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
                                 dvdx(k), surf_usm%eastward(m)  .OR.  surf_usm%westward(m) )
                dwdx(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * wsus                         &
                                 / ( km_neutral + 1E-10_wp ),                                      &
...
!--             required. However, this would require several data transfers between 2D-grid and
!--             wall type. The effect of this missing interpolation is negligible.
                dudz(k) = MERGE( SIGN( 1.0_wp, surf_usm%n_eff(m) )                                 &
                                 * ( u(kref,j,i) - surf_usm%u_0(m) ) * dd2zu(k),                   &
                                 dudz(k), surf_usm%upward(m)  .OR.  surf_usm%downward(m) )
                dvdz(k) = MERGE( SIGN( 1.0_wp, surf_usm%n_eff(m) )                                 &
                                 * ( v(kref,j,i) - surf_usm%v_0(m) ) * dd2zu(k),                   &
...
!
!--                      Overwrite vertical flux only at horizontal surfaces.
                         tmp_flux(k) = MERGE( drho_air_zw(kref) * surf_lsm%shf_agg(m), tmp_flux(k),&
...
!--                   RANS mode: Compute tendency for dissipation-rate-production from shear
       ...
       ELSE ! Or IF ( humidity )  THEN
          ...
          !$ACC PRESENT(dd2zu, diss, drho_air_zw, d_exner, e, exner, kh, pt, q, ql) &
          !$ACC PRESENT(surf_def, surf_def%downward, surf_def%end_index, surf_def%k, surf_def%qsws) &
          !$ACC PRESENT(surf_def%shf, surf_def%start_index, surf_def%upward, surf_lsm, surf_lsm%downward) &
          !$ACC PRESENT(surf_lsm%end_index, surf_lsm%k, surf_lsm%qsws_agg, surf_lsm%shf_agg) &
          !$ACC PRESENT(surf_lsm%start_index, surf_lsm%upward, surf_top, surf_usm, surf_usm%end_index) &
          !$ACC PRESENT(surf_usm%k, surf_usm%start_index, surf_usm%upward, tend, topo_flags, vpt) &
          !$ACC PRIVATE(tmp_flux, surf_s, surf_e) &
          !$ACC DEFAULT(NONE) IF(enable_openacc)
          DO  i = nxl, nxr
             DO  j = nys, nyn

                !$ACC LOOP SEQ PRIVATE(k, k1, k2, theta, temp)
                DO  k = nzb+1, nzt
...
!--                Treat horizontal default surfaces
                   surf_s = surf_def%start_index(j,i)
                   surf_e = surf_def%end_index(j,i)
                   !$ACC LOOP SEQ PRIVATE(k, kref, k1, k2, theta, temp)
                   DO  m = surf_s, surf_e
                      k    = surf_def%k(m)
                      kref = MERGE( k-1, k, surf_def%upward(m) )
...
!
!--                   Overwrite vertical flux only at horizontal surfaces.
                      tmp_flux(k) = MERGE( ( k1 * surf_lsm%shf_agg(m) + k2 * surf_lsm%qsws_agg(m) )&
                                           * drho_air_zw(kref),                                    &
                                           tmp_flux(k),                                            &
                                           surf_lsm%upward(m)  .OR.  surf_lsm%downward(m) )
                   ENDDO
...
!--       to maintain the already calculated gradient if it is not a north/south facing
!--       surface but a different one.
          dudy(k) = MERGE( -SIGN( 1.0_wp, surf_def%n_eff(m) ) * usvs / ( km_neutral + 1E-10_wp ),  &
                           dudy(k), surf_def%northward(m)  .OR.  surf_def%southward(m) )
          dwdy(k) = MERGE( -SIGN( 1.0_wp, surf_def%n_eff(m) ) * wsvs / ( km_neutral + 1E-10_wp ),  &
                           dwdy(k), surf_def%northward(m)  .OR.  surf_def%southward(m) )
!
!--       x -1.0 for east-facing wall, x 1.0 for west-facing wall
          dvdx(k) = MERGE( -SIGN( 1.0_wp, surf_def%n_eff(m) ) * vsus / ( km_neutral + 1E-10_wp ),  &
                           dvdx(k), surf_def%eastward(m)  .OR.  surf_def%westward(m) )
          dwdx(k) = MERGE( -SIGN( 1.0_wp, surf_def%n_eff(m) ) * wsus / ( km_neutral + 1E-10_wp ),  &
                           dwdx(k), surf_def%eastward(m)  .OR.  surf_def%westward(m) )
...
!--       required. However, this would require several data transfers between 2D-grid and
!--       wall type. The effect of this missing interpolation is negligible.
          dudz(k) = MERGE( SIGN( 1.0_wp, surf_def%n_eff(m) ) * ( u(kref,j,i) - surf_def%u_0(m) )   &
                                                             * dd2zu(k),                           &
                           dudz(k), surf_def%upward(m)  .OR.  surf_def%downward(m) )
          dvdz(k) = MERGE( SIGN( 1.0_wp, surf_def%n_eff(m) ) * ( v(kref,j,i) - surf_def%v_0(m) )   &
                                                             * dd2zu(k),                           &
...
!--       to maintain the already calculated gradient if it is not a north/south facing
!--       surface but a different one.
          dudy(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * usvs / ( km_neutral + 1E-10_wp ),  &
                           dudy(k), surf_lsm%northward(m)  .OR.  surf_lsm%southward(m) )
          dwdy(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * wsvs / ( km_neutral + 1E-10_wp ),  &
                           dwdy(k), surf_lsm%northward(m)  .OR.  surf_lsm%southward(m) )
!
!--       x -1.0 for east-facing wall, x 1.0 for west-facing wall
          dvdx(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * vsus / ( km_neutral + 1E-10_wp ),  &
                           dvdx(k), surf_lsm%eastward(m)  .OR.  surf_lsm%westward(m) )
          dwdx(k) = MERGE( -SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * wsus / ( km_neutral + 1E-10_wp ),  &
                           dwdx(k), surf_lsm%eastward(m)  .OR.  surf_lsm%westward(m) )
!
!--       Now compute vertical gradients using virtual surface velocity (for further
!--       explanation please see comments in production_e_init).
...
!--       required. However, this would require several data transfers between 2D-grid and
!--       wall type. The effect of this missing interpolation is negligible.
          dudz(k) = MERGE( SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * ( u(kref,j,i) - surf_lsm%u_0(m) )   &
                                                             * dd2zu(k),                           &
                           dudz(k), surf_lsm%upward(m)  .OR.  surf_lsm%downward(m) )
          dvdz(k) = MERGE( SIGN( 1.0_wp, surf_lsm%n_eff(m) ) * ( v(kref,j,i) - surf_lsm%v_0(m) )   &
                                                             * dd2zu(k),                           &
                           dvdz(k), surf_lsm%upward(m)  .OR.  surf_lsm%downward(m) )
       ENDDO
!
...
!--       to maintain the already calculated gradient if it is not a north/south facing
!--       surface but a different one.
          dudy(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * usvs / ( km_neutral + 1E-10_wp ),  &
                           dudy(k), surf_usm%northward(m)  .OR.  surf_usm%southward(m) )
          dwdy(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * wsvs / ( km_neutral + 1E-10_wp ),  &
                           dwdy(k), surf_usm%northward(m)  .OR.  surf_usm%southward(m) )
!
!--       x -1.0 for east-facing wall, x 1.0 for west-facing wall
          dvdx(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * vsus / ( km_neutral + 1E-10_wp ),  &
                           dvdx(k), surf_usm%eastward(m)  .OR.  surf_usm%westward(m) )
          dwdx(k) = MERGE( -SIGN( 1.0_wp, surf_usm%n_eff(m) ) * wsus / ( km_neutral + 1E-10_wp ),  &
                           dwdx(k), surf_usm%eastward(m)  .OR.  surf_usm%westward(m) )
!
!--       Now compute vertical gradients using virtual surface velocity (for further
!--       explanation please see comments in production_e_init).
...
!--       required. However, this would require several data transfers between 2D-grid and
!--       wall type. The effect of this missing interpolation is negligible.
          dudz(k) = MERGE( SIGN( 1.0_wp, surf_usm%n_eff(m) ) * ( u(kref,j,i) - surf_usm%u_0(m) )   &
                                                             * dd2zu(k),                           &
                           dudz(k), surf_usm%upward(m)  .OR.  surf_usm%downward(m) )
          dvdz(k) = MERGE( SIGN( 1.0_wp, surf_usm%n_eff(m) ) * ( v(kref,j,i) - surf_usm%v_0(m) )   &
                                                             * dd2zu(k),                           &
                           dvdz(k), surf_usm%upward(m)  .OR.  surf_usm%downward(m) )
       ENDDO
...
!-- Calculate the dissipation
    IF( les_default  .OR.  les_dynamic )  THEN
       ...
       !$ACC PRESENT(dd2zu, delta) PRESENT(topo_flags) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             !$ACC LOOP PRIVATE(k)
             DO  k = nzb+1, nzt
...
!--          future compiler versions.
    ...
    ELSEIF ( les_dai )  THEN
       ...
       !$ACC PRESENT(dd2zu, delta) PRESENT(distance_to_wall) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             !$ACC LOOP PRIVATE(k)
             DO  k = nzb+1, nzt
...
!--          future compiler versions.
    ...
    ELSEIF ( rans_tke_l )  THEN
      ...
      !$ACC PRESENT(dd2zu, ml_blackadar, delta) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
!
...
!-- Note, in case of rans_tke_e, the dissipation is already calculated in prognostic_equations
    ...
    !$ACC PRESENT(ddzu, ddzw, rho_air_zw, drho_air) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxl, nxr
       DO  j = nys, nyn
          DO  k = nzb+1, nzt
!
...
!--          conditions are set at walls.
             mask_right  = MERGE( 1.0_wp, 0.0_wp, rans_mode  .OR.  .NOT. rans_mode  .AND.          &
                                                  BTEST( topo_flags(k,j,i+1), 0 ) )
             mask_left   = MERGE( 1.0_wp, 0.0_wp, rans_mode  .OR.  .NOT. rans_mode  .AND.          &
                                                  BTEST( topo_flags(k,j,i-1), 0 ) )
             mask_north  = MERGE( 1.0_wp, 0.0_wp, rans_mode  .OR.  .NOT. rans_mode  .AND.          &
                                                  BTEST( topo_flags(k,j+1,i), 0 ) )
             mask_south  = MERGE( 1.0_wp, 0.0_wp, rans_mode  .OR.  .NOT. rans_mode  .AND.          &
                                                  BTEST( topo_flags(k,j-1,i), 0 ) )
             mask_top    = MERGE( 1.0_wp, 0.0_wp, rans_mode  .OR.  .NOT. rans_mode  .AND.          &
                                                  BTEST( topo_flags(k+1,j,i), 0 ) )
             mask_bottom = MERGE( 1.0_wp, 0.0_wp, rans_mode  .OR.  .NOT. rans_mode  .AND.          &
                                                  BTEST( topo_flags(k-1,j,i), 0 ) )
!
...
!--          conditions are set at walls.
             mask_down  = MERGE( 1.0_wp, 0.0_wp, rans_tke_e  .OR.  .NOT. rans_tke_e  .AND.         &
                                                 BTEST( topo_flags(k+1,j,i), 0 ) )
             mask_east  = MERGE( 1.0_wp, 0.0_wp, rans_tke_e  .OR.  .NOT. rans_tke_e  .AND.         &
                                                 BTEST( topo_flags(k,j,i-1), 0 ) )
             mask_north = MERGE( 1.0_wp, 0.0_wp, rans_tke_e  .OR.  .NOT. rans_tke_e  .AND.         &
                                                 BTEST( topo_flags(k,j-1,i), 0 ) )
             mask_south = MERGE( 1.0_wp, 0.0_wp, rans_tke_e  .OR.  .NOT. rans_tke_e  .AND.         &
                                                 BTEST( topo_flags(k,j+1,i), 0 ) )
             mask_west  = MERGE( 1.0_wp, 0.0_wp, rans_tke_e  .OR.  .NOT. rans_tke_e  .AND.         &
                                                 BTEST( topo_flags(k,j,i+1), 0 ) )
             mask_up    = MERGE( 1.0_wp, 0.0_wp, rans_tke_e  .OR.  .NOT. rans_tke_e  .AND.         &
                                                 BTEST( topo_flags(k-1,j,i), 0 ) )
...
!
!--    Set Neumann conditions at the bottom boundary. At surfaces, these values might be overwritten
!--    later on.
       IF ( ocean_mode )  THEN
          km(nzb,:,:) = km(nzb+1,:,:)
          kh(nzb,:,:) = kh(nzb+1,:,:)
...
!--    depends on the order how surfaces are accessed.
       !$OMP PARALLEL DO PRIVATE(i,j,k)
       !$ACC PARALLEL LOOP PRIVATE(i,j,k) &
       !$ACC PRESENT(bc_hv, kh, km) DEFAULT(NONE) IF(enable_openacc)
       DO  m = 1, bc_hv%ns
          i = bc_hv%i(m)
          j = bc_hv%j(m)
          k = bc_hv%k(m)
...
!-- Model top
    ...
    !$ACC PRIVATE(i,j) &
    !$ACC PRESENT(kh, km) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxl, nxr
       DO  j = nys, nyn
...
!$  tn = omp_get_thread_num()
    ...
    IF ( les_default  .OR.  les_dynamic )  THEN
       ...
       !$ACC PRESENT(kh, km, sums_l_l, rmask) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             !$ACC LOOP PRIVATE(k)
             DO  k = nzb+1, nzt
...
!--          Summation for averaged profile (cf. flow_statistics)
    ...
    ELSEIF ( les_dai )  THEN
       ...
       !$ACC PRIVATE(dvar_dz, ml, ml_stratification, ml_local_profile) &
       !$ACC PRESENT(topo_flags, var, dd2zu, e, delta) &
       !$ACC PRESENT(kh, km, sums_l_l, rmask) PRESENT(distance_to_wall) DEFAULT(PRESENT) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
             !$ACC LOOP PRIVATE(k)
             DO  k = nzb+1, nzt
...
!--          Summation for averaged profile (cf. flow_statistics)
    ...
    ELSEIF ( rans_tke_l )  THEN
       ...
       !$ACC PRESENT(kh, km, sums_l_l, rmask) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
!
!--          Calculate Richardson number
...
!--          Summation for averaged profile (cf. flow_statistics)
    ...
    ELSEIF ( rans_tke_e )  THEN
       ...
       !$ACC PRESENT(kh, km, sums_l_l) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
!
!--          Compute diffusion coefficients for momentum and heat
