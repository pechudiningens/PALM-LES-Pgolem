...
!--------------------------------------------------------------------------------------------------!
 MODULE diagnostic_output_quantities_mod
    ...
    CHARACTER(LEN=varnamelength), DIMENSION(:), ALLOCATABLE ::  do_all
    ...
    REAL(wp), DIMENSION(:,:), ALLOCATABLE ::  u_10m       !< 10-m u at center of grid box
    REAL(wp), DIMENSION(:,:), ALLOCATABLE ::  u_10m_av    !< mean of uc_10m
    ...
    REAL(wp), DIMENSION(:,:), ALLOCATABLE ::  v_10m       !< 10-m v at center of grid box
    REAL(wp), DIMENSION(:,:), ALLOCATABLE ::  v_10m_av    !< mean of vc_10m
    ...
    REAL(wp), DIMENSION(:,:), ALLOCATABLE ::  wdir_10m    !< 10-m wind direction
    REAL(wp), DIMENSION(:,:), ALLOCATABLE ::  wdir_10m_av !< 10-m mean wind direction
...
!--          restart IO yet.
          ...
          CASE ( 'wdir_10m*' )
             IF ( .NOT. ALLOCATED( u_10m_av ) )  THEN
                ALLOCATE( u_10m_av(nysg:nyng,nxlg:nxrg) )
             ENDIF
             IF ( .NOT. ALLOCATED( v_10m_av ) )  THEN
                ALLOCATE( v_10m_av(nysg:nyng,nxlg:nxrg) )
             ENDIF
             u_10m_av = 0.0_wp
             v_10m_av = 0.0_wp
    ...
    ELSEIF ( mode == 'sum' )  THEN
       ...
       SELECT CASE ( TRIM( variable ) )
          ...
          CASE ( 'wdir_10m*' )
             IF ( ALLOCATED( u_10m_av )  .AND.  ALLOCATED( v_10m_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      u_10m_av(j,i) = u_10m_av(j,i) + u_10m(j,i)
                      v_10m_av(j,i) = v_10m_av(j,i) + v_10m(j,i)
                   ENDDO
                ENDDO
             ENDIF
         ...
         CASE ( 'ta_2m*' )
          ...
          CASE ( 'wdir_10m*' )
             IF ( ALLOCATED( u_10m_av )  .AND.  ALLOCATED( v_10m_av ) )  THEN
                ...
                IF ( .NOT. ALLOCATED( wdir_10m_av ) )  THEN
                   ALLOCATE( wdir_10m_av(nysg:nyng,nxlg:nxrg) )
                ENDIF
                wdir_10m_av = 0.0_wp

                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      u_10m_av(j,i)    = u_10m_av(j,i) / REAL( average_count_3d, KIND=wp )
                      v_10m_av(j,i)    = v_10m_av(j,i) / REAL( average_count_3d, KIND=wp )
                      wdir_10m_av(j,i) = ATAN2( u_10m_av(j,i), v_10m_av(j,i) ) / pi * 180.0_wp +   &
                                         180.0_wp
                   ENDDO
                ENDDO
             ENDIF
...
!--    Treat horizotal cross-section output quantities.
       CASE ( 'kfd*', 'qv_2m*', 'theta_2m*', 'ta_2m*', 'vf25m*', 'vf50m*', 'vf75m*', 'vf100m*',    &
              'vfxxm*', 'vfd25m*', 'vfd50m*', 'vfd75m*', 'vfd100m*', 'vfdxxm*', 'wdir_10m*',       &
              'wspeed_10m*' )
!
          unit = 'm3/m/s'
!--       Check if output quantity is _xy only.
          ...
          IF ( TRIM( var ) == 'wdir_10m*'   )  unit = 'degree'
...
!--    s grid surface variables
       CASE ( 'kfd*_xy', 'qv_2m*_xy', 'theta_2m*_xy', 'ta_2m*_xy', 'wspeed_10m*_xy', 'vf25m*_xy',  &
              ...
              'vfd75m*_xy', 'vfd100m*_xy', 'vfdxxm*_xy', 'wdir_10m*_xy' )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE doq_output_2d( av, variable, found, grid, mode, local_pf, two_d, flag_nr, nzb_do,      &
                           nzt_do )
    ...
    SELECT CASE ( TRIM( variable ) )
       ...
       CASE ( 'uu_product_xy', 'uu_product_xz', 'uu_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => uu
          ELSE
             IF ( .NOT. ALLOCATED( uu_av ) )  THEN
       ...
       CASE ( 'uv_product_xy', 'uv_product_xz', 'uv_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => uv
          ELSE
             IF ( .NOT. ALLOCATED( uv_av ) )  THEN
       ...
       CASE ( 'uw_product_xy', 'uw_product_xz', 'uw_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => uw
          ELSE
             IF ( .NOT. ALLOCATED( uw_av ) )  THEN
       ...
       CASE ( 'vu_product_xy', 'vu_product_xz', 'vu_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => vu
          ELSE
             IF ( .NOT. ALLOCATED( vu_av ) )  THEN
       ...
       CASE ( 'vv_product_xy', 'vv_product_xz', 'vv_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => vv
          ELSE
             IF ( .NOT. ALLOCATED( vv_av ) )  THEN
       ...
       CASE ( 'vw_product_xy', 'vw_product_xz', 'vw_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => vw
          ELSE
             IF ( .NOT. ALLOCATED( vw_av ) )  THEN
       ...
       CASE ( 'wu_product_xy', 'wu_product_xz', 'wu_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => wu
          ELSE
             IF ( .NOT. ALLOCATED( wu_av ) )  THEN
       ...
       CASE ( 'wv_product_xy', 'wv_product_xz', 'wv_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => wv
          ELSE
             IF ( .NOT. ALLOCATED( wv_av ) )  THEN
       ...
       CASE ( 'ww_product_xy', 'ww_product_xz', 'ww_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => ww
          ELSE
             IF ( .NOT. ALLOCATED( ww_av ) )  THEN
       ...
       CASE ( 'wtheta_product_xy', 'wtheta_product_xz', 'wtheta_product_yz' )
          IF ( av == 0 )  THEN
       ...
       CASE ( 'wq_product_xy', 'wq_product_xz', 'wq_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => wq
          ELSE
             IF ( .NOT. ALLOCATED( wq_av ) )  THEN
       ...
       CASE ( 'ws_product_xy', 'ws_product_xz', 'ws_product_yz' )
          IF ( av == 0 )  THEN
             to_be_resorted => ws
          ELSE
             IF ( .NOT. ALLOCATED( ws_av ) )  THEN
       ...
       CASE ( 'wdir_10m*_xy' )        ! 2d-array
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   local_pf(i,j,nzb+1) = wdir_10m(j,i)
                ENDDO
             ENDDO
          ELSE
             IF ( .NOT. ALLOCATED( wdir_10m_av ) )  THEN
             ...
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   local_pf(i,j,nzb+1) = wdir_10m_av(j,i)
                ENDDO
             ENDDO
          ENDIF
          resorted = .TRUE.
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE doq_output_3d( av, variable, found, local_pf, resorted, flag_nr, nzb_do, nzt_do )

    IMPLICIT NONE

    CHARACTER (LEN=*) ::  variable !<

    INTEGER(iwp), INTENT(IN)    ::  av       !< index indicating averaged or instantaneous output
...
!--       Allocate array for wdir
          CASE ( 'wdir' )
             ...
             ENDIF
!
!--       Allocate array for 10-m wind direction
          CASE ( 'wdir_10m*' )
             IF ( .NOT. ALLOCATED( u_10m ) )  THEN
                ALLOCATE( u_10m(nysg:nyng,nxlg:nxrg) )
                u_10m = 0.0_wp
             ENDIF
             IF ( .NOT. ALLOCATED( v_10m ) )  THEN
                ALLOCATE( v_10m(nysg:nyng,nxlg:nxrg) )
                v_10m = 0.0_wp
             ENDIF
             IF ( .NOT. ALLOCATED( wdir_10m ) )  THEN
                ALLOCATE( wdir_10m(nysg:nyng,nxlg:nxrg) )
                wdir_10m = 0.0_wp
             ENDIF
...
!--                   Determine topography-top index on scalar-grid
                      DO  k = topo_top_ind(j,i,0)+1, topo_top_ind(j,i,0) + pch_index_ji(j,i)
                         ...
                         hr(k,j,i) = hr(k,j,i) + pcm_sensible_rate(kk,j,i)
                      ENDDO
                   ENDDO
                ENDDO
             ENDIF
...
!--          Heating of air-volume by surface interaction
             IF ( .NOT. neutral )  THEN
                ...
                DO  m = 1, surf_lsm%ns
                   ...
                   hr(k,j,i) = hr(k,j,i) + surf_lsm%shf_agg(m) * ddxyz
                ENDDO
                DO  m = 1, surf_usm%ns
                   i = surf_usm%i(m)
                   j = surf_usm%j(m)
...
!--       10-m wind speed
          CASE ( 'wspeed_10m*' )
!
!--          10-m wind speed is calculated from surface arrays. In case the 10m level is below the
...
!--       10-m wind direction
          CASE ( 'wdir_10m*' )
!
!--          10m wind direction is calculated from surface arrays. In ase the 10m level is below the
!--          first grid point, the lowest grid point is applied, else linear interpolation between
!--          two vertical grid levels is applied. To access all surfaces, iterate over all
!--          horizontally-upward facing surface types.
             surf => surf_def
             CALL calc_dir_10m
             surf => surf_lsm
             CALL calc_dir_10m
...
!-- ( integration interval is 0.5 * (max_integration_height - dzw(k) ) ). As surf%z_mo can lie
!-- beyond the first grid level in case of cut-cell topography, compute the k index accordingly.
    k = MINLOC( ABS( zw - zw(topo_top_ind(j,i,0)) - level ), DIM = 1 )
...
!--    Only compute 2-m mixing ratio at horizontally upward facing surfaces.
       IF ( surf%upward(m) )  THEN
          ...
          k = surf%k(m)
!
!--       If 2-m levelis below the level for the MOST relations, MOST is used for calculation of 2-m
!--       mixing ratio.
...
!--    Only compute 2-m temperature at horizontally upward facing surfaces.
       IF ( surf%upward(m) )  THEN
          ...
          k = surf%k(m)
!
!--       If 2-m level is below the reference level for the MOST relations, MOST is used for
!--       calculation of 2-m temperature.
          IF ( surf%z_mo(m) > 2.0_wp )  THEN
...
! Description:
! ------------
!> Calculation of 10-m wind direction.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE calc_dir_10m
    ...
    DO  m = 1, surf%ns
!
!--    Only compute 10-m wind direction at horizontally upward facing surfaces.
       IF ( surf%upward(m) ) THEN
...
!--       direction.
          IF ( surf%z_mo(m) > 10.0_wp )  THEN
             ...
             v_10m(j,i) = 0.5_wp * ( v(k,j,i) + v(k,j+1,i) )
!
...
!--          zw(k-1) defines the height of the surface.
             ...
             ENDDO
!
!--          kk defines the index of the first level >= 10m.
             u_10m(j,i) = 0.5_wp * ( u(kk-1,j,i) + u(kk-1,j,i+1) ) +                               &
                          0.5_wp * ( u(kk,j,i) + u(kk,j,i+1) - u(kk-1,j,i) - u(kk-1,j,i+1) ) *     &
                          ( zw(k-1) + 10.0_wp - zu(kk-1) ) / ( zu(kk) - zu(kk-1) )
             v_10m(j,i) = 0.5_wp * ( v(kk-1,j,i) + v(kk-1,j+1,i) ) +                               &
                          0.5_wp * ( v(kk,j,i) + v(kk,j+1,i) - v(kk-1,j,i) - v(kk-1,j+1,i) ) *     &
                          ( zw(k-1) + 10.0_wp - zu(kk-1) ) / ( zu(kk) - zu(kk-1) )
          ENDIF
          wdir_10m(j,i) = ATAN2( u_10m(j,i), v_10m(j,i) ) / pi * 180.0_wp + 180.0_wp
       ENDIF

    ENDDO

END SUBROUTINE calc_dir_10m
...
!--    Only compute 10-m wind speed at horizontally upward facing surfaces.
       IF ( surf%upward(m) )  THEN
          ...
          k = surf%k(m)
!
!--       If 10-m level is below the level for the MOST relations, MOST is used for calculation
!--       of 10-m wind speed.
          IF ( surf%z_mo(m) > 10.0_wp )  THEN
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE doq_prepare
    ...
    INTEGER(iwp) ::  do_all_size  !< size of do_all array
...
!
!-- Calculate required size and allocate array do_all.
    do_all_size = SUM( do2d_no ) + SUM( do3d_no ) + SUM( domask_no(1:masks,:) )
    ALLOCATE( do_all(do_all_size) )
 ...
 SUBROUTINE doq_rrd_local_ftn( k, nxlf, nxlc, nxl_on_file, nxrf, nxrc, nxr_on_file, nynf, nync,    &
    ...
    SELECT CASE ( restart_string(1:length) )
       ...
       CASE ( 'u_10m_av' )
          ...
          ENDIF
          IF ( k == 1 )  READ ( 13 )  tmp_2d
          u_10m_av(nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp)  =                                     &
                       tmp_2d(nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)
       ...
       CASE ( 'v_10m_av' )
          ...
          ENDIF
          IF ( k == 1 )  READ ( 13 )  tmp_2d
          v_10m_av(nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp)  =                                     &
                       tmp_2d(nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)
...
!-- This case, input of time-averaged data is useless and can lead to faulty averaging.
    IF ( .NOT. cyclic_fill_initialization )  THEN
       ...
       CALL rd_mpi_io_check_array( 'u_10m_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( u_10m_av ) )  ALLOCATE( u_10m_av(nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'u_10m_av', u_10m_av )
       ENDIF
       ...
       CALL rd_mpi_io_check_array( 'v_10m_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( v_10m_av ) )  ALLOCATE( v_10m_av(nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'v_10m_av', v_10m_av )
       ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE doq_wrd_local
    ...
    IF ( TRIM( restart_data_format_output ) == 'fortran_binary' )  THEN
       ...
       IF ( ALLOCATED( u_10m_av ) )  THEN
          CALL wrd_write_string( 'u_10m_av' )
          WRITE ( 14 )  u_10m_av
       ENDIF
       ...
       IF ( ALLOCATED( v_10m_av ) )  THEN
          CALL wrd_write_string( 'v_10m_av' )
          WRITE ( 14 )  v_10m_av
       ENDIF
    ...
    ELSEIF ( restart_data_format_output(1:3) == 'mpi' )  THEN
       ...
       IF ( ALLOCATED( u_10m_av ) )     CALL wrd_mpi_io( 'u_10m_av', u_10m_av )
       ...
       IF ( ALLOCATED( v_10m_av ) )     CALL wrd_mpi_io( 'v_10m_av', v_10m_av )
