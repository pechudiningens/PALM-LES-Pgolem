...
!$ACC COPY(tq_m(nzb:nzt+1,nysg:nyng,nxlg:nxrg)) &
!$ACC COPY(vpt(nzb:nzt+1,nysg:nyng,nxlg:nxrg)) &
!$ACC COPY(q_init(:)) &
!$ACC COPY(exner(:)) &
!$ACC COPY(d_exner(:)) &
!$ACC COPY(hyp(:)) &
!$ACC COPY(sums_wsts_bc_l(:,:)) IF(enable_openacc)
...
!$ACC COPY(q_p(nzb:nzt+1,nysg:nyng,nxlg:nxrg)) &
...
!$ACC COPY(tpt_m(nzb:nzt+1,nysg:nyng,nxlg:nxrg)) IF(enable_openacc)
...
!$ACC COPYIN(ug(0:nz+1), vg(0:nz+1)) IF(enable_openacc)

!
!-- Copy data from control_parameters
!$ACC DATA &
!$ACC COPYIN(tsc(1:5)) IF(enable_openacc)

!
!-- Copy data from grid_variables
!$ACC DATA &
!$ACC COPYIN(ddx, ddy) IF(enable_openacc)
...
!$ACC COPYIN(bc_hv%koff(1:bc_hv%ns)) IF(enable_openacc)
...
!$ACC COPY(sums_salsa_ws_l(nzb:nzt+1,0)) IF(enable_openacc)
...
!-- Synchronize the timestep in case of nested run.
    IF ( nested_run )  THEN
!
!--    Synchronization of model simulations. Either the timesteps among all domains are unified
!--    by computing the global minimum of all time-steps, or the maximum timestep over all domains
!--    is computed and distributed to all domains so that these can run on their own timestep
!--    until they reach the next coupling timestep.
...
!-- Iterate until end_time has been reached, and the run is not going to be terminated because
!-- the time step became too small (stop_dt) or because of a forced restart or running out of
!-- CPU time.
!-- In case of asynchronously-coupled nested simulations (each model uses its own time step),
!-- simulated_time is allowed to slightly exceed end_time and time-integration will be continued
!-- until all coupled models have reached the next time for coupling.
    CALL location_message( 'time-stepping', 'start' )

    DO  WHILE ( ( simulated_time < end_time  .AND.  ( .NOT. stop_dt  .AND.  .NOT. terminate_run ) )&
                .OR.  ( .NOT. nesting_coupling_time_reached  .AND.  .NOT. synchronize_timestep ) )
...
!--        boundary condition
           ...
           IF ( air_chemistry )  THEN
              ...
              ENDDO
           ENDIF
           IF ( salsa  .AND.  time_since_reference_point >= salsa_start_time )  THEN
...
!>           TODO: why the +1 ? Why not working with 0,1 as in the remaining model?
             CALL pmci_set_swaplevel( MOD( timestep_count, 2) + 1 )
          ENDIF
#endif
...
!
!--       Grid nesting is applied. In case of a two-way coupled or a synchronized one-way coupled
!--       parent-child simulation, data is transferred between parent and child each RK-substep.
!--       In case of a non-sychronized one-way coupled simulation, data transfer only happens at
!--       distinct coupling time steps determined by the largest timestep occurring among the
!--       models (usually this is the parent). This case, boundary conditions are only set at
!--       the first RK-substep and are held constant afterwards.
#if defined( __parallel )
          IF ( nested_run  .AND.  ( TRIM( nesting_mode ) == 'two-way'  .OR.                        &
               synchronize_timestep  .OR.  ( nesting_coupling_time_reached  .AND.                  &
                                             intermediate_timestep_count == 1 ) ) )  THEN

             CALL cpu_log( log_point(60), 'nesting', 'start' )
!
!--          Domain nesting. The data transfer subroutines pmci_parent_datatrans and
!--          pmci_child_datatrans are called inside the wrapper subroutine pmci_datatrans according
!--          to the control parameters nesting_mode and nesting_datatransfer_mode.
             CALL pmci_datatrans
...
!--          model. Call for horizontal as well as vertical surfaces
             IF ( land_surface .AND. time_since_reference_point >= lsm_start_time)  THEN
...
!--             might be important for the nesting, where arrays are transfered.
             ...
             ENDIF
!
!--          If required, solve the energy balance for SLUrb surfaces and run the material heat
!--          model.
             IF ( slurb )  THEN
               CALL cpu_log( log_point(74), 'slurb', 'start' )
               CALL slurb_model
               CALL cpu_log( log_point(74), 'slurb', 'stop' )
             ENDIF
!
...
!--    Will be used at some point by flow_statistics.
       ...
       !$ACC HOST(sums_salsa_ws_l(nzb:nzt+1,0)) IF(enable_openacc)

!
!--    If required, calculate radiative fluxes and heating rates
       IF ( radiation  .AND.  time_since_reference_point >= radiation_start_time )  THEN
...
!
!--          Set a flag that radiation has been called this timestep
          ...
          ELSE
!
!--          Set a flag that radiation wasn't been called this timestep.
...
!--    Synchronize the timestep in case of nested run.
       IF ( nested_run )  THEN
!
!--       Synchronization of model simulations. Either the time steps among all models are unified
!--       by computing the global minimum of all time steps, or the maximum timestep over all
!--       models is computed and distributed to all models, so that they can run on their own
!--       time step until they reach the time for the next coupling.
          CALL pmci_synchronize
       ENDIF
#endif
!
!--    Check, if restart is necessary (because cpu-time is expiring or because it is forced by
!--    user) and set stop flag.
