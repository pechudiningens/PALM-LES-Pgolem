...
!-- Check for photolysis scheme.
    ...
    IF ( photolysis_scheme == 'simple'  .AND.  .NOT. radiation )  THEN
       message_string = 'a radiation scheme is missing for photolysis_scheme = "simple"'
       CALL message( 'chem_check_parameters', 'CHM0050', 1, 2, 0, 6, 0 )
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE chem_data_output_3d( av, variable, found, local_pf, resorted, nzb_do, nzt_do )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE chem_prognostic_equations_ij( i, j, i_omp_start, tn )
    ...
    REAL(wp), DIMENSION(1:surf_top%ns) ::  s_flux_top  !< fluxes at model top
...
!--    Advection terms
       ...
       ENDIF
!
!--    Diffusion terms (the last three arguments are zero). Local arrays are used to prevent
!--    compiler warnings about creating temporary arrays.
       s_flux_def(:) = surf_def%cssws(ilsp,:)
       s_flux_lsm(:) = surf_lsm%cssws(ilsp,:)
       s_flux_usm(:) = surf_usm%cssws(ilsp,:)
       s_flux_top(:) = surf_top%cssws(ilsp,:)
       CALL diffusion_s( i, j, chem_species(ilsp)%conc, s_flux_top, s_flux_def, s_flux_lsm,        &
                         s_flux_usm )
!
...
!
!-- Restart input of chemistry variables is skipped in case of cyclic-fill initialization since it
!-- has not been realized with mpi-io restart mechanism yet.
