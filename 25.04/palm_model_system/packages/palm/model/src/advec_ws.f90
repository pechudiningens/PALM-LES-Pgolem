...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE ws_init_flags_momentum
    ...
    LOGICAL      ::  flux_x   !< flag to check if the flux in x-direction is defined
    LOGICAL      ::  flux_y   !< flag to check if the flux in y-direction is defined
    LOGICAL      ::  flux_z   !< flag to check if the flux in z-direction is defined
...
!-- u-component.
    DO  i = nxl, nxr
       DO  j = nys, nyn
          DO  k = nzb+1, nzt
!
!--          Set switches to check if the location where the fluxes are defined are topography or
!--          atmosphere. IF the tests give .F., no fluxes will be calculated below.
             flux_x = BTEST( topo_flags(k,j,i+1), 0  )
             flux_y = BTEST( topo_flags(k,j+1,i), 7  )
             flux_z = BTEST( topo_flags(k,j,i),   10 )
!
...
!-- v-component.
    DO  i = nxl, nxr
       DO  j = nys, nyn
          DO  k = nzb+1, nzt
!
!--          Set dummies to check if the location where the fluxes are defined are topography or
!--          atmosphere.
             flux_x = BTEST( topo_flags(k,j,i),   11 )
             flux_y = BTEST( topo_flags(k,j+1,i), 0  )
             flux_z = BTEST( topo_flags(k,j,i),   26 )
!
!--          At first, set flags to WS1.
...
!--          Set dummies to check if the location where the fluxes are defined are topography or
!--          atmosphere.
             flux_x = BTEST( topo_flags(k,j,i),   27 )
             flux_y = BTEST( topo_flags(k,j,i),   28 )
             flux_z = BTEST( topo_flags(k+1,j,i), 0  )
!
!--          At first, set flags to WS1.
!--          Since fluxes are swapped in advec_ws.f90, this is necessary to in order to handle the
!--          left/south flux.
...
!-- all non-prognostic grid points.
    DO  i = nxl, nxr
       DO  j = nys, nyn
          DO  k = nzb+1, nzt
!
!--          If Cartesian topography is used and the actual grid point is classified as topography,
!--          the fluxes at the faces of the grid box are zero and all further actions are skipped.
!--          If cut-cell topography is used and the actual grid point is classified as topography,
!--          the fluxes at the faces of the grid box are not necessarily zero. Even though this
!--          won't have any effect on the actual grid point, the computed flux also enters the
!--          prognostic equation of the neighboring grid point in x-, y- or z-direction. However,
!--          this, case, the fluxes will be only first order.
             IF ( .NOT.  BTEST( topo_flags_alt(k,j,i), 0 ) )  THEN

                IF ( cut_cell_topography ) THEN
!
!--                Flux in x-direction, defined at u-grid.
                   IF ( BTEST( topo_flags_alt(k,j,i+1), 1 ) )  THEN
                      advc_flag(k,j,i) = IBSET( advc_flag(k,j,i), 0 )
                   ENDIF
!
!--                Flux in y-direction, defined at v-grid.
                   IF ( BTEST( topo_flags_alt(k,j+1,i), 2 ) )  THEN
                      advc_flag(k,j,i) = IBSET( advc_flag(k,j,i), 3 )
                   ENDIF
!
!--                Flux in z-direction, defined at w-grid.
                   IF ( BTEST( topo_flags_alt(k,j,i), 3 ) )  THEN
                      advc_flag(k,j,i) = IBSET( advc_flag(k,j,i), 6 )
                   ENDIF

                ENDIF
!
!--             In any case, skip all further actions for this grid point.
                CYCLE
...
!--          Set dummies to check if the location where the fluxes are defined are topography or
!--          atmosphere.
             flux_x = BTEST( topo_flags(k,j,i+1), 1 )
             flux_y = BTEST( topo_flags(k,j+1,i), 2 )
             flux_z = BTEST( topo_flags(k,j,i),   3 )
!
!--          scalar - x-direction
!--          WS1 (0), WS3 (1), WS5 (2)
             IF ( flux_x )  THEN
...
!--                   Set flags that indicate 1st-order advection along x- and y-direction.
                      IF ( flux_x )  advc_flag(k,j,i) = IBSET( advc_flag(k,j,i), 0 )
                      IF ( flux_y )  advc_flag(k,j,i) = IBSET( advc_flag(k,j,i), 3 )
                   ENDIF
!
...
!--                   scheme.
                      IF ( BTEST( advc_flag(k,j,i-1), 31 )  .AND.                                  &
                           BTEST( advc_flag(k,j,i+1), 31 )  .AND.  flux_x )  THEN
...
!--                   scheme.
                      IF ( .NOT. BTEST( advc_flag(k,j,i-1), 31 )  .AND.                            &
                           ...
                           .NOT. BTEST( advc_flag(k,j,i+1), 31 )  .AND.                            &
                                 BTEST( advc_flag(k,j,i+2), 31 )  .AND.  flux_x )  THEN
                         advc_flag(k,j,i) = IBCLR( advc_flag(k,j,i), 0 )
                         advc_flag(k,j,i) = IBCLR( advc_flag(k,j,i), 1 )
                         advc_flag(k,j,i) = IBCLR( advc_flag(k,j,i), 2 )
...
!--                   scheme.
                      IF ( BTEST( advc_flag(k,j-1,i), 31 )  .AND.                                  &
                           BTEST( advc_flag(k,j+1,i), 31 )  .AND.  flux_y )  THEN
...
!--                   scheme.
                      IF ( .NOT. BTEST( advc_flag(k,j-1,i), 31 )  .AND.                            &
                           ...
                           .NOT. BTEST( advc_flag(k,j+1,i), 31 )  .AND.                            &
                                 BTEST( advc_flag(k,j+2,i), 31 )  .AND.  flux_y )  THEN
                         advc_flag(k,j,i) = IBCLR( advc_flag(k,j,i), 3 )
                         advc_flag(k,j,i) = IBCLR( advc_flag(k,j,i), 4 )
                         advc_flag(k,j,i) = IBCLR( advc_flag(k,j,i), 5 )
...
!-- prognostic_equations.
    IF ( ws_scheme_mom )  THEN
       ...
       !$ACC PRESENT(sums_us2_ws_l, sums_vs2_ws_l, sums_ws2_ws_l) DEFAULT(NONE) IF(enable_openacc)
       sums_wsus_ws_l = 0.0_wp
       sums_wsvs_ws_l = 0.0_wp
       sums_us2_ws_l  = 0.0_wp
       sums_vs2_ws_l  = 0.0_wp
    ...
    IF ( ws_scheme_sca )  THEN
       !$ACC KERNELS PRESENT(sums_wspts_ws_l) DEFAULT(NONE) IF(enable_openacc)
       sums_wspts_ws_l = 0.0_wp
       !$ACC END KERNELS
...
!--    the caller side.
    ...
    !$ACC PRESENT(sums_salsa_ws_l, topo_flags) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxl, nxr
       DO  j = nys, nyn
!
...
!-- the caller side.
    ...
    CALL cpu_log( log_point_s(105), 'advec_s_ws_div_bc', 'start' )
...
!-- Re-calculate tendency term which is further used within the RK time-integration.
    ...
    CALL cpu_log( log_point_s(105), 'advec_s_ws_div_bc', 'stop' )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE advec_u_ws
    ...
    !$ACC PRESENT(hom(:,1,1:3,0)) &
    !$ACC PRESENT(weight_substep(intermediate_timestep_count)) &
    !$ACC PRESENT(sums_us2_ws_l, sums_wsus_ws_l) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxlu, nxr
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE advec_v_ws
    ...
    !$ACC PRESENT(hom(:,1,1:3,0)) &
    !$ACC PRESENT(weight_substep(intermediate_timestep_count)) &
    !$ACC PRESENT(sums_vs2_ws_l, sums_wsvs_ws_l) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxl, nxr
       DO  j = nysv, nyn
!
!--       Used local modified copy of nzb_max (used to degrade order of discretization) at
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE advec_w_ws
    ...
    !$ACC PRESENT(drho_air_zw, ddzu, rho_air) &
