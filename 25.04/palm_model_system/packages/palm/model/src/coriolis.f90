...
!>
!> @note In this routine the topography is masked, even though this is again done in
!> prognostic_equations. However, omitting the masking here lead to slightly different results.
!> Reason unknown.
!--------------------------------------------------------------------------------------------------!
 MODULE coriolis_mod
...
!--                Predetermine flag to mask topography
                   ...
                   tend(k,j,i) = tend(k,j,i) + flag * (                                            &
                            f * ( 0.25_wp * ( v(k,j,i-1) + v(k,j,i) + v(k,j+1,i-1) + v(k,j+1,i) )  &
                                  - vg(k) )                                                        &
                         - fs * cos_rot_angle                                                      &
...
!--                Predetermine flag to mask topography
                   ...
                   tend(k,j,i) = tend(k,j,i) - flag * (                                            &
                            f * ( 0.25_wp * ( u(k,j-1,i) + u(k,j,i) + u(k,j-1,i+1) + u(k,j,i+1) )  &
                                  - ug(k) )                                                        &
                         + fs * sin_rot_angle                                                      &
...
!--                Predetermine flag to mask topography
       ...
       CASE DEFAULT
          ...
          CALL message( 'coriolis', 'PAC0178', 1, 2, 0, 6, 0 )
...
!--          Predetermine flag to mask topography
             ...
             tend(k,j,i) = tend(k,j,i) + fs * 0.25_wp * flag                                       &
                                         * ( cos_rot_angle                                         &
                                           * ( u(k,j,i) + u(k+1,j,i) + u(k,j,i+1) + u(k+1,j,i+1) ) &
                                           + sin_rot_angle                                         &
                                           * ( v(k,j,i) + v(k+1,j,i) + v(k,j+1,i) + v(k+1,j+1,i) ) &
