...
!-- Open output file.
    IF ( netcdf_data_format < 5 )  THEN
       IF ( myid == 0 )  CALL check_open( 200+mid+av*max_masks )
    ELSE
...
!-- given output interval.
    IF ( netcdf_data_format > 4 )  THEN
       IF ( domask_time_count(mid,av) + 1 > ntdim_masks(mid,av) )  THEN
          WRITE ( message_string, * ) 'no output for masked at t = ',                              &
                                      time_since_reference_point, ' s because maximum number of ', &
                                      ' of output time levels is exceeded'
          CALL message( 'data_output_masks', 'PAC0188', 0, 1, 0, 6, 0 )
...
!
!--    Reallocate local_pf on PE 0 since its shape changes during collection of data on PE0.
...
!--       (1) a. Parallel I/O using netCDF.
          ...
          ELSE
!
!--          No masked data on this specific subdomain / core.
             nc_stat = NF90_PUT_VAR( id_set_mask(mid,av),                                          &
                                     id_var_domask(mid,av,ivar), local_pf,                         &
