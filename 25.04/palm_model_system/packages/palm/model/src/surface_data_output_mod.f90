...
MODULE surface_data_output_mod
   ...
   CHARACTER(LEN=100), DIMENSION(0:1,300) ::  dosurf_unit = ' '       !< internal variable which holds the unit of the given
                                                                      !< output variable
   ...
   INTEGER(iwp), DIMENSION(0:1) ::  id_dim_3d_surf         !< netcdf ID for dimension 3d vector
   INTEGER(iwp), DIMENSION(0:1) ::  id_dim_faces_vert_surf !< netcdf ID for dimension number of face vertices
   ...
   INTEGER(iwp), DIMENSION(0:1) ::  id_dim_vert_surf       !< netcdf ID for dimension number of overall vertices
   INTEGER(iwp), DIMENSION(0:1) ::  id_set_surf            !< netcdf ID for file
   INTEGER(iwp), DIMENSION(0:1) ::  id_var_face_surf       !< netcdf ID for variable faces
   INTEGER(iwp), DIMENSION(0:1) ::  id_var_vert_surf       !< netcdf ID for variable vertices

   INTEGER(iwp), DIMENSION(0:1) ::  id_var_area_surf       !< netcdf ID for variable area
...
#endif
!
!-- Determine the total number of vertices in the model domain
    IF ( cut_cell_topography )  THEN
       surf_out%num_vert       = cct%num_vert
       surf_out%num_faces_vert = cct%num_faces_vert
    ENDIF
!
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE surface_data_output_init
    ...
    REAL(wp) ::  xs     !< x-position of a surface element
    REAL(wp) ::  ys     !< y-position of a surface element
...
!--       surfaces and i-index. This means that ioff and joff are only added if their value is 1.
...
!--     Allocate local coordinate arrays
        ...
        IF ( cut_cell_topography )  THEN
           ALLOCATE( surf_out%area(1:surf_out%ns) )
           ALLOCATE( surf_out%polygons(surf_out%num_faces_vert,surf_out%ns) )
           ALLOCATE( surf_out%vertices(3,1:surf_out%num_vert) )
        ENDIF
!
...
!--           Uward-facing
              IF ( surf_def%upward(m) )  THEN
                 ...
                 off_x = 0.5_wp
                 off_y = 0.5_wp
                 xs = ( surf_def%i(m) + off_x ) * dx
                 ys = ( surf_def%j(m) + off_y ) * dy
...
!--           Uward-facing
              IF ( surf_lsm%upward(m) )  THEN
                 ...
                 off_x = 0.5_wp
                 off_y = 0.5_wp
                 xs = ( surf_lsm%i(m) + off_x ) * dx
                 ys = ( surf_lsm%j(m) + off_y ) * dy
...
!--        Slanted faces
           DO  m = 1, surf_cct%ns
              ...
              surf_out%s(mm)          = i
              surf_out%xs(mm)         = surf_cct%wall_location_c(3,m)
              surf_out%ys(mm)         = surf_cct%wall_location_c(2,m)
              surf_out%zs(mm)         = surf_cct%wall_location_c(1,m)
              surf_out%azimuth(mm)    = az
              surf_out%zenith(mm)     = ze
              surf_out%polygons(:,mm) = surf_cct%faces(:,m)
              surf_out%area(mm)       = surf_cct%area(m)
...
!--     Include vertices.
        IF ( cut_cell_topography )  THEN
           surf_out%vertices(:,:) = cct%vertices(:,:)
        ENDIF
!
!--     Initialize NetCDF data output. Please note, local start position for the surface elements in
!--     the NetCDF file is surf_out%s(1), while the number of surfaces on the subdomain is given by
!--     surf_out%ns.
#if defined( __netcdf4_parallel )
...
!--        Define index of surface element
           CALL netcdf_create_dim( id_set_surf(av), 's', surf_out%ns_total, id_dim_s_surf(av),     &
                                   5558 )
!
!--        Define dimension for slanted faces
           IF ( cut_cell_topography )  THEN
              CALL netcdf_create_dim( id_set_surf(av), 'cct_num_faces_vert',                       &
                                      surf_out%num_faces_vert, id_dim_faces_vert_surf(av), 5558 )
              CALL netcdf_create_dim( id_set_surf(av), 'cct_num_vert', surf_out%num_vert,          &
                                      id_dim_vert_surf(av), 5558 )
              CALL netcdf_create_dim( id_set_surf(av), 'dim_3d', 3, id_dim_3d_surf(av), 5558 )
           ENDIF
...
!--        Define z coordinate
           ...
           IF ( cut_cell_topography )  THEN
!
!--           Define faces ! FIXME: numbers 5560 etc
              CALL netcdf_create_var( id_set_surf(av), (/ id_dim_faces_vert_surf(av),              &
                                      id_dim_s_surf(av) /), 'cct_faces', NF90_INT,                 &
                                      id_var_face_surf(av), 'polygon', 'index', 5560, 5560, 5560 )
              CALL netcdf_create_att( id_set_surf(av), id_var_face_surf(av), 'standard_name',      &
                                      'faces', 5583 )
!
!--           Define vertices ! FIXME: numbers 5560 etc
              CALL netcdf_create_var( id_set_surf(av), (/ id_dim_3d_surf(av),                      &
                                      id_dim_vert_surf(av) /), 'cct_vertices', NF90_DOUBLE,        &
                                      id_var_vert_surf(av), 'vertices', 'x,y,z coordinates',       &
                                      5560, 5560, 5560 )
              CALL netcdf_create_att( id_set_surf(av), id_var_vert_surf(av), 'standard_name',      &
                                      'vertices', 5583 )
           ENDIF
!
!--        Define angles
           CALL netcdf_create_var( id_set_surf(av), (/ id_dim_s_surf(av) /), 'azimuth',            &
                                   ...
                                   'azimuth angle', 5577, 5578, 5579, set_fillvalue = .TRUE. )
           ...
           CALL netcdf_create_var( id_set_surf(av), (/ id_dim_s_surf(av) /), 'zenith',             &
                                   ...
                                   5582, set_fillvalue = .TRUE. )
           IF ( cut_cell_topography )  THEN
                   !TODO numbers
              CALL netcdf_create_var( id_set_surf(av), (/ id_dim_s_surf(av) /), 'cct_area',        &
                                      NF90_DOUBLE, id_var_area_surf(av), 'square meters',          &
                                      'surface element area', 5561, 5561, 5561 )
...
!--        Define the variables
           ...
           DO WHILE ( dosurf(av,i)(1:1) /= ' ' )
              ...
              CALL netcdf_create_var( id_set_surf(av),                                             &
                                      ...
                                      dosurf(av,i), 5565, 5565, 5565, set_fillvalue = .TRUE.,      &
                                      no_prefill = .TRUE. )
!
...
!--        Write surface positions to file
           ...
           IF ( cut_cell_topography )  THEN
              nc_stat = NF90_PUT_VAR( id_set_surf(av), id_var_face_surf(av), surf_out%polygons,    &
                                      start = (/ 1,                       surf_out%s(1) /),        &
                                      count = (/ surf_out%num_faces_vert, surf_out%ns /) )
              CALL netcdf_handle_error( 'surface_data_output_init', 5587 )
              IF ( myid == 0 )  THEN
                 nc_stat = NF90_PUT_VAR( id_set_surf(av), id_var_vert_surf(av), surf_out%vertices, &
                                         start = (/ 1, 1 /),                                       &
                                         count = (/ 3, surf_out%num_vert /) )
                 CALL netcdf_handle_error( 'surface_data_output_init', 5588 )
              ENDIF
           ENDIF
           ...
           IF ( cut_cell_topography )  THEN
              nc_stat = NF90_PUT_VAR( id_set_surf(av), id_var_area_surf(av), surf_out%area,        &
                                      start = (/ surf_out%s(1) /), count = (/ surf_out%ns /) )
              CALL netcdf_handle_error( 'surface_data_output_init', 5573 )
           ENDIF
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%usws, surf_lsm%usws, surf_usm%usws,     &
                                                  surf_cct%usws, momentumflux_output_conversion )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%vsws, surf_lsm%vsws, surf_usm%vsws,     &
                                                  surf_cct%vsws, momentumflux_output_conversion )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%shf, surf_lsm%shf_agg, surf_usm%shf,    &
                                                  surf_cct%shf, heatflux_output_conversion )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%qsws, surf_lsm%qsws_agg, surf_usm%qsws, &
                                                  surf_cct%qsws, waterflux_output_conversion )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%ssws, surf_lsm%ssws, surf_usm%ssws,     &
                                                  surf_cct%ssws, scalarflux_output_conversion )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%q_surface, surf_lsm%q_surface,          &
                                                  surf_usm%q_surface, surf_cct%q_surface )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%pt_surface, surf_lsm%pt_surface,        &
                                                  surf_usm%pt_surface, surf_cct%pt_surface )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%vpt_surface, surf_lsm%vpt_surface,      &
                                                  surf_usm%vpt_surface, surf_cct%vpt_surface )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%pt_10cm, surf_lsm%pt_10cm,              &
                                                  surf_usm%pt_10cm, surf_cct%pt_10cm )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_net, surf_lsm%rad_net,              &
                                                  surf_usm%rad_net, surf_cct%rad_net )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_lw_in, surf_lsm%rad_lw_in,          &
                                                  surf_usm%rad_lw_in, surf_cct%rad_lw_in )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_lw_out, surf_lsm%rad_lw_out,        &
                                                  surf_usm%rad_lw_out, surf_cct%rad_lw_out )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_sw_in, surf_lsm%rad_sw_in,          &
                                                  surf_usm%rad_sw_in, surf_cct%rad_sw_in )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_sw_out, surf_lsm%rad_sw_out,        &
                                                  surf_usm%rad_sw_out, surf_cct%rad_sw_out )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%r_soil, surf_lsm%r_soil,                &
                                                  surf_usm%r_soil, surf_cct%r_soil )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%r_canopy, surf_lsm%r_canopy,            &
                                                  surf_usm%r_canopy, surf_cct%r_canopy )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_sw_dir, surf_lsm%rad_sw_dir,        &
                                                  surf_usm%rad_sw_dir, surf_cct%rad_sw_dir )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_sw_dif, surf_lsm%rad_sw_dif,        &
                                                  surf_usm%rad_sw_dif, surf_cct%rad_sw_dif )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_sw_ref, surf_lsm%rad_sw_ref,        &
                                                  surf_usm%rad_sw_ref, surf_cct%rad_sw_ref )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_sw_res, surf_lsm%rad_sw_res,        &
                                                  surf_usm%rad_sw_res, surf_cct%rad_sw_res )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_lw_dif, surf_lsm%rad_lw_dif,        &
                                                  surf_usm%rad_lw_dif, surf_cct%rad_lw_dif )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_lw_ref, surf_lsm%rad_lw_ref,        &
                                                  surf_usm%rad_lw_ref, surf_cct%rad_lw_ref )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%rad_lw_res, surf_lsm%rad_lw_res,        &
                                                  surf_usm%rad_lw_res, surf_cct%rad_lw_res )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%uvw_abs, surf_lsm%uvw_abs,              &
                                                  surf_usm%uvw_abs, surf_cct%uvw_abs )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%waste_heat, surf_lsm%waste_heat,        &
                                                  surf_usm%waste_heat, surf_cct%waste_heat )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%iwghf_eb, surf_lsm%iwghf_eb,            &
                                                  surf_usm%iwghf_eb, surf_cct%iwghf_eb )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%albedo, surf_lsm%albedo,                &
                                                  surf_usm%albedo, surf_cct%albedo )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
...
!--          Output of instantaneous data
             IF ( av == 0 )  THEN
                CALL surface_data_output_collect( surf_def%emissivity, surf_lsm%emissivity,        &
                                                  surf_usm%emissivity, surf_cct%emissivity )
             ELSE
!
!--             Output of averaged data
                surf_out%var_out(:) = surf_out%var_av(:,n_out) / REAL( average_count_surf, KIND=wp )
 ...
 SUBROUTINE surface_data_output_averaging
    ...
    DO  WHILE ( dosurf(1,n_out+1)(1:1) /= ' ' )
       ...
       SELECT CASE ( trimvar )
          ...
          CASE ( 'us' )
             CALL surface_data_output_sum_up( surf_def%us, surf_lsm%us, surf_usm%us,               &
                                              surf_cct%us, n_out )

          CASE ( 'ts' )
             CALL surface_data_output_sum_up( surf_def%ts, surf_lsm%ts, surf_usm%ts,               &
                                              surf_cct%ts, n_out )

          CASE ( 'qs' )
             CALL surface_data_output_sum_up( surf_def%qs, surf_lsm%qs, surf_usm%qs,               &
                                              surf_cct%qs, n_out )

          CASE ( 'ss' )
             CALL surface_data_output_sum_up( surf_def%ss, surf_lsm%ss, surf_usm%ss,               &
                                              surf_cct%ss, n_out )

          CASE ( 'qcs' )
             CALL surface_data_output_sum_up( surf_def%qcs, surf_lsm%qcs, surf_usm%qcs,            &
                                              surf_cct%qcs, n_out )

          CASE ( 'ncs' )
             CALL surface_data_output_sum_up( surf_def%ncs, surf_lsm%ncs, surf_usm%ncs,            &
                                              surf_cct%ncs, n_out )

          CASE ( 'qis' )
             CALL surface_data_output_sum_up( surf_def%qis, surf_lsm%qis, surf_usm%qis,            &
                                              surf_cct%qis, n_out )

          CASE ( 'nis' )
             CALL surface_data_output_sum_up( surf_def%nis, surf_lsm%nis, surf_usm%nis,            &
                                              surf_cct%nis, n_out )

          CASE ( 'qrs' )
             CALL surface_data_output_sum_up( surf_def%qrs, surf_lsm%qrs, surf_usm%qrs,            &
                                              surf_cct%qrs, n_out )

          CASE ( 'nrs' )
             CALL surface_data_output_sum_up( surf_def%nrs, surf_lsm%nrs, surf_usm%nrs,            &
                                              surf_cct%nrs, n_out )

          CASE ( 'ol' )
             CALL surface_data_output_sum_up( surf_def%ol, surf_lsm%ol, surf_usm%ol,               &
                                              surf_cct%ol, n_out )

          CASE ( 'z0' )
             CALL surface_data_output_sum_up( surf_def%z0, surf_lsm%z0, surf_usm%z0,               &
                                              surf_cct%z0, n_out )

          CASE ( 'z0h' )
             CALL surface_data_output_sum_up( surf_def%z0h, surf_lsm%z0h, surf_usm%z0h,            &
                                              surf_cct%z0h, n_out )

          CASE ( 'z0q' )
             CALL surface_data_output_sum_up( surf_def%z0q, surf_lsm%z0q, surf_usm%z0q,            &
                                              surf_cct%z0q, n_out )

          CASE ( 'theta1' )
             CALL surface_data_output_sum_up( surf_def%pt1, surf_lsm%pt1, surf_usm%pt1,            &
                                              surf_cct%pt1, n_out )

          CASE ( 'qv1' )
             CALL surface_data_output_sum_up( surf_def%qv1, surf_lsm%qv1, surf_usm%qv1,            &
                                              surf_cct%qv1, n_out )

          CASE ( 'thetav1' )
             CALL surface_data_output_sum_up( surf_def%vpt1, surf_lsm%vpt1, surf_usm%vpt1,         &
                                              surf_cct%vpt1, n_out )

          CASE ( 'usws' )
             CALL surface_data_output_sum_up( surf_def%usws, surf_lsm%usws, surf_usm%usws,         &
                                              surf_cct%usws, n_out, momentumflux_output_conversion )

          CASE ( 'vsws' )
             CALL surface_data_output_sum_up( surf_def%vsws, surf_lsm%vsws, surf_usm%vsws,         &
                                              surf_cct%vsws, n_out, momentumflux_output_conversion )

          CASE ( 'shf' )
             CALL surface_data_output_sum_up( surf_def%shf, surf_lsm%shf_agg, surf_usm%shf,        &
                                              surf_cct%shf, n_out, heatflux_output_conversion )

          CASE ( 'qsws' )
             CALL surface_data_output_sum_up( surf_def%qsws, surf_lsm%qsws_agg, surf_usm%qsws,     &
                                              surf_cct%qsws, n_out, waterflux_output_conversion )

          CASE ( 'ssws' )
             CALL surface_data_output_sum_up( surf_def%ssws, surf_lsm%ssws, surf_usm%ssws,         &
                                              surf_cct%ssws, n_out, scalarflux_output_conversion )

          CASE ( 'qcsws' )
             CALL surface_data_output_sum_up( surf_def%qcsws, surf_lsm%qcsws, surf_usm%qcsws,      &
                                              surf_cct%qcsws, n_out )

          CASE ( 'ncsws' )
             CALL surface_data_output_sum_up( surf_def%ncsws, surf_lsm%ncsws, surf_usm%ncsws,      &
                                              surf_cct%ncsws, n_out )

          CASE ( 'qisws' )
             CALL surface_data_output_sum_up( surf_def%qisws, surf_lsm%qisws, surf_usm%qisws,      &
                                              surf_cct%qisws, n_out )

          CASE ( 'nisws' )
             CALL surface_data_output_sum_up( surf_def%nisws, surf_lsm%nisws, surf_usm%nisws,      &
                                              surf_cct%nisws, n_out )

          CASE ( 'qrsws' )
             CALL surface_data_output_sum_up( surf_def%qrsws, surf_lsm%qrsws, surf_usm%qrsws,      &
                                              surf_cct%qrsws, n_out )

          CASE ( 'nrsws' )
             CALL surface_data_output_sum_up( surf_def%nrsws, surf_lsm%nrsws, surf_usm%nrsws,      &
                                              surf_cct%nrsws, n_out )

          CASE ( 'sasws' )
             CALL surface_data_output_sum_up( surf_def%sasws, surf_lsm%sasws, surf_usm%sasws,      &
                                              surf_cct%sasws, n_out )

          CASE ( 'q_surface' )
             CALL surface_data_output_sum_up( surf_def%q_surface, surf_lsm%q_surface,              &
                                              surf_usm%q_surface, surf_cct%q_surface, n_out )

          CASE ( 'theta_surface' )
             CALL surface_data_output_sum_up( surf_def%pt_surface, surf_lsm%pt_surface,            &
                                              surf_usm%pt_surface, surf_cct%pt_surface, n_out )

          CASE ( 'thetav_surface' )
             CALL surface_data_output_sum_up( surf_def%vpt_surface, surf_lsm%vpt_surface,          &
                                              surf_usm%vpt_surface, surf_cct%vpt_surface, n_out )

          CASE ( 'theta_10cm' )
             CALL surface_data_output_sum_up( surf_def%pt_10cm, surf_lsm%pt_10cm,                  &
                                              surf_usm%pt_10cm, surf_cct%pt_10cm, n_out )

          CASE ( 'rad_net' )
             CALL surface_data_output_sum_up( surf_def%rad_net, surf_lsm%rad_net,                  &
                                              surf_usm%rad_net, surf_cct%rad_net, n_out )

          CASE ( 'rad_lw_in' )
             CALL surface_data_output_sum_up( surf_def%rad_lw_in, surf_lsm%rad_lw_in,              &
                                              surf_usm%rad_lw_in, surf_cct%rad_lw_in, n_out )

          CASE ( 'rad_lw_out' )
             CALL surface_data_output_sum_up( surf_def%rad_lw_out, surf_lsm%rad_lw_out,            &
                                              surf_usm%rad_lw_out, surf_cct%rad_lw_out, n_out )

          CASE ( 'rad_sw_in' )
             CALL surface_data_output_sum_up( surf_def%rad_sw_in, surf_lsm%rad_sw_in,              &
                                              surf_usm%rad_sw_in, surf_cct%rad_sw_in, n_out )

          CASE ( 'rad_sw_out' )
             CALL surface_data_output_sum_up( surf_def%rad_sw_out, surf_lsm%rad_sw_out,            &
                                              surf_usm%rad_sw_out, surf_cct%rad_sw_out, n_out )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE surface_data_output_sum_up_1d( var_def, var_lsm, var_usm, var_cct, n_out, fac )
    ...
    REAL(wp), DIMENSION(:), ALLOCATABLE, INTENT(IN) ::  var_cct  !< output variable for cutcell-type surfaces
...
!-- position, increment the counter.
    IF ( cut_cell_topography )  THEN
       IF ( ALLOCATED( var_cct ) )  THEN
          DO  m = 1, surf_cct%ns
             n_surf                         = n_surf + 1
             k                              = surf_cct%k(m)
             surf_out%var_av(n_surf,n_out) = surf_out%var_av(n_surf,n_out) + var_cct(m) *          &
                                             conversion_factor(k)
          ENDDO
       ELSE
          n_surf = n_surf + surf_cct%ns
       ENDIF
    ELSE
...
!> approach.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE surface_data_output_sum_up_2d( var_def, var_lsm, var_usm, var_cct, n_out, fac )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE surface_data_output_collect_1d( var_def, var_lsm, var_usm, var_cct, fac )
    ...
    IF ( cut_cell_topography )  THEN
       IF ( ALLOCATED( var_cct ) )  THEN
          DO  m = 1, surf_cct%ns
             n_surf = n_surf + 1
             k      = surf_cct%k(m)
             surf_out%var_out(n_surf) = var_cct(m) * conversion_factor(k)
          ENDDO
       ELSE
          n_surf = n_surf + surf_cct%ns
       ENDIF
    ELSE
       IF ( ALLOCATED( var_def ) )  THEN
          DO  m = 1, surf_def%ns
             n_surf = n_surf + 1
!
!--          Distinguish between horizontal and vertical surfaces. Conversion of fluxes
!--          must not be applied to vertical surfaces.
             IF ( surf_def%upward(m)  .OR.  surf_def%downward(m) )  THEN
                k      = surf_def%k(m)
                surf_out%var_out(n_surf) = var_def(m) * conversion_factor(k)
             ELSE
                surf_out%var_out(n_surf) = var_def(m)
             ENDIF
       ...
       IF ( ALLOCATED( var_lsm ) )  THEN
          DO  m = 1, surf_lsm%ns
             ...
             ELSE
                surf_out%var_out(n_surf) = var_lsm(m)
             ENDIF
       ...
       IF ( ALLOCATED( var_usm ) )  THEN
          DO  m = 1, surf_usm%ns
             ...
             ELSE
                surf_out%var_out(n_surf) = var_usm(m)
             ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE surface_data_output_collect_2d( var_def, var_lsm, var_usm, var_cct, fac )
...
!--          must not be applied to vertical surfaces.
             IF ( surf_def%upward(m)  .OR.  surf_def%downward(m) )  THEN
                k      = surf_def%k(m)
                surf_out%var_out(n_surf) = SUM( surf_def%frac(m,:) * var_def(m,:) ) *                 &
                                           conversion_factor(k)
             ELSE
                surf_out%var_out(n_surf) = SUM( surf_def%frac(m,:) * var_def(m,:) )
             ENDIF
       ...
       IF ( ALLOCATED( var_lsm ) )  THEN
          DO  m = 1, surf_lsm%ns
             ...
             IF ( surf_lsm%upward(m)  .OR.  surf_lsm%downward(m) )  THEN
                k      = surf_lsm%k(m)
                surf_out%var_out(n_surf) = SUM( surf_lsm%frac(m,:) * var_lsm(m,:) ) *                 &
                                           conversion_factor(k)
             ELSE
                surf_out%var_out(n_surf) = SUM( surf_lsm%frac(m,:) * var_lsm(m,:) )
             ENDIF
       ...
       IF ( ALLOCATED( var_usm ) )  THEN
          DO  m = 1, surf_usm%ns
             ...
             IF ( surf_usm%upward(m)  .OR.  surf_usm%downward(m) )  THEN
                k      = surf_usm%k(m)
                surf_out%var_out(n_surf) = SUM( surf_usm%frac(m,:) * var_usm(m,:) ) *                 &
                                           conversion_factor(k)
             ELSE
                surf_out%var_out(n_surf) = SUM( surf_usm%frac(m,:) * var_usm(m,:) )
             ENDIF
