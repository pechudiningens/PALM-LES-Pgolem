...
!-- Serial mode does not allow nesting, but requires the following variables as steering quantities.
    ...
    LOGICAL, SAVE ::  nesting_coupling_time_reached = .TRUE.     !< flag to check if coupling time has been reached
    ...
    LOGICAL, SAVE ::  synchronize_timestep = .TRUE.              !< switch to control timestep synchronization
    ...
    REAL(wp), SAVE ::  nesting_coupling_time = 0.0_wp !< time for next data exchange between models

    PUBLIC nesting_coupling_time,                                                                  &
    ...
    USE dust_emission_and_transport_mod,                                                           &
        ONLY:  det_start_time,                                                                     &
               dm,                                                                                 &
               dm_2,                                                                               &
               n_dust_bins
    ...
    USE pmc_parent,                                                                                &
        ONLY:  pmc_p_clear_next_array_list,                                                        &
               pmc_p_fillbuffer,                                                                   &
               pmc_p_finalize,                                                                     &
               pmc_p_getbuffer,                                                                    &
               pmc_p_getnextarray,                                                                 &
...
!
!-- Geometry
    REAL(wp), SAVE, PUBLIC ::  child_nest_shift_z          !< child z-coordinate of the bottom of the domain
    REAL(wp), SAVE         ::  dt_3d_mean = HUGE( 1.0_wp ) !< mean time step to avoid big change in the last time step before coupling
...
!-- Children's parent-grid arrays
    ...
    REAL(wp), DIMENSION(:,:), ALLOCATABLE ::  det_p_init           !< parent-grid 1d profile on child domain - DET
    ...
    REAL(wp), SAVE, DIMENSION(:,:,:,:), ALLOCATABLE, TARGET ::  aerosol_mass_c    !< aerosol mass (parent grid on child domain)
    REAL(wp), SAVE, DIMENSION(:,:,:,:), ALLOCATABLE, TARGET ::  aerosol_number_c  !< aerosol number (parent grid on child domain)
    REAL(wp), SAVE, DIMENSION(:,:,:,:), ALLOCATABLE, TARGET ::  chem_spec_c       !< chemical species (parent grid on child domain)
    REAL(wp), SAVE, DIMENSION(:,:,:,:), ALLOCATABLE, TARGET ::  dust_mass_c       !< DET dust mass (parent grid on child domain)
...
!-- Variables for particle coupling
    ...
    INTERFACE pmci_check_dt_coupling
       MODULE PROCEDURE pmci_check_dt_coupling
    END INTERFACE pmci_check_dt_coupling
...
!-- Check steering parameter values.
    ...
    ENDIF
!
!-- Check for timestep synchronization.
    IF ( TRIM( nesting_mode ) == 'two-way'  .AND.  .NOT. synchronize_timestep )  THEN
       message_string = 'a two-way nested simulation in conjuction with ' //                       &
                        'synchronize_timestep = .F. is not allowed'
       CALL message( 'pmci_init', 'PMC0036', 3, 2, 0, 6, 0 )
    ENDIF
!
...
!-- pmci) out of the pmc-core.
    CALL pmc_get_model_info( comm_world_nesting = comm_world_nesting, cpl_id = cpl_id,             &
                             ...
                             child_nest_shift_z  = child_nest_shift_z,                             &
...
!
!-- Allocate memory for 2d surface arrays to be used for exchanging data in coupled runs.
    CALL surface_coupler_alloc_mem
!
!-- Determine the number of coupled arrays, store in pmc_max_array.
    CALL pmci_num_arrays
!
!-- Initialize the child (must be called before pmc_setup_parent)
!-- There is always an communication between parent and child
!-- The root model cannot be a child and will automatically start with pmci_setup_parent
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_setup_parent
    ...
    INTEGER(iwp) ::  lm = 1          !< running index for DET dust mass bins
...
!--          points along x or y must be a multiple of the total number of atmosphere grid points.
          ...
          ELSE
!
!--          In case of 3d nesting, check that the child domain is completely inside its parent
...
!--             Set some logical auxiliary parameters to simplify the IF-condition.
                ...
                IF ( ( m_left_in_msib   .OR.  m_right_in_msib  .OR.                                &
                     ...
                     ( m_south_in_msib  .OR.  m_north_in_msib  .OR.                                &
                       msib_south_in_m  .OR.  msib_north_in_m ) )                                  &
                THEN
...
!--       Also send the dzu-, dzw-, zu- and zw-arrays here
          ...
          CALL pmc_send_to_child( child_id, zu(kz_child_start-1:kz_child_end+1), nz_child + 2, 0,  &
                                  28, ierr )
          CALL pmc_send_to_child( child_id, zw(kz_child_start-1:kz_child_end+1), nz_child + 2, 0,  &
                                  29, ierr )
!
...
!--    specified again.
       CALL pmc_p_clear_next_array_list
       DO WHILE ( pmc_p_getnextarray( child_id, myname ) )
          ...
          ELSEIF ( INDEX( TRIM( myname ), 'sg_' ) /= 0  .AND.  .NOT. salsa_gases_from_chem )  THEN
             CALL pmci_set_array_pointer( myname, child_id = child_id, nz_child = nz_child,        &
                                          ...
                                          n = lg )
             lg = lg + 1
          ELSEIF ( INDEX( TRIM( myname ), 'dm_' ) /= 0 )  THEN
             CALL pmci_set_array_pointer( myname, child_id = child_id, nz_child = nz_child,        &
                                          ks_child = kz_child_start, ke_child = kz_child_end,      &
                                          n = lm )
             lm = lm + 1
       ...
       CALL pmc_p_setind_and_allocmem( child_id )
...
!> The index list provides the connection between the i,j values and PE number of every child
!> 2d grid point and the correponding i,j values and PE number of the parent.
!> Every entry in this list represents the connection of one grid point pair.
!--------------------------------------------------------------------------------------------------!
...
!--    Loop over all children PEs.
       DO  n = 1, size_of_childs_parent_grid_bounds_all(2)
!
!--       Subspace along y required by actual child PE (jp = jps, jpn of child PE# n).
          DO  jp = childs_parent_grid_bounds_all(3,n), childs_parent_grid_bounds_all(4,n)
!
!--          Subspace along x required by actual child PE (ip = ipl, ipr of child PE# n).
             DO  ip = childs_parent_grid_bounds_all(1,n), childs_parent_grid_bounds_all(2,n)

                IF ( non_uniform_subdomain )  THEN
!
!--                Loop indices pex and pey are at the point of exit are used later on.
                   DO  pex = 0, npex-1
                      IF ( ip >= nxl_pe(pex)  .AND.  ip <= nxr_pe(pex) )  EXIT
                   ENDDO
                   DO  pey = 0, npey-1
                      IF ( jp >= nys_pe(pey)  .AND.  jp <= nyn_pe(pey) )  EXIT
                   ENDDO
                ELSE
                   pex = ip / nrx
                   pey = jp / nry
...
!
!--             Index j of child's parent-grid array (e.g. uc).
...
!--             PE number of parent.
       ...
       ENDDO
!
!--    The index list is calculated only on PE0. The following subroutine distributes the index list
!--    to all parent PEs involved in the parent-child transfer.
       CALL pmc_p_split_and_broadcast_index_list( child_id, index_list(:,1:ilist) )

    ELSE
!
!--    pmc_p_split_and_broadcast_index_list is called on all parent processes. Although index_list
!--    is only used on PE 0 inside this subroutine, the array should be allocated to avoid problems
!--    during the calling sequence.
       ALLOCATE( index_list(6,1) )
       CALL pmc_p_split_and_broadcast_index_list( child_id, index_list )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_set_child_edge_coords
    ...
    INTEGER(iwp) ::  nbgp_lpm = 1  !< number of ghost-point layers used for lpm
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_setup_child
    ...
    CHARACTER(LEN=5)          ::  det_char    !< name extension for the variable name in case of DET variable
    ...
    INTEGER(iwp) ::  lm    !< running index for DET dust mass bins
...
!-- Root model does not have a parent and is not a child, therefore no child setup on root model.
    IF ( .NOT. root_model )  THEN
!
!--    Get/define the child's MPI environment, create the intra communicator and allocate me%pes and
!--    me%pes%array_list.
       CALL pmc_c_childinit

       IF ( atmosphere_ocean_coupled_run )  THEN

          CALL pmc_c_set_dataarray_name( 'exchange_array_1' )
          CALL pmc_c_set_dataarray_name( 'exchange_array_2' )
          CALL pmc_c_set_dataarray_name( 'exchange_array_3' )
          CALL pmc_c_set_dataarray_name( 'exchange_array_4' )
...
!--       pmci_create_childs_parent_grid_arrays (for child's parent-grid arrays)
          CALL pmc_c_set_dataarray_name( 'u' )
          CALL pmc_c_set_dataarray_name( 'v' )
          CALL pmc_c_set_dataarray_name( 'w' )
!
...
!--       shall be interpolated / anterpolated is not available yet.
          CALL pmc_c_set_dataarray_name( 'e' )
!
...
!--       closure is applied. Please see also comment for TKE above.
          CALL pmc_c_set_dataarray_name( 'diss' )

          IF ( .NOT. neutral )  THEN
             CALL pmc_c_set_dataarray_name( 'pt' )
          ENDIF

          IF ( humidity )  THEN

             CALL pmc_c_set_dataarray_name( 'q' )

             IF ( bulk_cloud_model  .AND.  microphysics_morrison )  THEN
                CALL pmc_c_set_dataarray_name( 'qc' )
                CALL pmc_c_set_dataarray_name( 'nc' )
             ENDIF

             IF ( bulk_cloud_model  .AND.  microphysics_seifert )  THEN
                CALL pmc_c_set_dataarray_name( 'qr' )
                CALL pmc_c_set_dataarray_name( 'nr' )
             ENDIF

          ENDIF

          IF ( passive_scalar )  THEN
             CALL pmc_c_set_dataarray_name( 's' )
          ENDIF

          IF ( ocean_mode  .AND.  salinity )  THEN
             CALL pmc_c_set_dataarray_name( 'sa' )
          ENDIF

          IF ( particle_advection )  THEN
             CALL pmc_c_set_dataarray_name( 'nr_part' )
             CALL pmc_c_set_dataarray_name( 'part_adr' )
          ENDIF

          IF ( air_chemistry  .AND.  nesting_chem )  THEN
             DO  n = 1, nspec
                CALL pmc_c_set_dataarray_name( 'chem_' // TRIM( chem_species(n)%name ) )
             ENDDO
          ENDIF
          ...
          IF ( salsa  .AND.  nesting_salsa )  THEN
             DO  lb = 1, nbins_aerosol
                ...
                CALL pmc_c_set_dataarray_name( 'an_' // TRIM( salsa_char ) )
             ENDDO
             DO  lc = 1, nbins_aerosol * ncomponents_mass
                WRITE( salsa_char, '(I0)' )  lc
                CALL pmc_c_set_dataarray_name( 'am_' // TRIM( salsa_char ) )
             ENDDO
             IF ( .NOT. salsa_gases_from_chem )  THEN
                DO  lg = 1, ngases_salsa
                   WRITE( salsa_char, '(I0)' )  lg
                   CALL pmc_c_set_dataarray_name( 'sg_' // TRIM( salsa_char ) )
                ENDDO
             ENDIF
          ENDIF
          ...
          IF ( det_enabled )  THEN
             DO  lm = 1, n_dust_bins
                WRITE( det_char, '(I0)' )  lm
                CALL pmc_c_set_dataarray_name( 'dm_' // TRIM( det_char ) )
             ENDDO
          ENDIF

       ENDIF

       CALL pmc_c_set_dataarray_name( lastentry = .TRUE. )
...
!--       Bottom condition for elevated childs. Then the bottom boundary is "open".
          ...
          ENDIF
!
!--       Child top boundary is always "open" (nested) unless in the ocean mode.
...
!
!--    Allocate 1d arrays for parent-grid coordinates and grid-spacings in the z-direction.
...
!
!--    Get index List generated on parent side.
       CALL pmc_c_get_2d_index_list
!
!--    Create and allocate receive arrays in parent context (e.g.uc).
...
!--       for nesting. Note that in case of chemical species, SALSA, or DET variables an additional
!--       parameter needs to be passed. The parameter is required to set the pointer correctly to
!--       the chemical-species, SALSA, or DET data structure. Hence, first check if the current
!--       variable is a chemical species, SALSA, or DET variable. If so, pass index id of respective
!--       sub-variable (species or bin) and increment this subsequently.
          IF ( INDEX( TRIM( myname ), 'chem_' ) /= 0 )  THEN
          ...
          ELSEIF ( INDEX( TRIM( myname ), 'sg_' ) /= 0  .AND.  .NOT.  salsa_gases_from_chem )  THEN
             ...
             lg = lg + 1
          ELSEIF ( INDEX( TRIM( myname ), 'dm_' ) /= 0 )  THEN
             CALL pmci_create_childs_parent_grid_arrays( myname, ipl, ipr, jps, jpn, pg%nz, lm )
             lm = lm + 1
          ELSE
...
!> The 2d-boundary data to be exchanged are not contiguous in general, so derived types
!> are required to simplify the exchange.
!--------------------------------------------------------------------------------------------------!
...
! Description:
! ------------
!> Transform the coordinates of the current domain into the root coordinate system and store them
!> (z remains the same).
!--------------------------------------------------------------------------------------------------!
...
!
!-- SALSA, depends on the number aerosol size bins and chemical components + the number of default
!-- gases.
...
!
!-- DET, depends on the number of dust mass bins.
    IF ( det_enabled )  pmc_max_array = pmc_max_array + n_dust_bins
...
! Description:
! ------------
!> Assigns a temporary pointer to the array to be coupled. This routine is called separately
!> for each array. The temporary pointer is passed to pmc_p_set_dataarray where it is assigned to
!> the respective variable in the transfer data structure.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_set_array_pointer( name, child_id, nz_child, ks_child, ke_child, n )
    ...
    INTEGER(iwp), INTENT(IN), OPTIONAL ::  n         !< index of chemical species, SALSA-, or DET-bin
...
!-- In case of 3d please change also the second array for the pointer version.
    ...
    IF ( INDEX( TRIM(name), 'dm_' ) /= 0 )  p_3d => dm(n)%conc

!
!-- Next four lines are for ocean-atmosphere coupling only.
    ...
    IF ( INDEX( TRIM(name), 'sg_' )   /= 0  .AND.  .NOT.  salsa_gases_from_chem )                  &
                                 p_3d_sec => gconc_2(:,:,:,n)
    IF ( INDEX( TRIM(name), 'dm_' )   /= 0 )  p_3d_sec => dm_2(:,:,:,n)

!
!-- Now the temporary pointer is assigned to the respective variable in the transfer data structure.
    IF ( ASSOCIATED( p_3d ) )  THEN
       CALL pmc_p_set_dataarray( child_id, p_3d, nz_child, nz, array_2 = p_3d_sec, ks_cl=ks_child, &
                                 ke_cl = ke_child )
    ELSEIF  ( ASSOCIATED( p_2d ) )  THEN
       CALL pmc_p_set_dataarray( child_id, p_2d )
    ELSEIF  ( ASSOCIATED( i_2d ) )  THEN
       CALL pmc_p_set_dataarray( child_id, i_2d )
    ELSE
!
!--    Give only one message for the root domain to avoid multiple messages in the output.
...
! Description:
! ------------
!> Allocate child's parent-grid arrays (e.g. uc) for data transfer between parent and child.
!> Assigns a temporary pointer to the array to be coupled. This routine is called separately
!> for each array. The temporary pointer is passed to pmc_c_set_dataarray where it is assigned to
!> the respective variable in the transfer data structure.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_create_childs_parent_grid_arrays( name, is, ie, js, je, nzc, n  )
    ...
    INTEGER(iwp), INTENT(IN), OPTIONAL ::  n  !< number of chemical species / SALSA- / DET-bin
...
!-- List of array names, which can be coupled.
    ...
    ELSEIF ( TRIM( name(1:3) ) == 'sg_'  .AND.  .NOT. salsa_gases_from_chem )  THEN
       ...
       p_3d => salsa_gas_c(:,:,:,n)
    ELSEIF ( TRIM( name(1:3) ) == 'dm_' )  THEN
       IF ( .NOT. ALLOCATED( dust_mass_c ) )                                                       &
          ALLOCATE( dust_mass_c(0:nzc+1,js:je,is:ie,1:n_dust_bins) )
       p_3d => dust_mass_c(:,:,:,n)
    ENDIF
...
!
!--    No initialization in case of restarts, except that the respective child is activated in
!--    restart for the first time.
       IF ( TRIM( initializing_actions ) == 'read_restart_data'  .AND.                             &
            .NOT. ( child_id == init_child_id ) )                                                  &
       THEN
          CYCLE
       ENDIF
...
!--       These are later used to initialize the corresponding variable in the child-domain.
       ...
       ELSE
          CALL pmc_p_fillbuffer( child_id )
       ENDIF
...
!-- SGS-TKE.
    IF ( ( rans_mode_parent  .AND.         rans_mode )  .OR.                                       &
       ...
       IF ( myid == 0 )  THEN
          CALL pmc_send_to_child( child_id, mean_profile, nz_child+2, 0, tag_nr, ierr )
       ENDIF
       tag_nr = tag_nr + 1
    ENDIF
!
...
!-- DET dust mass.
    IF ( det_enabled )  THEN
       DO  n = 1, n_dust_bins
          CALL pmci_compute_average( dm(n)%conc, m, 0, nz_child, mean_profile, 's' )
          IF ( myid == 0 )  THEN
             CALL pmc_send_to_child( child_id, mean_profile, nz_child+2, 0, tag_nr, ierr )
          ENDIF
          tag_nr = tag_nr + 1
...
! Description:
! ------------
!> Compute domain-averaged profile of the given array.
!--------------------------------------------------------------------------------------------------!
       ...
       DO  i = nxl, nxr
          DO  j = nys, nyn
             ...
             DO  k = nzb, nz_child+1
                ...
                num_gp(k) = num_gp(k) + MERGE( 1_iwp, 0_iwp, BTEST( topo_flags(k,j,i), bit_nr ) ) *&
          ...
          DO  i = nxl, nxr
             DO  j = nys, nyn
                ...
                DO  k = nzb, nz_child+1
                   ...
                   num_gp(k) = num_gp(k) + MERGE( 1_iwp, 0_iwp, within_child_area )
                ENDDO
             ENDDO
          ENDDO
       ENDIF
...
!--       DET dust mass.
          IF ( det_enabled )  THEN
             DO  n = 1, n_dust_bins
                CALL pmci_interp_1d( dm(n)%conc, det_p_init(:,n), kcto, kflo, kfuo )
             ENDDO
          ENDIF
!
...
! Description:
! ------------
!> Child-domain initialization using 3d interpolation from the parent domain.
!--------------------------------------------------------------------------------------------------!
...
!-- DET dust mass.
    IF ( det_enabled )  THEN
       ALLOCATE( det_p_init(0:pg%nz+1,1:n_dust_bins) )
       IF ( myid == 0 )  THEN
          DO  n = 1, n_dust_bins
             CALL pmc_recv_from_parent( det_p_init(:,n), pg%nz+2, 0, tag_nr, ierr )
             tag_nr = tag_nr + 1
          ENDDO
       ENDIF
    ENDIF
...
!-- Broadcast the initial profiles among all child cores.
    IF ( ALLOCATED( u_p_init    ) )  CALL MPI_BCAST( u_p_init,    pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( v_p_init    ) )  CALL MPI_BCAST( v_p_init,    pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( e_p_init    ) )  CALL MPI_BCAST( e_p_init,    pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( diss_p_init ) )  CALL MPI_BCAST( diss_p_init, pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( pt_p_init   ) )  CALL MPI_BCAST( pt_p_init,   pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( q_p_init    ) )  CALL MPI_BCAST( q_p_init,    pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( qc_p_init   ) )  CALL MPI_BCAST( qc_p_init,   pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( nc_p_init   ) )  CALL MPI_BCAST( nc_p_init,   pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( qr_p_init   ) )  CALL MPI_BCAST( qr_p_init,   pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( nr_p_init   ) )  CALL MPI_BCAST( nr_p_init,   pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    IF ( ALLOCATED( s_p_init    ) )  CALL MPI_BCAST( s_p_init,    pg%nz+2, MPI_REAL, 0, comm2d,    &
                                                     ierr )
    ...
    IF ( ALLOCATED( det_p_init ) )                                                                &
       CALL MPI_BCAST( det_p_init, (pg%nz+2)*n_dust_bins, MPI_REAL, 0, comm2d, ierr )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_check_setting_mismatches
    ...
    REAL(wp) ::  end_time_child            !< child value of variable
...
!-- respective variable to all others, and its value will then be compared with the local values.
    IF ( root_model )  THEN
       end_time_root = end_time
       IF ( atmosphere_run_coupled_to_ocean )  end_time_root = end_time_root - coupling_start_time
    ENDIF

    CALL MPI_BCAST( end_time_root, 1, MPI_REAL, 0, comm_world_nesting, ierr )

    IF ( .NOT. root_model )  THEN
       end_time_child = end_time
       IF ( ocean_run_coupled_to_atmosphere )  end_time_child = end_time_child - coupling_start_time
       IF ( end_time_child /= end_time_root )  THEN
          WRITE( message_string, * )  'mismatch between root model and child settings:& ' //       &
                                      'end_time(root) = ', end_time_root,                          &
                                      '& end_time(child) = ', end_time_child, '& child value ',    &
                                      'is set to root value'
          CALL message( 'pmci_check_setting_mismatches', 'PMC0021', 0, 1, 0, 6, 0 )
          end_time = end_time_root
          IF ( ocean_run_coupled_to_atmosphere )  end_time = end_time + coupling_start_time
       ENDIF
    ENDIF
!
...
!
!-- Dust emission and transport requires to be set in all models in the same way.
    IF ( root_model )  det_enabled_root = det_enabled
    CALL MPI_BCAST( det_enabled_root, 1, MPI_LOGICAL, 0, comm_world_nesting, ierr )

    IF ( .NOT. root_model )  THEN
       IF ( det_enabled .NEQV. det_enabled_root )  THEN
          message_string = 'mismatch concerning activation of dust emission and transport &' //    &
                           'between root model and child(s)'
          CALL message( 'pmci_check_setting_mismatches', 'PMC0033', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF

    IF ( det_enabled )  THEN
!
!--    Number of dust bins must match.
       IF ( root_model )  n_dust_bins_root = n_dust_bins
       CALL MPI_BCAST( n_dust_bins_root, 1, MPI_INTEGER, 0, comm_world_nesting, ierr )

       IF ( .NOT. root_model )  THEN
          IF ( n_dust_bins /= n_dust_bins_root )  THEN
             message_string = 'mismatch of n_dust_bins between root model and child(s)'
             CALL message( 'pmci_check_setting_mismatches', 'PMC0034', 1, 2, 0, 6, 0 )
          ENDIF
       ENDIF
!
!--    Adjust DET start time.
       IF ( root_model )  det_start_time_root = det_start_time
       CALL MPI_BCAST( det_start_time_root, 1, MPI_REAL, 0, comm_world_nesting, ierr )

       IF ( .NOT. root_model )  THEN
          IF ( det_start_time /= det_start_time_root )  THEN
             WRITE( message_string, * )  'mismatch between root model and ',                       &
                                         'child settings: & dt_restart(root) = ', dt_restart_root, &
                                         '& dt_restart(child) = ', dt_restart, '& child ',         &
                                         'value is set to root value'
             CALL message( 'pmci_check_setting_mismatches', 'PMC0021', 0, 1, 0, 6, 0 )
             det_start_time = det_start_time_root
          ENDIF
       ENDIF
...
!-- In ocean mode salinity requires to be set in all models in the same way.
    ...
    CALL MPI_BCAST( salinity_root, 1, MPI_LOGICAL, 0, comm_world_nesting, ierr )
...
! Description:
! ------------
!> The subroutine controls the timestep-coupling of the model domains. Either timesteps among all
!> domains are unified (synchronized) using an MPI_ALLREDUCE with the MPI_MIN operator over all
!> processes using the global communicator MPI_COMM_WORLD, or in case of asynchronuous coupling
!> (1-way nesting only), the coupling timesteps are computed and control flags are set accordingly.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_synchronize
    ...
    INTEGER(iwp) ::  ierr    !< MPI error code
    INTEGER(iwp) ::  nsteps  !< number of time steps that complete fit into the maximum time step
    ...
    REAL(wp) ::  dt_last      !< estimated length of last time step before coupling
    REAL(wp) ::  dt_max_pmci  !< maximum timestep among all processes


    IF ( debug_output_timestep )  CALL debug_message( 'pmci_synchronize', 'start' )
!
!-- Synchronuous coupling. Unify time step among all domains.
    IF ( synchronize_timestep )  THEN
...
!--    the stop flag (due to too small timestep), i.e. they need to set the stop flag, too. The
!--    stop_dt flag has been computed for each domain individually in routine timestep.
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, stop_dt, 1, MPI_LOGICAL, MPI_LOR, MPI_COMM_WORLD, ierr )
!
!-- Asynchronuous coupling. This case, it is checked if all domains have already reached the
!-- coupling timestep. If not, models that have already reached that point need to wait until
!-- all domains arrive at this point.
    ELSE
!
!--    Check if the domain has already reached the coupling timestep.
!--    Due to round-off errors time_since_reference_point (which is calculated by integrating the
!--    local timesteps in time_integration) may slightly vary from nesting_coupling_time at the
!--    end on some of the nests, which would cause calculation of an additional artificial
!--    negligible timestep. As this timestep is effectively zero and fp imprecision is a random
!--    value, the timestepping in this domain cycles in an unfinal loop on one time.
       IF ( ABS( time_since_reference_point - nesting_coupling_time ) < 1.0E-9_wp )  THEN
          nesting_coupling_time_reached = .TRUE.
       ELSE
          nesting_coupling_time_reached = .FALSE.
       ENDIF
!
!--    If the domain has already reached the coupling timestep, it may need to wait until
!--    all domains within the nested setup arrive at this point. When all models have arrived
!--    that point, compute the next coupling time by using the largest timestep among all
!--    model domains.
       IF ( nesting_coupling_time_reached )  THEN
...
!
!--       In case of a nested run all parent/child processes have to terminate if one process has
!--       set the stop flag (due to too small timestep), i.e. they need to set the stop flag too.
!--       The stop_dt flag has been computed for each domain individually in routine timestep.
          CALL MPI_ALLREDUCE( MPI_IN_PLACE, stop_dt, 1, MPI_LOGICAL, MPI_LOR, MPI_COMM_WORLD, ierr )
!
!--       Calculate a mean value to be used for all time steps until next coupling.
!--       This is to avoid a possible big change in the last time step before coupling.
          nsteps = INT( dt_max_pmci / dt_3d )
          dt_last = dt_max_pmci - nsteps * dt_3d
          IF ( ( dt_max_pmci /= dt_3d )  .AND.  ( dt_last < 0.99 * dt_3d ) )  THEN
             dt_3d_mean = dt_max_pmci / REAL( nsteps + 1, KIND = wp )
             dt_3d = dt_3d_mean
          ELSE
             dt_3d_mean = dt_3d
          ENDIF
!
!--    If the domain has not yet reached the coupling timestep, it need to further time-integrate
!--    until it reaches nesting_coupling_time. To exactly match that point in time, the timestep
!--    possibly requires an adjustment.
       ELSE
!
!--       Take care that the time step criterion is not violated by dt_3d_mean.
          dt_3d = MIN( dt_3d_mean, dt_3d )
!
!--       Adjust time step to match the coupling time exactly, in order to avoid possible
!--       very small last time steps. An increase of the time step by 5% should not violate
!--       the time step criterion too much.
          dt_last = ABS( time_since_reference_point + dt_3d - nesting_coupling_time )
          IF ( dt_last < 0.05_wp * dt_3d )  THEN
             dt_3d = nesting_coupling_time - time_since_reference_point
          ELSE
             dt_3d = MIN( dt_3d, nesting_coupling_time - time_since_reference_point )
          ENDIF
       ENDIF
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_set_swaplevel( swaplevel )
    ...
    DO  m = 1, SIZE( pmc_parent_for_child ) - 1
       child_id = pmc_parent_for_child(m)
       CALL pmc_p_set_active_data_array( child_id, swaplevel )
    ENDDO
#endif
...
! Description:
! ------------
!> This subroutine does the surface coupling between atmosphere and ocean.
!--------------------------------------------------------------------------------------------------!
...
!--    Interpolation from ocean-grid to atmosphere-grid.
       DO  i = ipl, ipr
          DO  j = jps, jpn
             cellsum  = 0.0_wp
             icellsum = 0
             DO  ii = i*dx_nr, MIN( (i+1)*dx_nr-1, nxrg )
                DO  jj = j*dy_nr, MIN( (j+1)*dy_nr-1, nyng )
                   cellsum  = cellsum + surface_coupler_exchange_array(jj,ii)
                   icellsum = icellsum + 1
                ENDDO
             ENDDO
             surface_coupler_exchange_array_parent(j,i) = cellsum / REAL( icellsum )
          ENDDO
       ENDDO
    ENDIF
...
! Description:
! ------------
!> Check the time interval after which atmosphere and ocean are coupled, to be at least
!> as large as the maximum timestep of the atmosphere and the ocean model.
!> Violation of this condition leads to hanging situations at the end of a run.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_check_dt_coupling

#if defined( __parallel )
    IMPLICIT NONE


    IF ( dt_3d > dt_coupling )  THEN
       WRITE( message_string, '(A,F8.2,A,F6.2)' )  'dt_3d = ', dt_3d, ' exceeds dt_coupling = ',   &
                                                   dt_coupling
       CALL message( 'pmci_check_dt_coupling', 'PMC0035', 3, 2, 0, 6, 0 )
    ENDIF
#endif

 END SUBROUTINE pmci_check_dt_coupling
...
!> Note that interpolation ( parent_to_child ) must always be carried out
!> before anterpolation ( child_to_parent ).
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_datatrans
...
!--          since it would overrun the default Neumann condition (bc_e_b).
             DO   i = nxlg, nxrg
                DO   j = nysg, nyng
                   DO  k = nzb, nzt+1
...
!--    else, interpolation is replaced by a Neumann condition.
       ...
       IF ( det_enabled )  THEN
          DO  lm = 1, n_dust_bins
             CALL pmci_interp_lr( dm(lm)%conc, dust_mass_c(:,:,:,lm), kcto, jflo, jfuo,            &
                                  kflo, kfuo, 'l', 's' )
          ENDDO
       ENDIF
...
!--    else, interpolation is replaced by a Neumann condition.
       ...
       IF ( det_enabled )  THEN
          DO  lm = 1, n_dust_bins
             CALL pmci_interp_sn( dm(lm)%conc, dust_mass_c(:,:,:,lm), kcto, iflo, ifuo,            &
                                  kflo, kfuo, 's', 's' )
          ENDDO
       ENDIF
...
!--    else, interpolation is replaced by a Neumann condition.
       ...
       IF ( det_enabled )  THEN
          DO  lm = 1, n_dust_bins
             CALL pmci_interp_bt( dm(lm)%conc, dust_mass_c(:,:,:,lm), kcto, iflo, ifuo,            &
                                  jflo, jfuo, 't', 's' )
          ENDDO
       ENDIF
...
!--    else, interpolation is replaced by a Neumann condition.
       ...
       IF ( det_enabled )  THEN
          DO  n = 1, n_dust_bins
             CALL pmci_interp_bt( dm(n)%conc, dust_mass_c(:,:,:,n), 0, iflo, ifuo,                 &
                                  jflo, jfuo, 'b', 's' )
          ENDDO
       ENDIF
...
! Description:
! ------------
!> A wrapper routine for all anterpolation actions. Note that in LES mode SGS-TKE is not
!> anterpolated.
!--------------------------------------------------------------------------------------------------!
...
!
!-- Anterpolation of TKE and dissipation rate if parent and child are in RANS mode.
    IF ( rans_mode_parent  .AND.  rans_mode )  THEN
...
!--    Anterpolation of dissipation rate only if TKE-e closure is applied.
    ...
    IF ( det_enabled )  THEN
       DO  lm = 1, n_dust_bins
          CALL pmci_anterp_var( dm(lm)%conc, dust_mass_c(:,:,:,lm), kcto, iflo, ifuo,              &
                                jflo, jfuo, kflo, kfuo, ijkfc_s, 's' )
       ENDDO
    ENDIF
...
!--    3d nesting the south (psouth == MPI_PROC_NULL) and north (pnorth == MPI_PROC_NULL)
...
!--    3d nesting the left (pleft == MPI_PROC_NULL) and right (pright == MPI_PROC_NULL) boundaries
...
!-- location of the PE-subdomain relative to the side boundaries.
    ...
    jpsc = jpsw + 1
    jpnc = jpnw - 1
    IF ( bc_dirichlet_l  .AND.  .NOT. nesting_bounds_vertical_only )  iplc = iplw
    IF ( bc_dirichlet_r  .AND.  .NOT. nesting_bounds_vertical_only )  iprc = iprw
    IF ( bc_dirichlet_s  .AND.  .NOT. nesting_bounds_vertical_only )  jpsc = jpsw
    IF ( bc_dirichlet_n  .AND.  .NOT. nesting_bounds_vertical_only )  jpnc = jpnw
    workarr_bt = 0.0_wp
!
...
!
!-- Left-right exchange if more than one PE subdomain in the x-direction. Note that in case of
!-- 3d nesting the left and right boundaries are not exchanged because the nest domain is not
!-- cyclic.
    IF ( npex > 1 )  THEN
!
!--    From left to right.
...
!-- Note that in case of 3d nesting the south and north boundaries are not exchanged because the
...
!> TODO: add Dirichlet boundary conditions for pot. temperature, humididity and passive scalar.
!--------------------------------------------------------------------------------------------------!
...
!-- Set Neumann boundary conditions for aerosols and salsa gases.
    ...
    ENDIF
!
!-- Set Neumann boundary conditions for DET dust mass bins.
    IF ( det_enabled )  THEN
       DO  n = 1, n_dust_bins
          DO  m = 1, bc_hv%ns
             IF ( bc_hv%koff(m) /= 0  )  THEN
                ...
                jc = bc_hv%j(m)
                kc = bc_hv%k(m)
                dm(n)%conc(kc+bc_hv%koff(m),jc+bc_hv%joff(m),ic+bc_hv%ioff(m)) =                   &
                                                                               dm(n)%conc(kc,jc,ic)
             ENDIF
          ENDDO
       ENDDO
    ENDIF
!
!-- In case of asynchronuous timesteps, make sure that boundary conditions are kept constant
!-- at all Runge-Kutta substeps. This requires that also boundary conditions for _p arrays need to
!-- be set.
    IF ( .NOT. synchronize_timestep  .AND.  child_domain )  THEN
       IF ( bc_dirichlet_l )  THEN
          u_p(:,:,nxl-1:nxl) = u(:,:,nxl-1:nxl)
          v_p(:,:,nxl-1) = v(:,:,nxl-1)
          w_p(:,:,nxl-1) = w(:,:,nxl-1)
          IF ( .NOT. neutral )   pt_p(:,:,nxl-1) = pt(:,:,nxl-1)
          IF ( humidity )        q_p(:,:,nxl-1) = q(:,:,nxl-1)
          IF ( passive_scalar )  s_p(:,:,nxl-1) = s(:,:,nxl-1)
          IF ( bulk_cloud_model  .AND.  microphysics_morrison )  THEN
             nc_p(:,:,nxl-1) = nc(:,:,nxl-1)
             qc_p(:,:,nxl-1) = qc(:,:,nxl-1)
          ENDIF
          IF ( bulk_cloud_model  .AND.  microphysics_seifert )  THEN
             ...
             qr_p(:,:,nxl-1) = qr(:,:,nxl-1)
          ENDIF
          IF ( ocean_mode  .AND.  salinity )  sa_p(:,:,nxl-1) = sa(:,:,nxl-1)
          IF ( air_chemistry  .AND.  nesting_chem )  THEN
             DO  n = 1, nspec
          ...
          IF ( salsa  .AND.  nesting_salsa )  THEN
             DO  lb = 1, nbins_aerosol
                aerosol_number(lb)%conc_p(:,:,nxl-1) = aerosol_number(lb)%conc(:,:,nxl-1)
             ENDDO
             DO  lc = 1, nbins_aerosol * ncomponents_mass
                aerosol_mass(lc)%conc_p(:,:,nxl-1) = aerosol_mass(lc)%conc(:,:,nxl-1)
             ENDDO
             IF ( .NOT. salsa_gases_from_chem )  THEN
                DO  lg = 1, ngases_salsa
                   salsa_gas(lg)%conc_p(:,:,nxl-1) = salsa_gas(lg)%conc(:,:,nxl-1)
                ENDDO
             ENDIF
          ENDIF
       ENDIF
       ...
       IF ( bc_dirichlet_r )  THEN
          ...
          IF ( passive_scalar )  s_p(:,:,nxr+1) = s(:,:,nxr+1)
          IF ( bulk_cloud_model  .AND.  microphysics_morrison )  THEN
             nc_p(:,:,nxr+1) = nc(:,:,nxr+1)
             qc_p(:,:,nxr+1) = qc(:,:,nxr+1)
          ENDIF
          IF ( bulk_cloud_model  .AND.  microphysics_seifert )  THEN
             ...
             qr_p(:,:,nxr+1) = qr(:,:,nxr+1)
          ENDIF
          IF ( ocean_mode  .AND.  salinity )  sa_p(:,:,nxr+1) = sa(:,:,nxr+1)
          IF ( air_chemistry  .AND.  nesting_chem )  THEN
             DO  n = 1, nspec
          ...
          IF ( salsa  .AND.  nesting_salsa )  THEN
             DO  lb = 1, nbins_aerosol
                aerosol_number(lb)%conc_p(:,:,nxr+1) = aerosol_number(lb)%conc(:,:,nxr+1)
             ENDDO
             DO  lc = 1, nbins_aerosol * ncomponents_mass
                aerosol_mass(lc)%conc_p(:,:,nxr+1) = aerosol_mass(lc)%conc(:,:,nxr+1)
             ENDDO
             IF ( .NOT. salsa_gases_from_chem )  THEN
                DO  lg = 1, ngases_salsa
                   salsa_gas(lg)%conc_p(:,:,nxr+1) = salsa_gas(lg)%conc(:,:,nxr+1)
                ENDDO
             ENDIF
          ENDIF
       ENDIF
       ...
       IF ( bc_dirichlet_n )  THEN
          ...
          IF ( passive_scalar )  s_p(:,nyn+1,:) = s(:,nyn+1,:)
          IF ( bulk_cloud_model  .AND.  microphysics_morrison )  THEN
             nc_p(:,nyn+1,:) = nc(:,nyn+1,:)
             qc_p(:,nyn+1,:) = qc(:,nyn+1,:)
          ENDIF
          IF ( bulk_cloud_model  .AND.  microphysics_seifert )  THEN
             ...
             qr_p(:,nyn+1,:) = qr(:,nyn+1,:)
          ENDIF
          IF ( ocean_mode  .AND.  salinity )  sa_p(:,nyn+1,:) = sa(:,nyn+1,:)
          IF ( air_chemistry  .AND.  nesting_chem )  THEN
             DO  n = 1, nspec
          ...
          IF ( salsa  .AND.  nesting_salsa )  THEN
             DO  lb = 1, nbins_aerosol
                aerosol_number(lb)%conc_p(:,nyn+1,:) = aerosol_number(lb)%conc(:,nyn+1,:)
             ENDDO
             DO  lc = 1, nbins_aerosol * ncomponents_mass
                aerosol_mass(lc)%conc_p(:,nyn+1,:) = aerosol_mass(lc)%conc(:,nyn+1,:)
             ENDDO
             IF ( .NOT. salsa_gases_from_chem )  THEN
                DO  lg = 1, ngases_salsa
                   salsa_gas(lg)%conc_p(:,nyn+1,:) = salsa_gas(lg)%conc(:,nyn+1,:)
                ENDDO
             ENDIF
          ENDIF
       ENDIF
       ...
       IF ( bc_dirichlet_s )  THEN
          ...
          IF ( bulk_cloud_model  .AND.  microphysics_seifert )  THEN
             ...
             qr_p(:,nys-1,:) = qr(:,nys-1,:)
          ENDIF
          IF ( ocean_mode  .AND.  salinity )  sa_p(:,nys-1,:) = sa(:,nys-1,:)
          IF ( air_chemistry  .AND.  nesting_chem )  THEN
             DO  n = 1, nspec
          ...
          IF ( salsa  .AND.  nesting_salsa )  THEN
             DO  lb = 1, nbins_aerosol
                aerosol_number(lb)%conc_p(:,nys-1,:) = aerosol_number(lb)%conc(:,nys-1,:)
             ENDDO
             DO  lc = 1, nbins_aerosol * ncomponents_mass
                aerosol_mass(lc)%conc_p(:,nys-1,:) = aerosol_mass(lc)%conc(:,nys-1,:)
             ENDDO
             IF ( .NOT. salsa_gases_from_chem )  THEN
                DO  lg = 1, ngases_salsa
                   salsa_gas(lg)%conc_p(:,nys-1,:) = salsa_gas(lg)%conc(:,nys-1,:)
                ENDDO
             ENDIF
          ENDIF
       ENDIF
!
!--    Set botom boundary conditions.
       u_p(nzb,:,:) = u(nzb,:,:)
       v_p(nzb,:,:) = v(nzb,:,:)
       w_p(nzb,:,:) = w(nzb,:,:)
       IF ( .NOT. neutral )   pt_p(nzb,:,:) = pt(nzb,:,:)
       IF ( humidity )        q_p(nzb,:,:) = q(nzb,:,:)
       IF ( passive_scalar )  s_p(nzb,:,:) = s(nzb,:,:)
       IF ( bulk_cloud_model  .AND.  microphysics_morrison )  THEN
          nc_p(nzb,:,:) = nc(nzb,:,:)
          qc_p(nzb,:,:) = qc(nzb,:,:)
       ENDIF
       IF ( bulk_cloud_model  .AND.  microphysics_seifert )  THEN
             ...
             qr_p(nzb,:,:) = qr(nzb,:,:)
          ENDIF
       IF ( ocean_mode  .AND.  salinity )  sa_p(nzb,:,:) = sa(nzb,:,:)
       IF ( air_chemistry  .AND.  nesting_chem )  THEN
          DO  n = 1, nspec
             chem_species(n)%conc_p(nzb,:,:) = chem_species(n)%conc(nzb,:,:)
          ENDDO
       ENDIF
       IF ( salsa  .AND.  nesting_salsa )  THEN
          DO  lb = 1, nbins_aerosol
             aerosol_number(lb)%conc_p(nzb,:,:) = aerosol_number(lb)%conc(nzb,:,:)
          ENDDO
          DO  lc = 1, nbins_aerosol * ncomponents_mass
             aerosol_mass(lc)%conc_p(nzb,:,:) = aerosol_mass(lc)%conc(nzb,:,:)
          ENDDO
          IF ( .NOT. salsa_gases_from_chem )  THEN
             DO  lg = 1, ngases_salsa
                salsa_gas(lg)%conc_p(nzb,:,:) = salsa_gas(lg)%conc(nzb,:,:)
             ENDDO
          ENDIF
       ENDIF
!
!--    Top boundary.
       ...
       IF ( bulk_cloud_model  .AND.  microphysics_seifert )  THEN
             ...
             qr_p(nzt+1,:,:) = qr(nzt+1,:,:)
          ENDIF
       IF ( ocean_mode  .AND.  salinity )  sa_p(nzt+1,:,:) = sa(nzt+1,:,:)
       IF ( air_chemistry  .AND.  nesting_chem )  THEN
          DO  n = 1, nspec
       ...
       IF ( salsa  .AND.  nesting_salsa )  THEN
          DO  lb = 1, nbins_aerosol
             aerosol_number(lb)%conc_p(nzt+1,:,:) = aerosol_number(lb)%conc(nzt+1,:,:)
          ENDDO
          DO  lc = 1, nbins_aerosol * ncomponents_mass
             aerosol_mass(lc)%conc_p(nzt+1,:,:) = aerosol_mass(lc)%conc(nzt+1,:,:)
          ENDDO
          IF ( .NOT. salsa_gases_from_chem )  THEN
             DO  lg = 1, ngases_salsa
                salsa_gas(lg)%conc_p(nzt+1,:,:) = salsa_gas(lg)%conc(nzt+1,:,:)
             ENDDO
          ENDIF
       ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_ensure_nest_mass_conservation
    ...
    REAL(wp) ::  sub_sum               !< intermediate sum for reducing the loss of signifigant digits in 2d summations
