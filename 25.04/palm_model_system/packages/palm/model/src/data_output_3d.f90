...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE data_output_3d( av )
    ...
    INTEGER(iwp) ::  flag_nr  !< number of masking flag, 0 = scalar, 1 = u, 2 = v, 3 = w
    ...
    LOGICAL ::  mask_topography  !< true if topography grid points should be masked (false e.g. in case of terrain following data)
...
!
!--    Allocate a temporary array with the desired output dimensions. Set it to FillValue, because
...
!--          Quantities of other modules
             IF ( .NOT. found )  THEN
                CALL module_interface_data_output_3d( av, trimvar, found, local_pf, resorted,      &
                                                      flag_nr, mask_topography, nzb_do, nzt_do )
             ENDIF
...
!--             For soil model quantities, it is required to re-allocate local_pf
                ...
                CALL lsm_data_output_3d( av, trimvar, found, local_pf, mask_topography )
                resorted = .TRUE.
...
!
!--          Temporary workaround: we let the SLUrb model to reallocate the array
             IF ( .NOT. found  .AND.  slurb )  THEN
!
!--             With SLUrb, there is no limitation that walls, windows, roads, roofs, etc. must have
!--             the same amount of layers. Thus, we let the output routine of the module to
!--             perform reallocation. Thus, we pass a non-allocated ALLOCATABLE local_pf as an
!--             argument for the subroutine. For DCEP this is implemented differently, as there
!--             is a workaround for it before this SELECT structure. However, as a future-proof
!--             solution, we let the module decide on the dimensionality here.
                DEALLOCATE ( local_pf )
                CALL slurb_data_output_3d( av, trimvar, found, local_pf, mask_topography, nzb_do,  &
                                           nzt_do, output_fill_value )

!
!--             Allocate back if it wasn't a SLUrb variable.
                IF ( .NOT. ALLOCATED( local_pf ) )  THEN
                   ALLOCATE( local_pf(nxl:nxr,nys:nyn,nzb_do:nzt_do) )
                   IF ( .NOT. data_output_raw )  local_pf = output_fill_value
                ENDIF
                resorted = .TRUE.
...
!--    Terrain following data must not be masked at all.
       IF ( .NOT. data_output_raw  .AND.  mask_topography )  THEN
          DO  i = nxl, nxr
             DO  j = nys, nyn
                DO  k = nzb_do, nzt_do
                   IF ( .NOT. BTEST( topo_flags(k,j,i), flag_nr ) )  THEN
                      local_pf(i,j,k) = output_fill_value
                   ENDIF
                ENDDO
             ENDDO
          ENDDO
