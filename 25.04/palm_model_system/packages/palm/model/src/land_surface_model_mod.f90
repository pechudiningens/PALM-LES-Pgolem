...
!-- LSM variables
    ...
    LOGICAL ::  constant_roughness = .TRUE.      !< use fixed/dynamic roughness lengths for water surfaces
...
!   otherwise -> no generic variable and user setting is optional
    ...
    REAL(wp) ::  lsm_start_time = 0.0_wp                       !< LSM is not called before this time
    ...
    IF ( TRIM( surface_type ) == 'netcdf'  .OR.  TRIM( surface_type ) == 'read_from_file' )  THEN
       IF ( pavement_type_f%from_file )  THEN
          IF ( ANY( pavement_type_f%var /= pavement_type_f%fill )  .AND.                           &
             ...
             CALL message( 'lsm_check_parameters', 'LSM0029', 2, 2, myid, 6, 0 )
          ENDIF
       ENDIF
    ENDIF
...
!-- Check if the static input *_pars variables are correctly dimensioned.
    IF ( pavement_pars_f%from_file )  THEN
       IF ( pavement_pars_f%np /= SIZE( pavement_pars, 1 ) )  THEN
    ...
    IF ( pavement_subsurface_pars_f%from_file )  THEN
       IF ( pavement_subsurface_pars_f%np /= npavement_subsurface_pars )  THEN
    ...
    IF ( vegetation_pars_f%from_file )  THEN
       IF ( vegetation_pars_f%np /= SIZE( vegetation_pars, 1 ) )  THEN
    ...
    IF ( water_pars_f%from_file )  THEN
       IF ( water_pars_f%np /= SIZE( water_pars, 1 ) )  THEN
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lsm_energy_balance
    ...
    calc_sm = ( .NOT. spinup_phase  .OR.  calc_soil_moisture_during_spinup )

!
!-- Recompute outgoing and net radiation in case of SLUrb+RTM combination.
    IF ( slurb )  CALL  lsm_recompute_outgoing_radiation
...
!--    oscillations, especially when setting radiation_start_time /= 0. The threshold value of 0.2
...
!-- Write land surface model header
    ...
    IF ( surface_type == 'read_from_file' )  THEN
       WRITE( io, 4 )  surface_type
       WRITE( io, 5 )  surface_type
    ELSEIF ( surface_type == 'vegetation' )  THEN
       WRITE( io, 4 ) TRIM( vegetation_type_name(vegetation_type) )
       WRITE( io, 5 ) TRIM( soil_type_name(soil_type) )
    ELSEIF ( surface_type == 'pavement' )  THEN
       WRITE( io, 4 ) TRIM( pavement_type_name(pavement_type) )
    ELSEIF ( surface_type == 'water' )  THEN
       WRITE( io, 4 ) TRIM( water_type_name(water_type) )
    ENDIF
...
!--          grid points take the classification at the given
             ...
             IF ( cut_cell_topography )  THEN
                IF ( surf_lsm%vtype(m) > 0 )  THEN
                   surf_lsm%vegetation_surface(m) = .TRUE.
                ELSEIF ( surf_lsm%ptype(m) > 0 )  THEN
                   surf_lsm%pavement_surface(m) = .TRUE.
                ELSEIF ( surf_lsm%wtype(m) > 0 )  THEN
...
!--       is implemented, so that relative fractions are either 1 or zero.
    ...
    ELSEIF ( input_pids_static  .AND.  .NOT. surface_fraction_f%from_file )  THEN
       DO  m = 1, surf_lsm%ns
          ...
          IF ( surf_lsm%vegetation_surface(m) )  surf_lsm%frac(m,ind_veg_wall)  = 1.0_wp
          IF ( surf_lsm%pavement_surface(m) )    surf_lsm%frac(m,ind_pav_green) = 1.0_wp
          IF ( surf_lsm%water_surface(m) )       surf_lsm%frac(m,ind_wat_win)   = 1.0_wp
       ENDDO
...
!-- assumed here.
    IF ( vegetation_type /= 0  .AND.  surface_type == 'vegetation' )  THEN
...
!-- Map values onto surface elemements
    ...
    ENDDO
!
!-- Level 2, initialization of vegation parameters via vegetation_type. In level 2,
!-- vegetation_type is provided by the static input file, either via rastered input for each
!-- (j,i)-pair, or via the cut-cell specific input for each surface individually. In the latter
!-- case, static input of rastered vegetation_type (if supplied) is overwritten by cut-cell
!-- specific input. Vegetation parameters are initialized using default paramter settings
!-- according to the given vegetation type.
    IF ( vegetation_type_f%from_file  .OR.  cut_cell_topography )  THEN
       DO  m = 1, surf_lsm%ns
          ...
          IF ( surf_lsm%vegetation_surface(m) )  THEN
             ...
             IF ( vegetation_type_f%from_file )  st = vegetation_type_f%var(j,i)
             IF ( cut_cell_topography         )  st = surf_lsm%vtype(m)
...
!--         not be used.
            ...
            ELSE
               ...
               IF ( vegetation_pars_f%pars_xy(ind_v_at,j,i) /= vegetation_pars_f%fill )           &
                  surf_lsm%albedo_type(m,ind_veg_wall) =                                          &
...
!-- here.
    IF ( water_type /= 0  .AND.  surface_type == 'water' )  THEN
...
!-- Map values onto surfaces
    ...
    ENDDO
!
!-- Level 2, initialization of water parameters via water_type. In level 2,
!-- water_type is provided by the static input file, either via rastered input for each
!-- (j,i)-pair, or via the cut-cell specific input for each surface individually. In the latter
!-- case, static input of rastered water_type (if supplied) is overwritten by cut-cell
!-- specific input. Water parameters are initialized using default paramter settings according
!-- to the given water type.
    IF ( water_type_f%from_file  .OR.  cut_cell_topography  )  THEN
       DO  m = 1, surf_lsm%ns
          IF ( surf_lsm%water_surface(m) )  THEN
             i = surf_lsm%i(m) + MERGE( 0, surf_lsm%ioff(m), surf_lsm%building_covered(m) )
             j = surf_lsm%j(m) + MERGE( 0, surf_lsm%joff(m), surf_lsm%building_covered(m) )

             IF ( water_type_f%from_file )  st = water_type_f%var(j,i)
             IF ( cut_cell_topography    )  st = surf_lsm%wtype(m)
...
!-- Initialize pavement-type surfaces, level 1
    IF ( pavement_type /= 0  .AND.  surface_type == 'pavement' )  THEN
!
...
!-- assumed.
    ...
    ENDDO
!
!-- Level 2, initialization of pavement parameters via vegetation_type. In level 2,
!-- pavement_type is provided by the static input file, either via rastered input for each
!-- (j,i)-pair, or via the cut-cell specific input (if supplied) for each surface individually.
!-- In the latter case, static input of rastered pavement_type is overwritten by cut-cell
!-- specific input. Pavement parameters are initialized using default paramter settings according
!-- to the given pavement type.
    IF ( pavement_type_f%from_file  .OR.  cut_cell_topography )  THEN
       DO  m = 1, surf_lsm%ns
          IF ( surf_lsm%pavement_surface(m) )  THEN
             i = surf_lsm%i(m) + MERGE( 0, surf_lsm%ioff(m), surf_lsm%building_covered(m) )
             j = surf_lsm%j(m) + MERGE( 0, surf_lsm%joff(m), surf_lsm%building_covered(m) )

             IF ( pavement_type_f%from_file )  st = pavement_type_f%var(j,i)
             IF ( cut_cell_topography       )  st = surf_lsm%ptype(m)
...
!--             Determine deepmost index of pavement layer
                ...
                DO  k = nzb_soil, surf_lsm%nzt_pavement(m)
                   surf_lsm%lambda_h_def(k,m)    = pavement_subsurface_pars_1(k,st)
                   surf_lsm%rho_c_total_def(k,m) = pavement_subsurface_pars_2(k,st)
                ENDDO
             ENDIF
          ENDIF
       ENDDO
...
!--          used.
             ...
             ELSE
                ...
                IF ( pavement_pars_f%pars_xy(ind_p_at,j,i) /= pavement_pars_f%fill )               &
                   surf_lsm%albedo_type(m,ind_pav_green) =                                         &
...
!--          level-of-detail 2 - read 3D initialization data
             ELSEIF ( init_3d%lod_msoil == 2 )  THEN
               ...
               CALL get_variable( pids_id, 'init_soil_m',                                          &
                                  ...
                                  nxl, nxr, nys, nyn, 0, init_3d%nzs-1, nbgp=0 )
...
!--    The following actions must not be executed when an additional child is activated.
       IF ( nested_run )  THEN
#if defined( __parallel )
          IF ( init_child_id == -1 )  THEN
             CALL pmc_get_model_info( root_model = root_model )
!
...
!--          In case of a nested run, first average the soil profiles in the root domain.
             IF ( init_msoil_from_driver_root  .OR.  init_tsoil_from_driver_root )  THEN
                ...
                IF ( root_model )  THEN
!
!--                Child domains will be only initialized with horizontally averaged soil profiles
!--                from the parent domain (for sake of simplicity).
!--                If required, average soil data on root parent domain before the soil profiles
!--                are distributed onto the child domains.
...
!--                Proceed with soil temperature.
                   IF ( init_3d%from_file_tsoil  .AND.  init_3d%lod_tsoil == 2 )  THEN
                      ...
                      ENDDO
!
!--                   Allocate 1D array for soil-temperature profile (will not be allocated in
!--                   lod==2 case).
                      ALLOCATE( init_3d%tsoil_1d(0:init_3d%nzs-1) )
...
!--             Distribute the mean soil profiles to all child domains.
                ...
                CALL MPI_BCAST( z_soil_root, SIZE( z_soil_root ), MPI_REAL, 0, MPI_COMM_WORLD,     &
                                ierr )
                ...
                IF ( init_tsoil_from_driver_root )                                                 &
                   CALL MPI_BCAST( t_soil_root, SIZE( t_soil_root ), MPI_REAL, 0, MPI_COMM_WORLD,  &
                                   ierr )
!
!--             In the following, the child domains decide whether they take the information from
!--             the root domain or not.
                IF ( .NOT. root_model )  THEN
!
!--                If soil moisture or temperature isn't in dynamic input file for the child, take
!--                the information provided from the root model.
...
!--       Assure that r_a cannot be zero at model start
          IF ( surf_lsm%pt1(m) == surf_lsm%pt_surface(m) )  THEN
...
!
!-- Initialize aggregated fluxes (for initial run and restarts).
    DO  m = 1, surf_lsm%ns
...
!
!-- Allocate aggregated atmosphere-surface fluxes
...
! Description:
! ------------
!> Workaround subroutine to recompute the radiative fluxes for LSM in case SLUrb is enabled.
!> In this case, the outoging LW and SW flux contain also the urban contribution, whereas
!> the non-urban non-weighted flux is needed for LSM. The output values are not affected
!> as the radiation model is called after LSM and SLUrb, thus the output is always aggregated.
!--------------------------------------------------------------------------------------------------!
SUBROUTINE lsm_recompute_outgoing_radiation
    ...
    DO  m = 1, surf_lsm%ns
       k = surf_lsm%k(m)
!
!--    Outgoing SW radiation from albedo and incoming SW radiation.
       surf_lsm%rad_sw_out(m) = MERGE( SUM( surf_lsm%frac(m,:) * surf_lsm%albedo(m,:) )            &
                                          * surf_lsm%rad_sw_in(m),                                 &
                                       surf_lsm%rad_sw_out(m),                                     &
                                       surf_lsm%upward(m) )
!
!--    Outgoing LW radiation from emissivity and surface temperature.
       surf_lsm%rad_lw_out(m) = MERGE( SUM( surf_lsm%frac(m,:) * surf_lsm%emissivity(m,:) )        &
                                       * sigma_sb * ( surf_lsm%pt_surface(m) * exner(k) )**4,      &
                                       surf_lsm%rad_lw_out(m),                                     &
                                       surf_lsm%upward(m) )
!
!--    Linearization term for outgoing LW radiation. N.B. This is currently not computed in RTM,
!--    so this computation is just to be sure in the case of it is added there at some point.
       surf_lsm%rad_lw_out_change_0(m) = MERGE( SUM( surf_lsm%frac(m,:)                            &
                                                     * surf_lsm%emissivity(m,:) )                  &
                                                * 4.0_wp * sigma_sb                                &
                                                * ( surf_lsm%pt_surface(m) * exner(k) )**3,        &
                                                surf_lsm%rad_lw_out_change_0(m),                   &
                                                surf_lsm%upward(m) )
!
!--    Net radiation on LSM surface.
       surf_lsm%rad_net(m) = MERGE( surf_lsm%rad_sw_in(m) + surf_lsm%rad_sw_in(m)                  &
                                    - surf_lsm%rad_sw_out(m) - surf_lsm%rad_lw_out(m),             &
                                    surf_lsm%rad_net(m),                                           &
                                    surf_lsm%upward(m) )
    ENDDO

END SUBROUTINE lsm_recompute_outgoing_radiation
...
!> Subroutine defining 3D output variables
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lsm_data_output_3d( av, variable, found, local_pf, mask_topography )
...
! Description:
! ------------
!> Calculate maximum allowed timestep at LSM surfaces according to the diffusion criterion.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lsm_timestep

    INTEGER(iwp) ::  k  !< grid index - soil depth
    INTEGER(iwp) ::  m  !< running index for surface elements

    REAL(wp), DIMENSION(nzb_soil:nzt_soil) ::  max_dt_soil_column  !< allowed timestep for each vertical level of a soil column

    REAL(wp), DIMENSION(:), ALLOCATABLE ::  max_dt_lsm  !< maximum allowed timestep for each lsm surface


    ALLOCATE( max_dt_lsm(1:surf_lsm%ns) )
!
!-- Determine allowed timestep according a diffusion criterion. This is only required for
!-- the last substep.
!-- ATTENTION: Since loop order has been changed, it is unclear if the nointerchange directive
!--            is required any more! Must be tested on NEC-Systems.
    !$OMP PARALLEL DO PRIVATE (m, k, max_dt_soil_column) SCHEDULE (STATIC)
!NEC$ nointerchange
    DO  m = 1, surf_lsm%ns
       IF ( .NOT. surf_lsm%water_surface(m) )  THEN
          DO  k = nzb_soil, nzt_soil
             max_dt_soil_column(k) = surf_lsm%rho_c_total(k,m) / surf_lsm%lambda_h(k,m) *          &
                                     ( dz_soil(k) )**2
          ENDDO
          max_dt_lsm(m) = MINVAL( max_dt_soil_column(:) )
       ELSE
!
!--       The diffusion criterion won't be evaluated at water surfaces. To guarantee that water
!--       surfaces do not impact the timestep, set the maximum allowed timestep to a huge number.
          max_dt_lsm(m) = HUGE( 1.0_wp )
       ENDIF
    ENDDO

!
!-- Consider a pre-factor (1/8) for the diffusion criterion.
    dt_lsm = MINVAL( max_dt_lsm ) * 0.125_wp
#if defined( __parallel )
    IF ( collective_wait )  CALL MPI_BARRIER( comm2d, ierr )
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, dt_lsm, 1, MPI_REAL, MPI_MIN, comm2d, ierr )
#endif

    DEALLOCATE( max_dt_lsm )
