...
!--------------------------------------------------------------------------------------------------!
 MODULE topography_mod
    ...
    INTEGER(iwp), DIMENSION(:),     ALLOCATABLE ::  build_ids_unique !< list of building IDs in the domain
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE filter_topography
    ...
    INTEGER(iwp) ::  num_hole        !< number of holes (in topography) resolved by only one grid point detected during one sweep
    INTEGER(iwp) ::  num_hole_total  !< total number of holes (in topography) resolved by only one grid point detected during all sweeps
    INTEGER(iwp) ::  num_wall        !< number of surrounding vertical walls for a single grid point
    INTEGER(iwp) ::  sweep           !< counts how often the topography filter is apllied
...
!-- Issue an informative message if any 1-grid point wide holes were filled.
    IF ( num_hole_total > 0 )  THEN
       WRITE( message_string, '(A,I6,A,A,I2,A)' )  'topography was filtered: &', num_hole_total,   &
