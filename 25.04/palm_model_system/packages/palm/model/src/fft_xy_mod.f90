...
!--          (prime_factors(7) = 1) has been reached.
             IF ( factor < 2 )  THEN
                ...
                IF ( bc_ns_cyc )  THEN
                   WRITE( message_string, '(A,I5,A,I6,A)' )                                        &
                         'ny+1 = ', ny+1, ' contains prime factor ', n, '&poor performance expected'
                ELSE
                   WRITE( message_string, '(A,I5,A,I6,A)' )                                        &
                         'ny = ', ny, ' contains prime factor ', n, '&poor performance expected'
                ENDIF
                CALL message( 'fft_init', 'PAC0203', 0, 1, 0, 6, 0 )
...
!--                Vector version of the Temperton-algorithm. Computes multiple 1-D FFT's.
       ...
       ELSEIF ( fft_method == 'system-specific' )  THEN
          ...
          IF ( forward_fft )  THEN
             ...
             !$ACC PRESENT(ar, ar_tmp) DEFAULT(NONE) IF(enable_openacc)
             DO  k = nzb_x, nzt_x
                DO  j = nys_x, nyn_x
