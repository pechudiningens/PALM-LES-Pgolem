...
!--          External pressure gradient
             IF ( dp_external )  THEN
                DO  k = dp_level_ind_b+1, nzt
                   tend(k,j,i) = tend(k,j,i) - dpdxy(1) * drho_air(k) * dp_smooth_factor(k)
                ENDDO
             ENDIF
...
!
!--          Apply inertial oscillation damping.
             IF ( dmp_enabled )  THEN
                DO  k = nzb+1, nzt
                   u_p(k,j,i) = u_p(k,j,i) + tsc(5) * tend_dmp(k)
                ENDDO
             ENDIF
!
...
!--          External pressure gradient
             IF ( dp_external )  THEN
                DO  k = dp_level_ind_b+1, nzt
                   tend(k,j,i) = tend(k,j,i) - dpdxy(2) * drho_air(k) * dp_smooth_factor(k)
                ENDDO
             ENDIF
...
!--          Apply inertial oscillation damping.
             IF ( dmp_enabled )  THEN
                DO  k = nzb+1, nzt
                   v_p(k,j,i) = v_p(k,j,i) + tsc(5) * tend_dmp(k)
                ENDDO
             ENDIF
!
!--          Add turbulence generated by wave breaking (in ocean mode only)
...
!--          Tendency terms for potential temperature
             ...
             ENDIF
             CALL diffusion_s( i, j, pt, surf_top%shf, surf_def%shf, surf_lsm%shf_agg,             &
                               surf_usm%shf )
...
!--          If required, add tendency due to radiative heating/cooling
             IF ( radiation  .AND.  simulated_time > radiation_start_time )  THEN
...
!--          Tendency-terms for total water content / scalar
             ...
             ENDIF
             CALL diffusion_s( i, j, q, surf_top%qsws, surf_def%qsws,surf_lsm%qsws_agg,            &
                               surf_usm%qsws )
...
!--          large_scale_forcing, which is to date not implemented for scalars.
             IF ( large_scale_subsidence  .AND.  .NOT. use_subsidence_tendencies  .AND.            &
                ...
                CALL subsidence( i, j, tend, s, s_init )
             ENDIF
...
!-- u-velocity component
    ...
    !$ACC KERNELS PRESENT(tend) IF(enable_openacc)
    tend = 0.0_wp
    !$ACC END KERNELS
    IF ( timestep_scheme(1:5) == 'runge' )  THEN
       IF ( ws_scheme_mom )  THEN
...
!-- Prognostic equation for u-velocity component
    ...
    !$ACC PRESENT(tsc(2:5)) &
    !$ACC PRESENT(u_p) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxlu, nxr
       DO  j = nys, nyn
!
...
!-- Calculate tendencies for the next Runge-Kutta step
    IF ( timestep_scheme(1:5) == 'runge' )  THEN
       IF ( intermediate_timestep_count == 1 )  THEN
          !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
          !$ACC PRESENT(tend, tu_m) DEFAULT(NONE) IF(enable_openacc)
          DO  i = nxlu, nxr
             DO  j = nys, nyn
                DO  k = nzb+1, nzt
                   tu_m(k,j,i) = tend(k,j,i)
       ...
       ELSEIF ( intermediate_timestep_count < intermediate_timestep_count_max )  THEN
          !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
          !$ACC PRESENT(tend, tu_m) IF(enable_openacc)
          DO  i = nxlu, nxr
             DO  j = nys, nyn
                DO  k = nzb+1, nzt
                   tu_m(k,j,i) = - 9.5625_wp * tend(k,j,i) + 5.3125_wp * tu_m(k,j,i)
...
!-- Prognostic equation for v-velocity component
    ...
    !$ACC PRESENT(tsc(2:5)) &
    !$ACC PRESENT(v_p) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxl, nxr
       DO  j = nysv, nyn
!
!--       Following directive is required to vectorize on Intel19
...
!-- Calculate tendencies for the next Runge-Kutta step
    IF ( timestep_scheme(1:5) == 'runge' )  THEN
       IF ( intermediate_timestep_count == 1 )  THEN
          !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
          !$ACC PRESENT(tend, tv_m) DEFAULT(NONE) IF(enable_openacc)
          DO  i = nxl, nxr
             DO  j = nysv, nyn
                DO  k = nzb+1, nzt
                   tv_m(k,j,i) = tend(k,j,i)
       ...
       ELSEIF ( intermediate_timestep_count < intermediate_timestep_count_max )  THEN
          !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
          !$ACC PRESENT(tend, tv_m) IF(enable_openacc)
          DO  i = nxl, nxr
             DO  j = nysv, nyn
                DO  k = nzb+1, nzt
                   tv_m(k,j,i) = - 9.5625_wp * tend(k,j,i) + 5.3125_wp * tv_m(k,j,i)
...
!-- Prognostic equation for w-velocity component
    ...
    !$ACC PRESENT(tsc(2:5)) &
    !$ACC PRESENT(w_p) DEFAULT(NONE) IF(enable_openacc)
    DO  i = nxl, nxr
       DO  j = nys, nyn
!
!--       Following directive is required to vectorize on Intel19
...
!-- Calculate tendencies for the next Runge-Kutta step
    IF ( timestep_scheme(1:5) == 'runge' )  THEN
       IF ( intermediate_timestep_count == 1 )  THEN
          !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
          !$ACC PRESENT(tend, tw_m) DEFAULT(NONE) IF(enable_openacc)
          DO  i = nxl, nxr
             DO  j = nys, nyn
                DO  k = nzb+1, nzt-1
                   tw_m(k,j,i) = tend(k,j,i)
       ...
       ELSEIF ( intermediate_timestep_count < intermediate_timestep_count_max )  THEN
          !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
          !$ACC PRESENT(tend, tw_m) IF(enable_openacc)
          DO  i = nxl, nxr
             DO  j = nys, nyn
                DO  k = nzb+1, nzt-1
                   tw_m(k,j,i) = - 9.5625_wp * tend(k,j,i) + 5.3125_wp * tw_m(k,j,i)
...
!--    pt-tendency terms with no communication
       ...
       CALL diffusion_s( pt, surf_top%shf, surf_def%shf, surf_lsm%shf_agg, surf_usm%shf )
...
!--    Prognostic equation for potential temperature
       ...
       !$ACC PRESENT(tsc(3:5)) &
       !$ACC PRESENT(pt_p) DEFAULT(NONE) IF(enable_openacc)
       DO  i = nxl, nxr
          DO  j = nys, nyn
!
!--          Following directive is required to vectorize on Intel19
...
!--    Calculate tendencies for the next Runge-Kutta step
       IF ( timestep_scheme(1:5) == 'runge' )  THEN
          IF ( intermediate_timestep_count == 1 )  THEN
             !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
             !$ACC PRESENT(tend, tpt_m) DEFAULT(NONE) IF(enable_openacc)
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb+1, nzt
                      tpt_m(k,j,i) = tend(k,j,i)
          ...
          ELSEIF ( intermediate_timestep_count < intermediate_timestep_count_max )  THEN
             !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(i, j, k) &
             !$ACC PRESENT(tend, tpt_m) IF(enable_openacc)
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb+1, nzt
                      tpt_m(k,j,i) = - 9.5625_wp * tend(k,j,i) + 5.3125_wp * tpt_m(k,j,i)
...
!--    Scalar/q-tendency terms with no communication
       IF ( scalar_advec /= 'bc-scheme' )  THEN
          ...
          IF ( timestep_scheme(1:5) == 'runge' )  THEN
             IF ( ws_scheme_sca )  THEN
                !$ACC DATA PRESENT(q) IF(enable_openacc)
                IF ( .NOT. advanced_div_correction )  THEN
       ...
       CALL diffusion_s( q, surf_top%qsws, surf_def%qsws, surf_lsm%qsws_agg, surf_usm%qsws )
...
!--    Prognostic equation for total water content
       ...
       !$ACC PRESENT(q, q_init, q_p, rdf_sc, tend, topo_flags, tq_m, tsc(3:5)) &
...
!
!--    Scalar-tendency terms with communication
       sbt = tsc(2)
       IF ( scalar_advec == 'bc-scheme' )  THEN
...
!
!--    Scalar-tendency terms with no communication
       IF ( scalar_advec /= 'bc-scheme' )  THEN
          tend = 0.0_wp
          IF ( timestep_scheme(1:5) == 'runge' )  THEN
             IF ( ws_scheme_sca )  THEN
