...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE chem_emis_nonstationary_init
    ...
    CHARACTER(LEN=512) ::  ch  !< dummy character
    ...
    INTEGER(iwp) ::  char_len    !< number of characters (letters or digits) in input string
    ...
    INTEGER(iwp) ::  str_len     !< string length given in input file
...
!-- Read character length - required to process input characters from NetCDF file.
    CALL get_dimension_length( pids_emis_nonstat_id, str_len, 'field_length' )
!
...
!--    Allocate memory for emission species and read the strings.
       ...
       emis_ns(n)%emis_species = ''

       CALL get_variable( pids_emis_nonstat_id, 'species' // TRIM( dum ), buf_char,                &
                          emis_ns(n)%num_emis_species, str_len )
!
!--    Store temporary string for species name on data structure.
       DO  m = 0, emis_ns(n)%num_emis_species-1
          ch = buf_char(m+1)
!
!--       Determine the number of letter or digit-based characters in the given string.
          DO  k = 1, str_len
!
!--          Digit.
             IF ( IACHAR( ch(k:k) ) >= 48  .AND.  IACHAR( ch(k:k) ) <= 57 )  THEN
                char_len = k
!
!--          Small or capital letter.
             ELSEIF ( ( IACHAR( ch(k:k) ) >= 65  .AND.  IACHAR( ch(k:k) ) <= 90 )  .OR.            &
                      ( IACHAR( ch(k:k) ) >= 97  .AND.  IACHAR( ch(k:k) ) <= 122 ) )  THEN
                char_len = k
             ENDIF
          ENDDO
          emis_ns(n)%emis_species(m) = ch(1:char_len)
       ENDDO
!
!--    Deallocate buf_char (allocated in get_variable).
       DEALLOCATE( buf_char )
...
!--    respect to prognostic variable.
       ...
       DO  l = 1, nvar
          DO  m = 0, emis_ns(n)%num_emis_species-1
             IF ( TRIM( chem_species(l)%name ) == TRIM( emis_ns(n)%emis_species(m) ) )  THEN
...
!--    next timestamp.
       ...
       CALL get_variable( pids_emis_nonstat_id, 'vsrc' // TRIM( dum ) // '_eutm', emis_ns(n)%eutm, &
                          0, emis_ns(n)%num_vsrc_pos-1, emis_ns(n)%ts, emis_ns(n)%tsp, nbgp=0 )
       ...
       CALL get_variable( pids_emis_nonstat_id, 'vsrc' // TRIM( dum ) // '_zag', emis_ns(n)%zag,   &
                          0, emis_ns(n)%num_vsrc_pos-1, emis_ns(n)%ts, emis_ns(n)%tsp, nbgp=0 )
!
!--    Refer emission locations to numerical grid and rotate accordingly.
...
!--       Update coordinates.
          CALL get_variable( pids_emis_nonstat_id, 'vsrc' // TRIM( dum ) // '_eutm',               &
                             ...
                             emis_ns(n)%ts, emis_ns(n)%tsp, nbgp=0 )
