...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE parin
    ...
    USE netcdf_interface,                                                                          &
        ONLY:  netcdf_compression_level,                                                           &
               netcdf_compression_nsd,                                                             &
               netcdf_data_format,                                                                 &
               netcdf_precision
    ...
    LOGICAL ::  bc_lr_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally
    LOGICAL ::  bc_ns_set_internally = .FALSE.  !< flag to indicate if boundary condition has been set internally

!> @todo remove this variable with next release (24.10)
    REAL(wp) ::  dt_spinup = HUGE( 1.0_wp ) !< initialization-parameter, which has no effect any more
...
!-- set_constant_profiles-branch in init_3d_model is executed even though the constant-profiles
...
!-- children. This arrangement means that initializing_actions never can be 'by user' for any
!-- child domain.
#if defined( __parallel )
    IF ( nested_run )  THEN
...
!
!--    Check, which initialization is required for the child in this run, and re-define
!--    initializing_actions appropriately. In a normal restart run, it must be set
!--    to 'read_restart_data', but if a new child is activated in a restart run for the first time,
!--    it must be set 'interpolate_from_parent'.
       IF ( child_domain )  THEN

!
!--       Check if this child needs to be activated and initialized in this run.
          CALL pmc_get_model_info( cpl_id = my_cpl_id )

          IF ( INDEX( root_initializing_actions, 'read_restart_data' ) /= 0  .AND.                 &
               init_child_id /= my_cpl_id )                                                        &
          THEN
             ...
             ENDIF
          ELSEIF ( INDEX( root_initializing_actions, 'read_restart_data' ) /= 0  .AND.             &
               init_child_id == my_cpl_id )                                                        &
          THEN
...
!
!-- Issue a warning, if dt_spinup (which has no effect any more) has been set.
    IF ( dt_spinup /= HUGE( 1.0_wp ) )  THEN
       message_string = 'parameter "dt_spinup" has no effect any more, &' //                       &
                        'please remove it from the initialization_parameters namelist, &' //       &
                        'during spinup, the timestep is automatically calculated&and adjusted now'
       CALL message( 'parin', 'PAC0363', 0, 1, 0, 6, 0 )
    ENDIF

!
!-- Determine the number of user-defined profiles and append them to the standard data output
!-- (data_output_pr).
!-- TODO: This block should belong to the user-module (e.g. user_parin), but data_output_pr
!--       is not available at the time the user parameters are read. Moving to user_init does not
!--       work too, because data_output_pr must already be available in check_parameters.
!--       The workflow for processing profiles (also from other modules) requires a complete
!--       revision to fit into the modular structure. So far, profiles defined in modules
!--       have to be set in the runtume_parameters namelist and not in the respective module
!--       namelist!
...
!--    calculated further above, and max_pr_user is read in via rrd_global.
!--    TODO: This check needs to be moved to the user-interface.
       IF ( max_pr_user /= max_pr_user_tmp )  THEN
          WRITE( message_string, '(2A,I4,2A,I4,A)' ) 'the number of user-defined profiles given ', &
                                                     'in data_output_pr (', max_pr_user_tmp,       &
                                                     ')& does not match the one found in the ',    &
                                                     'restart file (', max_pr_user, ')'
...
!
!-- If this line is reached, the number of user-defined profiles on restart file and namelist file
!-- matches.
    max_pr_user = max_pr_user_tmp
...
#if defined( __parallel )
!
!-- Actions if a child is activated in a restart run for the first time.
!-- in childs, and synchronize the time to be used at the beginning of time-stepping.
    IF ( init_child_id /= -1 )  THEN
!
!--    Childs activated in a restart run for the first time are not allowed to use a spinup.
!--    This is just because the time handling for the spinup is a bit difficult in such a case.
       IF ( child_domain  .AND.  spinup_time > 0.0_wp )  THEN
          message_string = 'mismatch between spinup and nesting'
          CALL message( 'parin', 'PAC0357', 1, 2, 0, 6, 0 )
       ENDIF
!
!--    Synchronize the time in the newly activated child to be used at the beginning of
!--    time-stepping.
!--    Synchronization is done here not only for the new child but for all other childs, too,
!--    but that is not really required, because the time for the other childs should be
!--    already synchronized (i.e. read from the restart file).
       CALL MPI_BCAST( simulated_time, 1, MPI_REAL, 0, comm_world_nesting, ierr )
       CALL MPI_BCAST( time_since_reference_point, 1, MPI_REAL, 0, comm_world_nesting, ierr )
!
!--    The following setting forces coupling between the models at the beginning of the restart.
       nesting_coupling_time = time_since_reference_point
...
!
!-- Check if nested boundary conditions have been set for non-allowed cases.
    IF ( bc_lr == 'nested'  .AND.  .NOT. bc_lr_set_internally )  THEN
       message_string = 'unknown boundary condition: bc_lr = "' // TRIM( bc_lr ) // '"'
       CALL message( 'parin', 'PAC0258', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( bc_ns == 'nested'  .AND.  .NOT. bc_ns_set_internally )  THEN
...
!
!--    Also check for negative horizontal grid spacings, because they will generate various errors.
       IF ( dx <= 0.0_wp )  THEN
          WRITE( message_string, * ) 'illegal value given for horizontal grid spacing dx: ', dx
          CALL message( 'parin', 'PAC0365', 1, 2, 0, 6, 0 )
       ENDIF
       IF ( dy <= 0.0_wp )  THEN
          WRITE( message_string, * ) 'illegal value given for horizontal grid spacing dy: ', dy
          CALL message( 'parin', 'PAC0365', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
