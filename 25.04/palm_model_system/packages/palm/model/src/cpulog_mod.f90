...
!------------------------------------------------------------------------------!
    SUBROUTINE cpu_statistics
       ...
       CHARACTER(LEN=80) ::  msgstr  !< string used for output of information
...
!--       Output of handling of MPI operations
          ...
          IF ( synchronous_exchange )  THEN
             msgstr = 'Exchange of ghostpoints via MPI_SENDRCV'
          ELSE
             msgstr = 'Exchange of ghostpoints via MPI_ISEND/MPI_IRECV'
          ENDIF
          IF ( use_contiguous_buffer )  msgstr = TRIM( msgstr ) // ' using contiguous buffer'
          WRITE ( 18, 105 )  TRIM( msgstr )
!
!--       Output of restart file size and data transfer speed.
          ...
          ENDIF
!
!--       Output of 3d-netCDF file size and data transfer speed.
          ...
          THEN
             ...
             IF ( TRIM( netcdf_compression_method ) /= 'none' )  THEN
                WRITE ( 18, 112 )  TRIM( netcdf_compression_method )
             ENDIF
          ENDIF
!
...
!--       Unit 18 is not needed anymore
   ...
   112 FORMAT (/'                     compression method "',A,'" is used')
