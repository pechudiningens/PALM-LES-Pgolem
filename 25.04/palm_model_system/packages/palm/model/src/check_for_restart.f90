...
!-- If necessary set a flag to stop the model run.
    ...
    IF ( remaining_time <= termination_time_needed  .AND.  write_binary )  terminate_run_l = .TRUE.
...
!-- Set the global communicator to be used (depends on the mode in which PALM is running).
    ...
    ENDIF
!
!-- In a nested simulation, communication via the global communicator shall only be done when
!-- coupling time has been reached, otherwise the following MPI_ALLREDUCE would procude a deadlock.
    IF ( .NOT. nested_run  .OR.  ( nested_run  .AND.  nesting_coupling_time_reached ) )  THEN
#if defined( __parallel )
!
!--    Make a logical OR for all processes. Stop the model run if at least one process has reached
!--    the time limit.
...
#if defined( __parallel )
!
!--    Make a logical OR for all processes. Stop the model run if a flag file has been detected
!--    above.
       IF ( collective_wait )  CALL MPI_BARRIER( global_communicator, ierr )
       CALL MPI_ALLREDUCE( terminate_run_l, terminate_run, 1, MPI_LOGICAL, MPI_LOR,                &
                           global_communicator, ierr )
...
!
!-- Set the stop flag also, if restart is forced by user settings. Nested domains must only stop
!-- if they have reached the nesting_coupling_time, otherwise some of them may stop before that
!-- time, which would cause a deadlock.
    IF ( time_restart /= 9999999.9_wp  .AND.  time_restart < time_since_reference_point  .AND.     &
         nesting_coupling_time_reached )                                                           &
    THEN
!
