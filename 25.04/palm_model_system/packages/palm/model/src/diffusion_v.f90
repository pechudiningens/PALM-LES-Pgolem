...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE diffusion_v
       ...
       !$ACC PRESENT(tend) &
       !$ACC PRESENT(surf_v, surf_v%westward, surf_v%eastward, surf_v%upward, surf_v%downward) &
       !$ACC PRESENT(surf_v%start_index, surf_v%end_index, surf_v%k, surf_v%vsus, surf_v%vsws) &
...
!--          are no wall grid points at given (j,i), loops won't be entered.
!--          For the v-component tendency, v'u' is added at walls
...
!
!--             In case of cut-cell topography, momentum fluxes are stored on dedicated surface
!--             types as the number of surfaces that correspond to the staggered grid points
!--             can be different compared to the classical surface structure associated with
!--             the grid center. In the finite step-like topography, the classical surface types
!--             are used instead.
...
!--             Cut-cell branch.
                ELSE
                   surf_s = surf_v%start_index(j,i)
                   surf_e = surf_v%end_index(j,i)
                   !$ACC LOOP PRIVATE(k, m)
                   DO  m = surf_s, surf_e
                      k           = surf_v%k(m)

                      flux_r(k)   = MERGE( surf_v%vsus(m), flux_r(k), surf_v%westward(m) )
                      flux_l(k)   = MERGE( surf_v%vsus(m), flux_l(k), surf_v%eastward(m) )
                      flux_t(k)   = MERGE( surf_v%vsws(m), flux_t(k), surf_v%downward(m) )
                      flux_d(k)   = MERGE( surf_v%vsws(m), flux_d(k), surf_v%upward(m)   )
                   ENDDO
                ENDIF
             ENDIF
!
!--          Add momentum flux at model top
             IF ( use_top_fluxes  .AND.                                                            &
                  ( constant_top_momentumflux  .OR.  ocean_run_coupled_to_atmosphere ) )           &
             THEN
...
!--    points at given (j,i), loops won't be entered.
