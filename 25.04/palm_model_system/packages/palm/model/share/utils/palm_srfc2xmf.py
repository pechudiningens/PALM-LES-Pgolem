...
if rounding_time_vals:
    if len(time_sec) > 1:
        ...
        decimals = int(np.round(-np.log10(dt/100.))) # round times to match the avg timestep plus one digit, never more than to the second
        if decimals > 0:
...
if dx is None:
    D_horizontal = D.isel(s=D.zenith==0)
    dx = np.unique(D_horizontal.xs.data)[1] - np.unique(D_horizontal.xs.data)[0]
if dy is None:
    D_horizontal = D.isel(s=D.zenith==0)
    dy = np.unique(D_horizontal.ys.data)[1] - np.unique(D_horizontal.ys.data)[0]
if dz is None:
    D_vertical = D.isel(s=D.zenith==90)
    dz = np.unique(D_vertical.zs.data)[1] - np.unique(D_vertical.zs.data)[0]
